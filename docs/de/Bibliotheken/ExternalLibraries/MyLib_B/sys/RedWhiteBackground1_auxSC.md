...
`RedWhiteBackground1_auxSC` ist ein **Kompakt-Wrapper** um [`RedWhiteBackground1_auxS`](RedWhiteBackground1_auxS.md) — reicht `u16ObjId`s und den Selector durch, ohne die `CNF`-Ereignisausgänge und Diagnosewerte (`STATUS_n`, `u8OldColour_n`, `result_n`) nach außen zu führen.
