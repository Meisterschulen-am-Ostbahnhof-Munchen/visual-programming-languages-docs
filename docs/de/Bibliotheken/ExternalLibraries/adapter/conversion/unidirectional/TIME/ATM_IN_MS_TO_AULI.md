...
| OUT | Plug | adapter::types::unidirectional::AULI | Unidirektionaler `ULINT`-Adapter als Ausgangsschnittstelle, Wert von `IN` in Millisekunden ausgedrückt. |

## Funktionsweise

Der Baustein enthält intern einen einzelnen Funktionsbaustein `F_TIME_IN_MS_TO_ULINT` aus der IEC-61131-3-Standardbibliothek. Die Ablaufsteuerung erfolgt rein ereignisgesteuert:

1. Ein Ereignis am Eingangsadapter **IN.E1** triggert den Eingang **REQ** des internen Umrechnungsbausteins.
2. Der Datenwert **IN.D1** (`TIME`) wird an den Eingang **IN** von `F_TIME_IN_MS_TO_ULINT` übergeben.
3. `F_TIME_IN_MS_TO_ULINT` rechnet den Zeitwert in eine Millisekunden-Zahl vom Typ `ULINT` um und signalisiert dies über seinen Ausgang **CNF**.
4. Dieses Ereignis wird an den Ausgangsadapter **OUT.E1** weitergeleitet.
5. Gleichzeitig wird der umgerechnete Wert (Ausgang **OUT** von `F_TIME_IN_MS_TO_ULINT`) an den Datenpunkt **OUT.D1** übergeben.
...
- **Einheitenumrechnung nach IEC 61131-3**: Der interne Baustein `F_TIME_IN_MS_TO_ULINT` interpretiert den intern in Nanosekunden gehaltenen `TIME`-Wert als Zahl in Millisekunden und liefert das Ergebnis als `ULINT` (64-Bit-Ganzzahl (vorzeichenlos)).
...
- **Anzeige/Weiterverarbeitung von Zeitdauern**: Eine über einen `ATM`-Adapter transportierte Zeitdauer (z. B. eine Zykluszeit oder Verzögerung) soll als Zahl in Millisekunden an einen Adapter-Socket vom Typ `AULI` weitergegeben werden, etwa zur Anzeige auf einem Panel oder zur Weiterverarbeitung in einer Berechnung.
...
- **Gleiche Einheit, andere Zieltypen**: [ATM_IN_MS_TO_ADI](ATM_IN_MS_TO_ADI.md), [ATM_IN_MS_TO_ALI](ATM_IN_MS_TO_ALI.md), [ATM_IN_MS_TO_ALR](ATM_IN_MS_TO_ALR.md), [ATM_IN_MS_TO_AUDI](ATM_IN_MS_TO_AUDI.md)
- **Gleicher Zieltyp, andere Einheiten**: [ATM_IN_NS_TO_AULI](ATM_IN_NS_TO_AULI.md), [ATM_IN_S_TO_AULI](ATM_IN_S_TO_AULI.md), [ATM_IN_US_TO_AULI](ATM_IN_US_TO_AULI.md)
- **[ATM_TIME_TO_TM](ATM_TIME_TO_TM.md) / [ATM_TM_TO_TIME](ATM_TM_TO_TIME.md)**: reine Adapter-↔-Variablen-Durchreichung ohne Einheitenumrechnung, im Gegensatz zur numerischen Umrechnung von `ATM_IN_MS_TO_AULI`.

## Fazit

`ATM_IN_MS_TO_AULI` liefert eine standardkonforme, vollständig adapterbasierte Umrechnung eines `TIME`-Werts in Millisekunden auf einen `ULINT`-Wert und fügt sich damit nahtlos in Netzwerke ein, die konsequent auf Adapterverbindungen statt klassischer Datenverdrahtung setzen.
