...
| OUT | Plug | adapter::types::unidirectional::ALR | Unidirektionaler `LREAL`-Adapter als Ausgangsschnittstelle, Wert von `IN` in Nanosekunden ausgedrückt. |

## Funktionsweise

Der Baustein enthält intern einen einzelnen Funktionsbaustein `F_TIME_IN_NS_TO_LREAL` aus der IEC-61131-3-Standardbibliothek. Die Ablaufsteuerung erfolgt rein ereignisgesteuert:

1. Ein Ereignis am Eingangsadapter **IN.E1** triggert den Eingang **REQ** des internen Umrechnungsbausteins.
2. Der Datenwert **IN.D1** (`TIME`) wird an den Eingang **IN** von `F_TIME_IN_NS_TO_LREAL` übergeben.
3. `F_TIME_IN_NS_TO_LREAL` rechnet den Zeitwert in eine Nanosekunden-Zahl vom Typ `LREAL` um und signalisiert dies über seinen Ausgang **CNF**.
4. Dieses Ereignis wird an den Ausgangsadapter **OUT.E1** weitergeleitet.
5. Gleichzeitig wird der umgerechnete Wert (Ausgang **OUT** von `F_TIME_IN_NS_TO_LREAL`) an den Datenpunkt **OUT.D1** übergeben.
...
- **Einheitenumrechnung nach IEC 61131-3**: Der interne Baustein `F_TIME_IN_NS_TO_LREAL` interpretiert den intern in Nanosekunden gehaltenen `TIME`-Wert als Zahl in Nanosekunden und liefert das Ergebnis als `LREAL` (64-Bit-Gleitkommazahl).
...
- **Anzeige/Weiterverarbeitung von Zeitdauern**: Eine über einen `ATM`-Adapter transportierte Zeitdauer (z. B. eine Zykluszeit oder Verzögerung) soll als Zahl in Nanosekunden an einen Adapter-Socket vom Typ `ALR` weitergegeben werden, etwa zur Anzeige auf einem Panel oder zur Weiterverarbeitung in einer Berechnung.
...
- **Gleiche Einheit, andere Zieltypen**: [ATM_IN_NS_TO_ADI](ATM_IN_NS_TO_ADI.md), [ATM_IN_NS_TO_ALI](ATM_IN_NS_TO_ALI.md), [ATM_IN_NS_TO_AUDI](ATM_IN_NS_TO_AUDI.md), [ATM_IN_NS_TO_AULI](ATM_IN_NS_TO_AULI.md)
- **Gleicher Zieltyp, andere Einheiten**: [ATM_IN_MS_TO_ALR](ATM_IN_MS_TO_ALR.md), [ATM_IN_S_TO_ALR](ATM_IN_S_TO_ALR.md), [ATM_IN_US_TO_ALR](ATM_IN_US_TO_ALR.md)
- **[ATM_TIME_TO_TM](ATM_TIME_TO_TM.md) / [ATM_TM_TO_TIME](ATM_TM_TO_TIME.md)**: reine Adapter-↔-Variablen-Durchreichung ohne Einheitenumrechnung, im Gegensatz zur numerischen Umrechnung von `ATM_IN_NS_TO_ALR`.

## Fazit

`ATM_IN_NS_TO_ALR` liefert eine standardkonforme, vollständig adapterbasierte Umrechnung eines `TIME`-Werts in Nanosekunden auf einen `LREAL`-Wert und fügt sich damit nahtlos in Netzwerke ein, die konsequent auf Adapterverbindungen statt klassischer Datenverdrahtung setzen.
