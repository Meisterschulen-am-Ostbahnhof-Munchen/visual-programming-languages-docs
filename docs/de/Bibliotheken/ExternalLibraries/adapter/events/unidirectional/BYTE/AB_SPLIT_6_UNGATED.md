...
Der Funktionsbaustein **AB_SPLIT_6_UNGATED** ist ein generischer Adapter-Splitter. Er dient dazu, einen eingehenden unidirektionalen Adapterdatenstrom (Typ `AB`) gleichzeitig auf sechs separate Ausgänge zu verteilen. Dadurch können die Daten eines einzelnen Adapters mehreren nachgeschalteten Bausteinen ohne Datenverlust zur Verfügung gestellt werden.
...
- **Generische Splits mit Ereignissen**: Manche Splitter arbeiten ereignisgesteuert (z. B. `E_SPLIT`), erfordern dann aber zusätzliche Trigger. Der `AB_SPLIT_6_UNGATED` arbeitet dagegen kontinuierlich und ereignislos über die Adapterschnittstelle.
- **Daten‑Splitter (z. B. `F_SPLIT`)**: Diese teilen einzelne Datenwerte (z. B. ein Array) auf, während der `AB_SPLIT_6_UNGATED` den gesamten Adapter-Datenstrom unverändert kopiert.

- **[`AB_SPLIT_6`](AB_SPLIT_6.md)**: Die gegatete Variante – aktualisiert den Ausgang nur bei tatsächlicher Wertänderung.
