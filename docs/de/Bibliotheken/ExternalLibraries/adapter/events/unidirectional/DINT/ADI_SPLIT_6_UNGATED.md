...
- **SPLIT für Standard-Datentypen:** Viele Frameworks bieten Splitter für einfache Datentypen (z. B. `SPLIT_INT`). Der `ADI_SPLIT_6_UNGATED` ist speziell für den Adapter-Typ `ADI` ausgelegt und arbeitet auf einer höheren Abstraktionsebene (Adapter statt einzelner Datenpunkte).
...
- **[`ADI_SPLIT_6`](ADI_SPLIT_6.md)**: Die gegatete Variante – aktualisiert den Ausgang nur bei tatsächlicher Wertänderung.
