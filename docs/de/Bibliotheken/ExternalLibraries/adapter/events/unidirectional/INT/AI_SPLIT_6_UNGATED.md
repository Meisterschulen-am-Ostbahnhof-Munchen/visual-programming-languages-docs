...
> ℹ️ **UNGATED-Variante:** Dieser Baustein ist die ungegatete Version von [`AI_SPLIT_6`](AI_SPLIT_6.md). Er unterdrückt **keine** unveränderten Wiederholungen – jedes neu berechnete Ergebnis wird bedingungslos weitergegeben, auch ohne Wertänderung. Das ist wichtig für Verbraucher, die eine periodische Kadenz unabhängig von Wertänderung brauchen (z. B. Ableitungs-/Frequenzberechnungen, die sonst nicht gegen Null abklingen). Alle Angaben zu Änderungserkennung/Change-Gating weiter unten auf dieser Seite gelten **nicht** für diesen Baustein.

## Einleitung

![AI_SPLIT_6_UNGATED](./AI_SPLIT_6_UNGATED.svg)

Der Funktionsblock **AI_SPLIT_6_UNGATED** dient als generischer Splitter, der einen analogen Eingangswert (AI) auf sechs identische Ausgänge verteilt. Er realisiert eine 1:6-Verteilung eines AI-Adapters, ohne dass Daten oder Ereignisse modifiziert werden. Der Baustein ist als generischer FB (GenericClassName: `GEN_AI_SPLIT`) definiert und arbeitet rein adapterbasiert.
...
- **AI_SPLIT_2 / AI_SPLIT_4**: Diese Bausteine teilen einen AI‑Eingang auf zwei bzw. vier Ausgänge. **AI_SPLIT_6_UNGATED** stellt die Variante mit sechs Ausgängen dar. Alle Splitter haben identische Funktionsweise und unterscheiden sich nur in der Anzahl der Ausgangsadapter.
- **AI_DUPLICATE (hypothetisch)**: Ein Baustein, der den Wert zu Kopierzwecken dupliziert, jedoch meist mit Datenports arbeitet. AI_SPLIT_6_UNGATED nutzt dagegen ausschließlich Adapter, was die direkte Kopplung ohne zusätzliche Ereignissteuerung erlaubt.

- **[`AI_SPLIT_6`](AI_SPLIT_6.md)**: Die gegatete Variante – aktualisiert den Ausgang nur bei tatsächlicher Wertänderung.
