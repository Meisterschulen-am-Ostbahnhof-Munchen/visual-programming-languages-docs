...
> ℹ️ **UNGATED-Variante:** Dieser Baustein ist die ungegatete Version von [`ALR_SPLIT_7`](ALR_SPLIT_7.md). Er unterdrückt **keine** unveränderten Wiederholungen – jedes neu berechnete Ergebnis wird bedingungslos weitergegeben, auch ohne Wertänderung. Das ist wichtig für Verbraucher, die eine periodische Kadenz unabhängig von Wertänderung brauchen (z. B. Ableitungs-/Frequenzberechnungen, die sonst nicht gegen Null abklingen). Alle Angaben zu Änderungserkennung/Change-Gating weiter unten auf dieser Seite gelten **nicht** für diesen Baustein.
...
Der Funktionsblock `ALR_SPLIT_7_UNGATED` dient dazu, ein eingehendes ALR-Signal (Agricultural Logic Resource) auf sieben identische Ausgangssignale aufzuteilen. Er ist als generischer Baustein konzipiert und ermöglicht die parallele Verteilung eines Alarm- oder Steuersignals an bis zu sieben nachfolgende Komponenten oder Subsysteme.
...
Der `ALR_SPLIT_7_UNGATED` – Baustein leitet das an seinem Socket `IN` anliegende ALR-Signal unverändert an alle sieben Plugs (`OUT1` bis `OUT7`) weiter. Es findet keine Transformation, Verzögerung oder Filterung des Signals statt. Der Baustein fungiert als reiner Verteiler (Splitter) für einen unidirektionalen ALR-Datenstrom.
...
- **[`ALR_SPLIT_7`](ALR_SPLIT_7.md)**: Die gegatete Variante – aktualisiert den Ausgang nur bei tatsächlicher Wertänderung.
