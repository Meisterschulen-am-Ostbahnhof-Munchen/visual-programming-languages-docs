...
Die 4diac-Bibliothek enthält Splitter-Bausteine für verschiedene Ausgangszahlen, z.B. `AULI_SPLIT_3` oder `AULI_SPLIT_5`. Der `AULI_SPLIT_7_UNGATED` unterscheidet sich lediglich durch die Anzahl der Ausgänge (7). Bausteine zur Aufteilung anderer Adaptertypen (z.B. `BOOL_SPLIT`, `INT_SPLIT`) haben eine ähnliche Logik, arbeiten jedoch mit unterschiedlichen Daten- bzw. Adapterformaten.

- **[`AULI_SPLIT_7`](AULI_SPLIT_7.md)**: Die gegatete Variante – aktualisiert den Ausgang nur bei tatsächlicher Wertänderung.
