...
Der **AIWS_SPLIT_5_UNGATED** besitzt keine explizite Zustandsmaschine. Seine Funktionalität ist rein kombinatorisch – sobald der Socket verbunden ist, liegen an allen Ausgängen die gleichen Adapter‑Signale an. Daher existieren keine internen Zustände, die das Verhalten beeinflussen.
...
| ---------- | ----------------- | ----- | -------------- |
| **AIWS_SPLIT_5_UNGATED** | 5 | AIWS‑Adapter | Generisch, keine Events/Daten |
...
Alle Split‑Varianten basieren auf demselben Prinzip und unterscheiden sich lediglich in der Anzahl der Ausgangsadapter. Der **AIWS_SPLIT_5_UNGATED** deckt einen mittleren Verteilungsbedarf ab und ist besonders dann vorteilhaft, wenn genau fünf parallele Ausgänge benötigt werden.

- **[`AIWS_SPLIT_5`](AIWS_SPLIT_5.md)**: Die gegatete Variante – aktualisiert den Ausgang nur bei tatsächlicher Wertänderung.
