...
Der Baustein `AUS_DIV_2_UNGATED` führt eine klassische Division nach folgendem mathematischen Prinzip aus:
...
Im Vergleich zu einem Standard-Ganzzahl-Divisionsbaustein (wie dem IEC 61131-3 `DIV` Baustein) bietet `AUS_DIV_2_UNGATED` folgende Vorteile:

- **Adapterbasierte Kopplung:** Standard-FBs benötigen separate Pins für Daten und Events (REQ/CNF). `AUS_DIV_2_UNGATED` bündelt diese logisch in den `AUS`-Adaptern.
...
- **[`AUS_DIV_2`](AUS_DIV_2.md)**: Die gegatete Variante – aktualisiert den Ausgang nur bei tatsächlicher Wertänderung.
