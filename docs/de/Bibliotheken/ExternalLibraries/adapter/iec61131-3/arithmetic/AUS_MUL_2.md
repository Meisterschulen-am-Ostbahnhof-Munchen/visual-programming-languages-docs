...
Im Vergleich zum Standard-IEC-61131-3-Multiplikationsbaustein (`MUL`), der mit einzelnen Pins für `REQ`, `CNF` sowie direkten Daten-Ein- und Ausgängen arbeitet, bündelt `AUS_MUL_2` all diese Signale in Adaptern. Dies verhindert "Kabelsalat" im Funktionsplan, erfordert jedoch, dass die angebundenen Signale bereits im `AUS`-Adapterformat vorliegen.

---

- **[`AUS_MUL_2_UNGATED`](AUS_MUL_2_UNGATED.md)**: Ungegatete Variante – aktualisiert den Ausgang bei jedem Durchlauf, auch ohne Wertänderung.
