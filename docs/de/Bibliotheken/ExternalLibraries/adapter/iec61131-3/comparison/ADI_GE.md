...
- **Normkonformität**: Der Baustein implementiert die Vergleichsfunktion aus IEC 61131-3 („greater than or equal to“) für den Datentyp `DINT`.
...
- **Steuerungslogik**: Auslösen einer Aktion, sobald zwei Werte vom Typ `DINT` die Bedingung „größer gleich“ erfüllen.
- **Datenfilterung**: Auswählen von Datensätzen oder Signalen, die eine Vergleichsbedingung auf `DINT`-Basis erfüllen.

## Vergleich mit ähnlichen Bausteinen

- **ADI_LE**: Prüft auf „kleiner gleich“ – invers zur Funktion von ADI_GE.
- **ADI_EQ**: Prüft auf Gleichheit der Werte.
- **ADI_GT**: Führt den Vergleich „größer als“ durch – die Basisvariante ohne Gleichheitsfall, deren erweiterte Version ADI_GE ist.

Der ADI_GE ergänzt die ADI-Vergleichsfamilie um den Grenzfall der Gleichheit.

## Fazit

Der **ADI_GE** ist ein einfacher, aber wichtiger Vergleichsbaustein für IEC 61131-3 Steuerungsanwendungen mit dem Datentyp `DINT`. Seine adapterorientierte Schnittstelle macht ihn vielseitig einsetzbar und ermöglicht eine klare Trennung von Ereignis- und Datenpfaden. Durch die ereignisgesteuerte Arbeitsweise eignet er sich besonders für Echtzeitsysteme, in denen Bedingungen sofort auf Werteänderungen reagiert werden müssen.
