...
- **Normkonformität**: Der Baustein implementiert die Vergleichsfunktion aus IEC 61131-3 („less than or equal to“) für den Datentyp `DINT`.
...
- **Steuerungslogik**: Auslösen einer Aktion, sobald zwei Werte vom Typ `DINT` die Bedingung „kleiner gleich“ erfüllen.
- **Datenfilterung**: Auswählen von Datensätzen oder Signalen, die eine Vergleichsbedingung auf `DINT`-Basis erfüllen.

## Vergleich mit ähnlichen Bausteinen

- **ADI_GE**: Prüft auf „größer gleich“ – invers zur Funktion von ADI_LE.
- **ADI_EQ**: Prüft auf Gleichheit der Werte.
- **ADI_LT**: Führt den Vergleich „kleiner als“ durch – die Basisvariante ohne Gleichheitsfall, deren erweiterte Version ADI_LE ist.

Der ADI_LE ergänzt die ADI-Vergleichsfamilie um den Grenzfall der Gleichheit.

## Fazit

Der **ADI_LE** ist ein einfacher, aber wichtiger Vergleichsbaustein für IEC 61131-3 Steuerungsanwendungen mit dem Datentyp `DINT`. Seine adapterorientierte Schnittstelle macht ihn vielseitig einsetzbar und ermöglicht eine klare Trennung von Ereignis- und Datenpfaden. Durch die ereignisgesteuerte Arbeitsweise eignet er sich besonders für Echtzeitsysteme, in denen Bedingungen sofort auf Werteänderungen reagiert werden müssen.
