...
- **Normkonformität**: Der Baustein implementiert die Vergleichsfunktion aus IEC 61131-3 („less than“) für den Datentyp `DINT`.
...
- **Steuerungslogik**: Auslösen einer Aktion, sobald zwei Werte vom Typ `DINT` die Bedingung „kleiner als“ erfüllen.
- **Datenfilterung**: Auswählen von Datensätzen oder Signalen, die eine Vergleichsbedingung auf `DINT`-Basis erfüllen.

## Vergleich mit ähnlichen Bausteinen

- **ADI_GT**: Prüft auf „größer als“ – invers zur Funktion von ADI_LT.
- **ADI_EQ**: Prüft auf Gleichheit der Werte.
- **ADI_LE**: Führt den Vergleich „kleiner gleich“ durch – die um den Gleichheitsfall erweiterte Version von ADI_LT.

Der ADI_LT ist die grundlegende „kleiner als“-Komponente ohne Gleichheitsfall, ergänzt durch ADI_LE.

## Fazit

Der **ADI_LT** ist ein einfacher, aber wichtiger Vergleichsbaustein für IEC 61131-3 Steuerungsanwendungen mit dem Datentyp `DINT`. Seine adapterorientierte Schnittstelle macht ihn vielseitig einsetzbar und ermöglicht eine klare Trennung von Ereignis- und Datenpfaden. Durch die ereignisgesteuerte Arbeitsweise eignet er sich besonders für Echtzeitsysteme, in denen Bedingungen sofort auf Werteänderungen reagiert werden müssen.
