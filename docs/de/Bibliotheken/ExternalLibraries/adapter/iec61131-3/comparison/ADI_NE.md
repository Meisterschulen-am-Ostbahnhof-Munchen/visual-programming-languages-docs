...
- **Normkonformität**: Der Baustein implementiert die Vergleichsfunktion aus IEC 61131-3 („not equal to“) für den Datentyp `DINT`.
...
- **Steuerungslogik**: Auslösen einer Aktion, sobald zwei Werte vom Typ `DINT` die Bedingung „ungleich“ erfüllen.
- **Datenfilterung**: Auswählen von Datensätzen oder Signalen, die eine Vergleichsbedingung auf `DINT`-Basis erfüllen.

## Vergleich mit ähnlichen Bausteinen

- **ADI_EQ**: Prüft auf „gleich“ – invers zur Funktion von ADI_NE.
- **ADI_EQ**: Prüft auf Gleichheit der Werte.

Der ADI_NE ist Teil der vollständigen ADI-Vergleichsfamilie (EQ/NE/GT/GE/LT/LE) für den Datentyp `DINT`.

## Fazit

Der **ADI_NE** ist ein einfacher, aber wichtiger Vergleichsbaustein für IEC 61131-3 Steuerungsanwendungen mit dem Datentyp `DINT`. Seine adapterorientierte Schnittstelle macht ihn vielseitig einsetzbar und ermöglicht eine klare Trennung von Ereignis- und Datenpfaden. Durch die ereignisgesteuerte Arbeitsweise eignet er sich besonders für Echtzeitsysteme, in denen Bedingungen sofort auf Werteänderungen reagiert werden müssen.
