...
- **Normkonformität**: Der Baustein implementiert die Vergleichsfunktion aus IEC 61131-3 („equal to“) für den Datentyp `STRING`.
...
- **Steuerungslogik**: Auslösen einer Aktion, sobald zwei Werte vom Typ `STRING` die Bedingung „gleich“ erfüllen.
- **Datenfilterung**: Auswählen von Datensätzen oder Signalen, die eine Vergleichsbedingung auf `STRING`-Basis erfüllen.

## Vergleich mit ähnlichen Bausteinen

- **AIS_NE**: Prüft auf „ungleich“ – invers zur Funktion von AIS_EQ.
- **AIS_NE**: Prüft auf Ungleichheit der Werte.

Der AIS_EQ ist Teil der vollständigen AIS-Vergleichsfamilie (EQ/NE/GT/GE/LT/LE) für den Datentyp `STRING`.

## Fazit

Der **AIS_EQ** ist ein einfacher, aber wichtiger Vergleichsbaustein für IEC 61131-3 Steuerungsanwendungen mit dem Datentyp `STRING`. Seine adapterorientierte Schnittstelle macht ihn vielseitig einsetzbar und ermöglicht eine klare Trennung von Ereignis- und Datenpfaden. Durch die ereignisgesteuerte Arbeitsweise eignet er sich besonders für Echtzeitsysteme, in denen Bedingungen sofort auf Werteänderungen reagiert werden müssen.
