...
- **Normkonformität**: Der Baustein implementiert die Vergleichsfunktion aus IEC 61131-3 („not equal to“) für den Datentyp `INT`.
...
- **Steuerungslogik**: Auslösen einer Aktion, sobald zwei Werte vom Typ `INT` die Bedingung „ungleich“ erfüllen.
- **Datenfilterung**: Auswählen von Datensätzen oder Signalen, die eine Vergleichsbedingung auf `INT`-Basis erfüllen.

## Vergleich mit ähnlichen Bausteinen

- **AI_EQ**: Prüft auf „gleich“ – invers zur Funktion von AI_NE.
- **AI_EQ**: Prüft auf Gleichheit der Werte.

Der AI_NE ist Teil der vollständigen AI-Vergleichsfamilie (EQ/NE/GT/GE/LT/LE) für den Datentyp `INT`.

## Fazit

Der **AI_NE** ist ein einfacher, aber wichtiger Vergleichsbaustein für IEC 61131-3 Steuerungsanwendungen mit dem Datentyp `INT`. Seine adapterorientierte Schnittstelle macht ihn vielseitig einsetzbar und ermöglicht eine klare Trennung von Ereignis- und Datenpfaden. Durch die ereignisgesteuerte Arbeitsweise eignet er sich besonders für Echtzeitsysteme, in denen Bedingungen sofort auf Werteänderungen reagiert werden müssen.
