...
- **Normkonformität**: Der Baustein implementiert die Vergleichsfunktion aus IEC 61131-3 („equal to“) für den Datentyp `LINT`.
...
- **Steuerungslogik**: Auslösen einer Aktion, sobald zwei Werte vom Typ `LINT` die Bedingung „gleich“ erfüllen.
- **Datenfilterung**: Auswählen von Datensätzen oder Signalen, die eine Vergleichsbedingung auf `LINT`-Basis erfüllen.

## Vergleich mit ähnlichen Bausteinen

- **ALI_NE**: Prüft auf „ungleich“ – invers zur Funktion von ALI_EQ.
- **ALI_NE**: Prüft auf Ungleichheit der Werte.

Der ALI_EQ ist Teil der vollständigen ALI-Vergleichsfamilie (EQ/NE/GT/GE/LT/LE) für den Datentyp `LINT`.

## Fazit

Der **ALI_EQ** ist ein einfacher, aber wichtiger Vergleichsbaustein für IEC 61131-3 Steuerungsanwendungen mit dem Datentyp `LINT`. Seine adapterorientierte Schnittstelle macht ihn vielseitig einsetzbar und ermöglicht eine klare Trennung von Ereignis- und Datenpfaden. Durch die ereignisgesteuerte Arbeitsweise eignet er sich besonders für Echtzeitsysteme, in denen Bedingungen sofort auf Werteänderungen reagiert werden müssen.
