...
- **Normkonformität**: Der Baustein implementiert die Vergleichsfunktion aus IEC 61131-3 („greater than“) für den Datentyp `LINT`.
...
- **Steuerungslogik**: Auslösen einer Aktion, sobald zwei Werte vom Typ `LINT` die Bedingung „größer als“ erfüllen.
- **Datenfilterung**: Auswählen von Datensätzen oder Signalen, die eine Vergleichsbedingung auf `LINT`-Basis erfüllen.

## Vergleich mit ähnlichen Bausteinen

- **ALI_LT**: Prüft auf „kleiner als“ – invers zur Funktion von ALI_GT.
- **ALI_EQ**: Prüft auf Gleichheit der Werte.
- **ALI_GE**: Führt den Vergleich „größer gleich“ durch – die um den Gleichheitsfall erweiterte Version von ALI_GT.

Der ALI_GT ist die grundlegende „größer als“-Komponente ohne Gleichheitsfall, ergänzt durch ALI_GE.

## Fazit

Der **ALI_GT** ist ein einfacher, aber wichtiger Vergleichsbaustein für IEC 61131-3 Steuerungsanwendungen mit dem Datentyp `LINT`. Seine adapterorientierte Schnittstelle macht ihn vielseitig einsetzbar und ermöglicht eine klare Trennung von Ereignis- und Datenpfaden. Durch die ereignisgesteuerte Arbeitsweise eignet er sich besonders für Echtzeitsysteme, in denen Bedingungen sofort auf Werteänderungen reagiert werden müssen.
