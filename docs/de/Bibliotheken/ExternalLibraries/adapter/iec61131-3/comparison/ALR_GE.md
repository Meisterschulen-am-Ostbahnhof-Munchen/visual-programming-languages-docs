...
- **Normkonformität**: Der Baustein implementiert die Vergleichsfunktion aus IEC 61131-3 („greater than or equal to“) für den Datentyp `LREAL`.
...
- **Steuerungslogik**: Auslösen einer Aktion, sobald zwei Werte vom Typ `LREAL` die Bedingung „größer gleich“ erfüllen.
- **Datenfilterung**: Auswählen von Datensätzen oder Signalen, die eine Vergleichsbedingung auf `LREAL`-Basis erfüllen.

## Vergleich mit ähnlichen Bausteinen

- **ALR_LE**: Prüft auf „kleiner gleich“ – invers zur Funktion von ALR_GE.
- **ALR_EQ**: Prüft auf Gleichheit der Werte.
- **ALR_GT**: Führt den Vergleich „größer als“ durch – die Basisvariante ohne Gleichheitsfall, deren erweiterte Version ALR_GE ist.

Der ALR_GE ergänzt die ALR-Vergleichsfamilie um den Grenzfall der Gleichheit.

## Fazit

Der **ALR_GE** ist ein einfacher, aber wichtiger Vergleichsbaustein für IEC 61131-3 Steuerungsanwendungen mit dem Datentyp `LREAL`. Seine adapterorientierte Schnittstelle macht ihn vielseitig einsetzbar und ermöglicht eine klare Trennung von Ereignis- und Datenpfaden. Durch die ereignisgesteuerte Arbeitsweise eignet er sich besonders für Echtzeitsysteme, in denen Bedingungen sofort auf Werteänderungen reagiert werden müssen.
