...
- **Normkonformität**: Der Baustein implementiert die Vergleichsfunktion aus IEC 61131-3 („less than or equal to“) für den Datentyp `LREAL`.
...
- **Steuerungslogik**: Auslösen einer Aktion, sobald zwei Werte vom Typ `LREAL` die Bedingung „kleiner gleich“ erfüllen.
- **Datenfilterung**: Auswählen von Datensätzen oder Signalen, die eine Vergleichsbedingung auf `LREAL`-Basis erfüllen.

## Vergleich mit ähnlichen Bausteinen

- **ALR_GE**: Prüft auf „größer gleich“ – invers zur Funktion von ALR_LE.
- **ALR_EQ**: Prüft auf Gleichheit der Werte.
- **ALR_LT**: Führt den Vergleich „kleiner als“ durch – die Basisvariante ohne Gleichheitsfall, deren erweiterte Version ALR_LE ist.

Der ALR_LE ergänzt die ALR-Vergleichsfamilie um den Grenzfall der Gleichheit.

## Fazit

Der **ALR_LE** ist ein einfacher, aber wichtiger Vergleichsbaustein für IEC 61131-3 Steuerungsanwendungen mit dem Datentyp `LREAL`. Seine adapterorientierte Schnittstelle macht ihn vielseitig einsetzbar und ermöglicht eine klare Trennung von Ereignis- und Datenpfaden. Durch die ereignisgesteuerte Arbeitsweise eignet er sich besonders für Echtzeitsysteme, in denen Bedingungen sofort auf Werteänderungen reagiert werden müssen.
