...
- **Normkonformität**: Der Baustein implementiert die Vergleichsfunktion aus IEC 61131-3 („less than“) für den Datentyp `LREAL`.
...
- **Steuerungslogik**: Auslösen einer Aktion, sobald zwei Werte vom Typ `LREAL` die Bedingung „kleiner als“ erfüllen.
- **Datenfilterung**: Auswählen von Datensätzen oder Signalen, die eine Vergleichsbedingung auf `LREAL`-Basis erfüllen.

## Vergleich mit ähnlichen Bausteinen

- **ALR_GT**: Prüft auf „größer als“ – invers zur Funktion von ALR_LT.
- **ALR_EQ**: Prüft auf Gleichheit der Werte.
- **ALR_LE**: Führt den Vergleich „kleiner gleich“ durch – die um den Gleichheitsfall erweiterte Version von ALR_LT.

Der ALR_LT ist die grundlegende „kleiner als“-Komponente ohne Gleichheitsfall, ergänzt durch ALR_LE.

## Fazit

Der **ALR_LT** ist ein einfacher, aber wichtiger Vergleichsbaustein für IEC 61131-3 Steuerungsanwendungen mit dem Datentyp `LREAL`. Seine adapterorientierte Schnittstelle macht ihn vielseitig einsetzbar und ermöglicht eine klare Trennung von Ereignis- und Datenpfaden. Durch die ereignisgesteuerte Arbeitsweise eignet er sich besonders für Echtzeitsysteme, in denen Bedingungen sofort auf Werteänderungen reagiert werden müssen.
