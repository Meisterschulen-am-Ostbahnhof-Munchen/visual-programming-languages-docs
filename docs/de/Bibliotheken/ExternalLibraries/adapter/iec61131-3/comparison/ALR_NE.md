...
- **Normkonformität**: Der Baustein implementiert die Vergleichsfunktion aus IEC 61131-3 („not equal to“) für den Datentyp `LREAL`.
...
- **Steuerungslogik**: Auslösen einer Aktion, sobald zwei Werte vom Typ `LREAL` die Bedingung „ungleich“ erfüllen.
- **Datenfilterung**: Auswählen von Datensätzen oder Signalen, die eine Vergleichsbedingung auf `LREAL`-Basis erfüllen.

## Vergleich mit ähnlichen Bausteinen

- **ALR_EQ**: Prüft auf „gleich“ – invers zur Funktion von ALR_NE.
- **ALR_EQ**: Prüft auf Gleichheit der Werte.

Der ALR_NE ist Teil der vollständigen ALR-Vergleichsfamilie (EQ/NE/GT/GE/LT/LE) für den Datentyp `LREAL`.

## Fazit

Der **ALR_NE** ist ein einfacher, aber wichtiger Vergleichsbaustein für IEC 61131-3 Steuerungsanwendungen mit dem Datentyp `LREAL`. Seine adapterorientierte Schnittstelle macht ihn vielseitig einsetzbar und ermöglicht eine klare Trennung von Ereignis- und Datenpfaden. Durch die ereignisgesteuerte Arbeitsweise eignet er sich besonders für Echtzeitsysteme, in denen Bedingungen sofort auf Werteänderungen reagiert werden müssen.
