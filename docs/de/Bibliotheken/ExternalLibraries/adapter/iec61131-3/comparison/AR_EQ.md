...
- **Normkonformität**: Der Baustein implementiert die Vergleichsfunktion aus IEC 61131-3 („equal to“) für den Datentyp `REAL`.
...
- **Steuerungslogik**: Auslösen einer Aktion, sobald zwei Werte vom Typ `REAL` die Bedingung „gleich“ erfüllen.
- **Datenfilterung**: Auswählen von Datensätzen oder Signalen, die eine Vergleichsbedingung auf `REAL`-Basis erfüllen.

## Vergleich mit ähnlichen Bausteinen

- **AR_NE**: Prüft auf „ungleich“ – invers zur Funktion von AR_EQ.
- **AR_NE**: Prüft auf Ungleichheit der Werte.

Der AR_EQ ist Teil der vollständigen AR-Vergleichsfamilie (EQ/NE/GT/GE/LT/LE) für den Datentyp `REAL`.

## Fazit

Der **AR_EQ** ist ein einfacher, aber wichtiger Vergleichsbaustein für IEC 61131-3 Steuerungsanwendungen mit dem Datentyp `REAL`. Seine adapterorientierte Schnittstelle macht ihn vielseitig einsetzbar und ermöglicht eine klare Trennung von Ereignis- und Datenpfaden. Durch die ereignisgesteuerte Arbeitsweise eignet er sich besonders für Echtzeitsysteme, in denen Bedingungen sofort auf Werteänderungen reagiert werden müssen.
