...
- **Normkonformität**: Der Baustein implementiert die Vergleichsfunktion aus IEC 61131-3 („equal to“) für den Datentyp `SINT`.
...
- **Steuerungslogik**: Auslösen einer Aktion, sobald zwei Werte vom Typ `SINT` die Bedingung „gleich“ erfüllen.
- **Datenfilterung**: Auswählen von Datensätzen oder Signalen, die eine Vergleichsbedingung auf `SINT`-Basis erfüllen.

## Vergleich mit ähnlichen Bausteinen

- **AS_NE**: Prüft auf „ungleich“ – invers zur Funktion von AS_EQ.
- **AS_NE**: Prüft auf Ungleichheit der Werte.

Der AS_EQ ist Teil der vollständigen AS-Vergleichsfamilie (EQ/NE/GT/GE/LT/LE) für den Datentyp `SINT`.

## Fazit

Der **AS_EQ** ist ein einfacher, aber wichtiger Vergleichsbaustein für IEC 61131-3 Steuerungsanwendungen mit dem Datentyp `SINT`. Seine adapterorientierte Schnittstelle macht ihn vielseitig einsetzbar und ermöglicht eine klare Trennung von Ereignis- und Datenpfaden. Durch die ereignisgesteuerte Arbeitsweise eignet er sich besonders für Echtzeitsysteme, in denen Bedingungen sofort auf Werteänderungen reagiert werden müssen.
