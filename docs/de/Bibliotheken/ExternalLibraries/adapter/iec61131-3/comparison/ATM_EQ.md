...
- **Normkonformität**: Der Baustein implementiert die Vergleichsfunktion aus IEC 61131-3 („equal to“) für den Datentyp `TIME`.
...
- **Steuerungslogik**: Auslösen einer Aktion, sobald zwei Werte vom Typ `TIME` die Bedingung „gleich“ erfüllen.
- **Datenfilterung**: Auswählen von Datensätzen oder Signalen, die eine Vergleichsbedingung auf `TIME`-Basis erfüllen.

## Vergleich mit ähnlichen Bausteinen

- **ATM_NE**: Prüft auf „ungleich“ – invers zur Funktion von ATM_EQ.
- **ATM_NE**: Prüft auf Ungleichheit der Werte.

Der ATM_EQ ist Teil der vollständigen ATM-Vergleichsfamilie (EQ/NE/GT/GE/LT/LE) für den Datentyp `TIME`.

## Fazit

Der **ATM_EQ** ist ein einfacher, aber wichtiger Vergleichsbaustein für IEC 61131-3 Steuerungsanwendungen mit dem Datentyp `TIME`. Seine adapterorientierte Schnittstelle macht ihn vielseitig einsetzbar und ermöglicht eine klare Trennung von Ereignis- und Datenpfaden. Durch die ereignisgesteuerte Arbeitsweise eignet er sich besonders für Echtzeitsysteme, in denen Bedingungen sofort auf Werteänderungen reagiert werden müssen.
