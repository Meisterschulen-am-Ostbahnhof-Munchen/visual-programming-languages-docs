...
- **Normkonformität**: Der Baustein implementiert die Vergleichsfunktion aus IEC 61131-3 („equal to“) für den Datentyp `ULINT`.
...
- **Steuerungslogik**: Auslösen einer Aktion, sobald zwei Werte vom Typ `ULINT` die Bedingung „gleich“ erfüllen.
- **Datenfilterung**: Auswählen von Datensätzen oder Signalen, die eine Vergleichsbedingung auf `ULINT`-Basis erfüllen.

## Vergleich mit ähnlichen Bausteinen

- **AULI_NE**: Prüft auf „ungleich“ – invers zur Funktion von AULI_EQ.
- **AULI_NE**: Prüft auf Ungleichheit der Werte.

Der AULI_EQ ist Teil der vollständigen AULI-Vergleichsfamilie (EQ/NE/GT/GE/LT/LE) für den Datentyp `ULINT`.

## Fazit

Der **AULI_EQ** ist ein einfacher, aber wichtiger Vergleichsbaustein für IEC 61131-3 Steuerungsanwendungen mit dem Datentyp `ULINT`. Seine adapterorientierte Schnittstelle macht ihn vielseitig einsetzbar und ermöglicht eine klare Trennung von Ereignis- und Datenpfaden. Durch die ereignisgesteuerte Arbeitsweise eignet er sich besonders für Echtzeitsysteme, in denen Bedingungen sofort auf Werteänderungen reagiert werden müssen.
