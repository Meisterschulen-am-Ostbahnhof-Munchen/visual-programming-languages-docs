...
- **Normkonformität**: Der Baustein implementiert die Vergleichsfunktion aus IEC 61131-3 („equal to“) für den Datentyp `USINT`.
...
- **Steuerungslogik**: Auslösen einer Aktion, sobald zwei Werte vom Typ `USINT` die Bedingung „gleich“ erfüllen.
- **Datenfilterung**: Auswählen von Datensätzen oder Signalen, die eine Vergleichsbedingung auf `USINT`-Basis erfüllen.

## Vergleich mit ähnlichen Bausteinen

- **AUS_NE**: Prüft auf „ungleich“ – invers zur Funktion von AUS_EQ.
- **AUS_NE**: Prüft auf Ungleichheit der Werte.

Der AUS_EQ ist Teil der vollständigen AUS-Vergleichsfamilie (EQ/NE/GT/GE/LT/LE) für den Datentyp `USINT`.

## Fazit

Der **AUS_EQ** ist ein einfacher, aber wichtiger Vergleichsbaustein für IEC 61131-3 Steuerungsanwendungen mit dem Datentyp `USINT`. Seine adapterorientierte Schnittstelle macht ihn vielseitig einsetzbar und ermöglicht eine klare Trennung von Ereignis- und Datenpfaden. Durch die ereignisgesteuerte Arbeitsweise eignet er sich besonders für Echtzeitsysteme, in denen Bedingungen sofort auf Werteänderungen reagiert werden müssen.
