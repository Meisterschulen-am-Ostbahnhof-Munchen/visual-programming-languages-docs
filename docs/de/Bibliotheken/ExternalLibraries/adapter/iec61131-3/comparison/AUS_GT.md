...
- **Normkonformität**: Der Baustein implementiert die Vergleichsfunktion aus IEC 61131-3 („greater than“) für den Datentyp `USINT`.
...
- **Steuerungslogik**: Auslösen einer Aktion, sobald zwei Werte vom Typ `USINT` die Bedingung „größer als“ erfüllen.
- **Datenfilterung**: Auswählen von Datensätzen oder Signalen, die eine Vergleichsbedingung auf `USINT`-Basis erfüllen.

## Vergleich mit ähnlichen Bausteinen

- **AUS_LT**: Prüft auf „kleiner als“ – invers zur Funktion von AUS_GT.
- **AUS_EQ**: Prüft auf Gleichheit der Werte.
- **AUS_GE**: Führt den Vergleich „größer gleich“ durch – die um den Gleichheitsfall erweiterte Version von AUS_GT.

Der AUS_GT ist die grundlegende „größer als“-Komponente ohne Gleichheitsfall, ergänzt durch AUS_GE.

## Fazit

Der **AUS_GT** ist ein einfacher, aber wichtiger Vergleichsbaustein für IEC 61131-3 Steuerungsanwendungen mit dem Datentyp `USINT`. Seine adapterorientierte Schnittstelle macht ihn vielseitig einsetzbar und ermöglicht eine klare Trennung von Ereignis- und Datenpfaden. Durch die ereignisgesteuerte Arbeitsweise eignet er sich besonders für Echtzeitsysteme, in denen Bedingungen sofort auf Werteänderungen reagiert werden müssen.
