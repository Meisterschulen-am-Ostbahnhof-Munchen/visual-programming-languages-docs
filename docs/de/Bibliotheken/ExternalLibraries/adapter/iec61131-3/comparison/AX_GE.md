...
- **Normkonformität**: Der Baustein implementiert die Vergleichsfunktion aus IEC 61131-3 („greater than or equal to“) für den Datentyp `BOOL`.
...
- **Steuerungslogik**: Auslösen einer Aktion, sobald zwei Werte vom Typ `BOOL` die Bedingung „größer gleich“ erfüllen.
- **Datenfilterung**: Auswählen von Datensätzen oder Signalen, die eine Vergleichsbedingung auf `BOOL`-Basis erfüllen.

## Vergleich mit ähnlichen Bausteinen

- **AX_LE**: Prüft auf „kleiner gleich“ – invers zur Funktion von AX_GE.
- **AX_EQ**: Prüft auf Gleichheit der Werte.
- **AX_GT**: Führt den Vergleich „größer als“ durch – die Basisvariante ohne Gleichheitsfall, deren erweiterte Version AX_GE ist.

Der AX_GE ergänzt die AX-Vergleichsfamilie um den Grenzfall der Gleichheit.

## Fazit

Der **AX_GE** ist ein einfacher, aber wichtiger Vergleichsbaustein für IEC 61131-3 Steuerungsanwendungen mit dem Datentyp `BOOL`. Seine adapterorientierte Schnittstelle macht ihn vielseitig einsetzbar und ermöglicht eine klare Trennung von Ereignis- und Datenpfaden. Durch die ereignisgesteuerte Arbeitsweise eignet er sich besonders für Echtzeitsysteme, in denen Bedingungen sofort auf Werteänderungen reagiert werden müssen.
