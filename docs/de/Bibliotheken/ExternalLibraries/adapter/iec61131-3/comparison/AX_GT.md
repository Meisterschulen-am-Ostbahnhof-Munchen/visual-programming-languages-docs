...
- **Normkonformität**: Der Baustein implementiert die Vergleichsfunktion aus IEC 61131-3 („greater than“) für den Datentyp `BOOL`.
...
- **Steuerungslogik**: Auslösen einer Aktion, sobald zwei Werte vom Typ `BOOL` die Bedingung „größer als“ erfüllen.
- **Datenfilterung**: Auswählen von Datensätzen oder Signalen, die eine Vergleichsbedingung auf `BOOL`-Basis erfüllen.

## Vergleich mit ähnlichen Bausteinen

- **AX_LT**: Prüft auf „kleiner als“ – invers zur Funktion von AX_GT.
- **AX_EQ**: Prüft auf Gleichheit der Werte.
- **AX_GE**: Führt den Vergleich „größer gleich“ durch – die um den Gleichheitsfall erweiterte Version von AX_GT.

Der AX_GT ist die grundlegende „größer als“-Komponente ohne Gleichheitsfall, ergänzt durch AX_GE.

## Fazit

Der **AX_GT** ist ein einfacher, aber wichtiger Vergleichsbaustein für IEC 61131-3 Steuerungsanwendungen mit dem Datentyp `BOOL`. Seine adapterorientierte Schnittstelle macht ihn vielseitig einsetzbar und ermöglicht eine klare Trennung von Ereignis- und Datenpfaden. Durch die ereignisgesteuerte Arbeitsweise eignet er sich besonders für Echtzeitsysteme, in denen Bedingungen sofort auf Werteänderungen reagiert werden müssen.
