...
- **Normkonformität**: Der Baustein implementiert die Vergleichsfunktion aus IEC 61131-3 („less than or equal to“) für den Datentyp `BOOL`.
...
- **Steuerungslogik**: Auslösen einer Aktion, sobald zwei Werte vom Typ `BOOL` die Bedingung „kleiner gleich“ erfüllen.
- **Datenfilterung**: Auswählen von Datensätzen oder Signalen, die eine Vergleichsbedingung auf `BOOL`-Basis erfüllen.

## Vergleich mit ähnlichen Bausteinen

- **AX_GE**: Prüft auf „größer gleich“ – invers zur Funktion von AX_LE.
- **AX_EQ**: Prüft auf Gleichheit der Werte.
- **AX_LT**: Führt den Vergleich „kleiner als“ durch – die Basisvariante ohne Gleichheitsfall, deren erweiterte Version AX_LE ist.

Der AX_LE ergänzt die AX-Vergleichsfamilie um den Grenzfall der Gleichheit.

## Fazit

Der **AX_LE** ist ein einfacher, aber wichtiger Vergleichsbaustein für IEC 61131-3 Steuerungsanwendungen mit dem Datentyp `BOOL`. Seine adapterorientierte Schnittstelle macht ihn vielseitig einsetzbar und ermöglicht eine klare Trennung von Ereignis- und Datenpfaden. Durch die ereignisgesteuerte Arbeitsweise eignet er sich besonders für Echtzeitsysteme, in denen Bedingungen sofort auf Werteänderungen reagiert werden müssen.
