...
- **Normkonformität**: Der Baustein implementiert die Vergleichsfunktion aus IEC 61131-3 („less than“) für den Datentyp `BOOL`.
...
- **Steuerungslogik**: Auslösen einer Aktion, sobald zwei Werte vom Typ `BOOL` die Bedingung „kleiner als“ erfüllen.
- **Datenfilterung**: Auswählen von Datensätzen oder Signalen, die eine Vergleichsbedingung auf `BOOL`-Basis erfüllen.

## Vergleich mit ähnlichen Bausteinen

- **AX_GT**: Prüft auf „größer als“ – invers zur Funktion von AX_LT.
- **AX_EQ**: Prüft auf Gleichheit der Werte.
- **AX_LE**: Führt den Vergleich „kleiner gleich“ durch – die um den Gleichheitsfall erweiterte Version von AX_LT.

Der AX_LT ist die grundlegende „kleiner als“-Komponente ohne Gleichheitsfall, ergänzt durch AX_LE.

## Fazit

Der **AX_LT** ist ein einfacher, aber wichtiger Vergleichsbaustein für IEC 61131-3 Steuerungsanwendungen mit dem Datentyp `BOOL`. Seine adapterorientierte Schnittstelle macht ihn vielseitig einsetzbar und ermöglicht eine klare Trennung von Ereignis- und Datenpfaden. Durch die ereignisgesteuerte Arbeitsweise eignet er sich besonders für Echtzeitsysteme, in denen Bedingungen sofort auf Werteänderungen reagiert werden müssen.
