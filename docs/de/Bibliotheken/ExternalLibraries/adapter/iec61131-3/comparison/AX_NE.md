...
- **Normkonformität**: Der Baustein implementiert die Vergleichsfunktion aus IEC 61131-3 („not equal to“) für den Datentyp `BOOL`.
...
- **Steuerungslogik**: Auslösen einer Aktion, sobald zwei Werte vom Typ `BOOL` die Bedingung „ungleich“ erfüllen.
- **Datenfilterung**: Auswählen von Datensätzen oder Signalen, die eine Vergleichsbedingung auf `BOOL`-Basis erfüllen.

## Vergleich mit ähnlichen Bausteinen

- **AX_EQ**: Prüft auf „gleich“ – invers zur Funktion von AX_NE.
- **AX_EQ**: Prüft auf Gleichheit der Werte.

Der AX_NE ist Teil der vollständigen AX-Vergleichsfamilie (EQ/NE/GT/GE/LT/LE) für den Datentyp `BOOL`.

## Fazit

Der **AX_NE** ist ein einfacher, aber wichtiger Vergleichsbaustein für IEC 61131-3 Steuerungsanwendungen mit dem Datentyp `BOOL`. Seine adapterorientierte Schnittstelle macht ihn vielseitig einsetzbar und ermöglicht eine klare Trennung von Ereignis- und Datenpfaden. Durch die ereignisgesteuerte Arbeitsweise eignet er sich besonders für Echtzeitsysteme, in denen Bedingungen sofort auf Werteänderungen reagiert werden müssen.
