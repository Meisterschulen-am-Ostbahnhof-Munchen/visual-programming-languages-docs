...
Der Funktionsblock **AB_PUBLISH_1** ist ein Composite-Funktionsblock, der den netzwerkbasierten `PUBLISH_1`-Funktionsblock aus der IEC 61499-Standardbibliothek kapselt und dessen Schnittstelle auf einen unidirektionalen **AB-Adapter** abbildet. Ein am Adapter-Socket `IN` anliegender **BYTE**-Wert wird über ein internes D-Flipflop (E_D_FF_ANY) gepuffert und anschließend über den konfigurierten Publish-Kanal an alle zugehörigen `SUBSCRIBE_1`-Empfänger gesendet.
...
|---------|------------------------------|----------|---------------------------------|
| IN      | adapter::types::unidirectional::AB | Socket (Eingang) | Zu veröffentlichender BYTE-Wert |

## Funktionsweise

1. Über das `INIT`-Ereignis wird der interne `PUBLISH_1`-Block mit `QI` und `ID` initialisiert. Bei Erfolg quittiert er mit `INITO`.
2. Sobald der AB-Socket `IN` ein Ereignis auf `IN.E1` liefert, wird der gleichzeitig an `IN.D1` anliegende BYTE-Wert in das interne **E_D_FF_ANY**-Flipflop übernommen (`CLK` bzw. `D`).
...
- **Kapselung**: Die ursprüngliche Ereignis-/Daten-Schnittstelle von `PUBLISH_1` (inkl. `REQ`/`SD_1`) wird nach innen verlegt; nach außen ist nur noch die AB-Adapter-Schnittstelle sichtbar.
...
3. **Sendeaktiv**: Ein am AB-Socket eintreffendes Ereignis puffert den Wert und veranlasst das Senden über den internen `PUBLISH_1`-Block.

## Anwendungsszenarien

- **Netzwerkkopplung in AB-Anwendungen**: Senden von BYTE-Signalen (z. B. Schaltbefehle oder Statusbits) aus der Adapter-basierten Logik über Publish/Subscribe an entfernte Empfänger.
...
- **PUBLISH_1**: Der reine Standardbaustein exponiert seine Schnittstelle als Events/Variablen (`REQ`, `SD_1`, ...). **AB_PUBLISH_1** kapselt diesen Baustein, puffert den Wert mit einem D-Flipflop (`E_D_FF_ANY`) und stellt nur noch den AB-Adapter bereit – dadurch ist die Schnittstelle erheblich kompakter und in Adapter-Netzwerke integrierbar.
- **AX_D_FF**: Puffert einen BYTE-Wert auf Adapter-Ebene, kennt aber keine Netzwerkfunktionalität. **AB_PUBLISH_1** vereint Puffern und Veröffentlichen in einem Baustein.

## Fazit

**AB_PUBLISH_1** verbindet die Publish/Subscribe-Kommunikation des Standardbausteins `PUBLISH_1` mit der Adapter-basierten BYTE-Verarbeitung. Die interne D-Flipflop-Pufferung stabilisiert den Sendewert und entkoppelt Rückkopplungsschleifen; die reduzierte AB-Schnittstelle macht den Baustein ideal für durchgängig adapterbasierte IEC 61499-Anwendungen.
