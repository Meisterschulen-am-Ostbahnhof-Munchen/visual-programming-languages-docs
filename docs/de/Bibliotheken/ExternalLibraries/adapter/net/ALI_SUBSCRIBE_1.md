...
## Einleitung
Der Funktionsblock **ALI_SUBSCRIBE_1** ist ein Composite-Funktionsblock, der den netzwerkbasierten `SUBSCRIBE_1`-Funktionsblock aus der IEC 61499-Standardbibliothek kapselt und dessen Schnittstelle auf einen unidirektionalen **ALI-Adapter** abbildet. Er abonniert einen über die `ID` konfigurierten Publish-Kanal und stellt den empfangenen **LINT**-Wert gepuffert über einen D-Flipflop (E_D_FF_ANY) am Adapter-Plug `OUT` bereit. Damit lassen sich Netzkommunikation und LINT-Signalverarbeitung in der ALI-Adapter-Welt sauber verbinden.
...
|---------|------------------------------|----------|--------------------------------------|
| OUT     | adapter::types::unidirectional::ALI | Plug (Ausgang) | Gepufferter empfangener LINT-Wert |
...
- **Pufferung mit D-Flipflop**: Der empfangene LINT-Wert wird über ein internes `iec61499::events::E_D_FF_ANY` gepuffert und bleibt stabil am Ausgang, bis der nächste Wert empfangen wird. `EO` feuert nur bei einer Änderung von `Q`.
...
- **Kapselung**: Die ursprüngliche Ereignis-/Daten-Schnittstelle von `SUBSCRIBE_1` (inkl. `IND`/`RD_1`) wird nach innen verlegt; nach außen ist nur noch die ALI-Adapter-Schnittstelle sichtbar.
...
3. **Datenempfang**: Bei `IND` wird der empfangene LINT-Wert in das interne D-Flipflop übernommen und über `OUT` ausgegeben.

## Anwendungsszenarien
- **Netzwerkkopplung in ALI-Anwendungen**: Empfang von LINT-Signalen (z. B. Schaltbefehle oder Statusbits) über Publish/Subscribe und Weiterleitung über ALI-Adapter an die Adapter-basierte Logik.
...
- **SUBSCRIBE_1**: Der reine Standardbaustein exponiert seine Schnittstelle als Events/Variablen (`IND`, `RD_1`, ...). **ALI_SUBSCRIBE_1** kapselt diesen Baustein, puffert den Wert mit einem D-Flipflop (`E_D_FF_ANY`) und stellt nur noch den ALI-Adapter bereit – dadurch ist die Schnittstelle erheblich kompakter und in Adapter-Netzwerke integrierbar.
- **AX_D_FF**: Puffert einen LINT-Wert auf Adapter-Ebene, kennt aber keine Netzwerkfunktionalität. **ALI_SUBSCRIBE_1** vereint Subscribing und Pufferung in einem Baustein.

## Fazit
**ALI_SUBSCRIBE_1** verbindet die Publish/Subscribe-Kommunikation des Standardbausteins `SUBSCRIBE_1` mit der Adapter-basierten LINT-Verarbeitung. Die interne D-Flipflop-Pufferung hält den empfangenen Wert stabil, bis neue Daten eintreffen, und die reduzierte ALI-Schnittstelle macht den Baustein ideal für durchgängig adapterbasierte IEC 61499-Anwendungen.
