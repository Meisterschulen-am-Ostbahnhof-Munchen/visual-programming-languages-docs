...
- **Kapselung**: Die ursprüngliche Ereignis-/Daten-Schnittstelle von `SUBSCRIBE_1` (inkl. `IND`/`RD_1`) wird nach innen verlegt; nach außen ist nur noch die AQ-Adapter-Schnittstelle sichtbar.
...
- **Netzwerkkopplung in AQ-Anwendungen**: Empfang von BYTE-Signalen (z. B. Schaltbefehle oder Statusbits) über Publish/Subscribe und Weiterleitung über AQ-Adapter an die Adapter-basierte Logik.
...
- **SUBSCRIBE_1**: Der reine Standardbaustein exponiert seine Schnittstelle als Events/Variablen (`IND`, `RD_1`, ...). **AQ_SUBSCRIBE_1** kapselt diesen Baustein, puffert den Wert mit einem D-Flipflop (`E_D_FF_ANY`) und stellt nur noch den AQ-Adapter bereit – dadurch ist die Schnittstelle erheblich kompakter und in Adapter-Netzwerke integrierbar.
- **AX_D_FF**: Puffert einen BYTE-Wert auf Adapter-Ebene, kennt aber keine Netzwerkfunktionalität. **AQ_SUBSCRIBE_1** vereint Subscribing und Pufferung in einem Baustein.

## Fazit

**AQ_SUBSCRIBE_1** verbindet die Publish/Subscribe-Kommunikation des Standardbausteins `SUBSCRIBE_1` mit der Adapter-basierten BYTE-Verarbeitung. Die interne D-Flipflop-Pufferung hält den empfangenen Wert stabil, bis neue Daten eintreffen, und die reduzierte AQ-Schnittstelle macht den Baustein ideal für durchgängig adapterbasierte IEC 61499-Anwendungen.
