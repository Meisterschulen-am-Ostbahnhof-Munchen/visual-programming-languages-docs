...
## Einleitung
Der Funktionsblock **AUI_PUBLISH_1** ist ein Composite-Funktionsblock, der den netzwerkbasierten `PUBLISH_1`-Funktionsblock aus der IEC 61499-Standardbibliothek kapselt und dessen Schnittstelle auf einen unidirektionalen **AUI-Adapter** abbildet. Ein am Adapter-Socket `IN` anliegender **UINT**-Wert wird über ein internes D-Flipflop (E_D_FF_ANY) gepuffert und anschließend über den konfigurierten Publish-Kanal an alle zugehörigen `SUBSCRIBE_1`-Empfänger gesendet.
...
|---------|------------------------------|----------|---------------------------------|
| IN      | adapter::types::unidirectional::AUI | Socket (Eingang) | Zu veröffentlichender UINT-Wert |

## Funktionsweise
1. Über das `INIT`-Ereignis wird der interne `PUBLISH_1`-Block mit `QI` und `ID` initialisiert. Bei Erfolg quittiert er mit `INITO`.
2. Sobald der AUI-Socket `IN` ein Ereignis auf `IN.E1` liefert, wird der gleichzeitig an `IN.D1` anliegende UINT-Wert in das interne **E_D_FF_ANY**-Flipflop übernommen (`CLK` bzw. `D`).
...
- **Pufferung mit D-Flipflop**: Der zu sendende UINT-Wert wird über ein internes `iec61499::events::E_D_FF_ANY` gepuffert. Dadurch wird verhindert, dass ein sich während der Verarbeitung ändernder Eingangswert den Sendevorgang stört.
...
- **Kapselung**: Die ursprüngliche Ereignis-/Daten-Schnittstelle von `PUBLISH_1` (inkl. `REQ`/`SD_1`) wird nach innen verlegt; nach außen ist nur noch die AUI-Adapter-Schnittstelle sichtbar.
...
3. **Sendeaktiv**: Ein am AUI-Socket eintreffendes Ereignis puffert den Wert und veranlasst das Senden über den internen `PUBLISH_1`-Block.

## Anwendungsszenarien
- **Netzwerkkopplung in AUI-Anwendungen**: Senden von UINT-Signalen (z. B. Schaltbefehle oder Statusbits) aus der Adapter-basierten Logik über Publish/Subscribe an entfernte Empfänger.
...
- **PUBLISH_1**: Der reine Standardbaustein exponiert seine Schnittstelle als Events/Variablen (`REQ`, `SD_1`, ...). **AUI_PUBLISH_1** kapselt diesen Baustein, puffert den Wert mit einem D-Flipflop (`E_D_FF_ANY`) und stellt nur noch den AUI-Adapter bereit – dadurch ist die Schnittstelle erheblich kompakter und in Adapter-Netzwerke integrierbar.
- **AX_D_FF**: Puffert einen UINT-Wert auf Adapter-Ebene, kennt aber keine Netzwerkfunktionalität. **AUI_PUBLISH_1** vereint Puffern und Veröffentlichen in einem Baustein.

## Fazit
**AUI_PUBLISH_1** verbindet die Publish/Subscribe-Kommunikation des Standardbausteins `PUBLISH_1` mit der Adapter-basierten UINT-Verarbeitung. Die interne D-Flipflop-Pufferung stabilisiert den Sendewert und entkoppelt Rückkopplungsschleifen; die reduzierte AUI-Schnittstelle macht den Baustein ideal für durchgängig adapterbasierte IEC 61499-Anwendungen.
