...
![AX_PUBLISH_1](./AX_PUBLISH_1.svg)

* * * * * * * * * *

## Einleitung

Der Funktionsblock **AX_PUBLISH_1** ist ein Composite-Funktionsblock, der den netzwerkbasierten `PUBLISH_1`-Funktionsblock aus der IEC 61499-Standardbibliothek kapselt und dessen Schnittstelle auf einen unidirektionalen **AX-Adapter** abbildet. Ein am Adapter-Socket `IN` anliegender **BOOL**-Wert wird über ein internes D-Flipflop (E_D_FF) gepuffert und anschließend über den konfigurierten Publish-Kanal an alle zugehörigen `SUBSCRIBE_1`-Empfänger gesendet.
...
- **Ereignisbasierte Entkopplung**: Durch das echte ereignisbasierte `E_D_FF` werden unmittelbare Rückkopplungsschleifen zwischen `PUBLISH_1` und `SUBSCRIBE_1` (z. B. auf demselben OPC-UA-Knoten) entkoppelt – der saubere Fix gegenüber dem Interim-Fix mit `AX_D_FF`.
- **Kapselung**: Die ursprüngliche Ereignis-/Daten-Schnittstelle von `PUBLISH_1` (inkl. `REQ`/`SD_1`) wird nach innen verlegt; nach außen ist nur noch die AX-Adapter-Schnittstelle sichtbar.
...
3. **Sendeaktiv**: Ein am AX-Socket eintreffendes Ereignis puffert den Wert und veranlasst das Senden über den internen `PUBLISH_1`-Block.

## Anwendungsszenarien

- **Netzwerkkopplung in AX-Anwendungen**: Senden von Bool-Signalen (z. B. Schaltbefehle oder Statusbits) aus der Adapter-basierten Logik über Publish/Subscribe an entfernte Empfänger.
...
- **PUBLISH_1**: Der reine Standardbaustein exponiert seine Schnittstelle als Events/Variablen (`REQ`, `SD_1`, ...). **AX_PUBLISH_1** kapselt diesen Baustein, puffert den Wert mit einem D-Flipflop und stellt nur noch den AX-Adapter bereit – dadurch ist die Schnittstelle erheblich kompakter und in Adapter-Netzwerke integrierbar.
- **AX_D_FF**: Puffert einen Bool-Wert auf Adapter-Ebene, kennt aber keine Netzwerkfunktionalität. **AX_PUBLISH_1** vereint Puffern und Veröffentlichen in einem Baustein.

## Fazit

**AX_PUBLISH_1** verbindet die Publish/Subscribe-Kommunikation des Standardbausteins `PUBLISH_1` mit der Adapter-basierten Bool-Verarbeitung. Die interne D-Flipflop-Pufferung stabilisiert den Sendewert und entkoppelt Rückkopplungsschleifen; die reduzierte AX-Schnittstelle macht den Baustein ideal für durchgängig adapterbasierte IEC 61499-Anwendungen.
