...
![AD_AUI_DEMUX_2](AD_AUI_DEMUX_2.svg)

* * * * * * * * * *
## Einleitung

Der `AD_AUI_DEMUX_2` ist die adapterbasierte Variante des generischen Demultiplexers für den Datentyp `DWORD`. Anders als [AD_DEMUX_2](AD_DEMUX_2.md) erhält er den Auswahlindex nicht über ein REQ-Ereignis mit zugehörigem K-Dateneingang, sondern über einen eigenen Adapter-Socket **K** vom Typ `AUI` („Adapter Unidirectional Interface“). Das erlaubt es, den Index direkt aus einem anderen Baustein mit passendem `AUI`-Plug einzuspeisen, ohne eigene Verdrahtung von Ereignis- und Datenleitung.

## Schnittstellenstruktur

### **Ereignis-Eingänge**

- Keine expliziten Ereignis-Eingänge am Baustein selbst. Der Index wird ausschließlich über den Adapter-Socket **K** empfangen: Sobald der dort angeschlossene `AUI`-Plug sein internes Ereignis `E1` mit dem Datenwert `D1` sendet, wertet `AD_AUI_DEMUX_2` intern den Index aus und löst die Verarbeitung aus.
...
Der `AD_AUI_DEMUX_2` empfängt über den Socket **IN** den am Eingang anliegenden Wert (`DWORD`, 32-Bit-Bitmuster) und leitet ihn an genau einen der 2 Ausgangs-Plugs `OUT1` … `OUT2` weiter. Welcher Ausgang beschrieben wird, bestimmt der Index, der über den Adapter-Socket **K** (Typ `AUI`) hereinkommt: Sobald der am Socket K angeschlossene `AUI`-Plug sein internes `E1`-Ereignis mit dem Datenwert `D1` sendet, wertet der `AD_AUI_DEMUX_2` diesen Wert als Index (0-basiert, `0` … `1`) aus, schreibt den Eingangswert auf den entsprechenden `OUT`-Plug und löst dort dessen Adapter-Event aus. Anschließend meldet `AD_AUI_DEMUX_2` den Abschluss der Operation über das Ereignis **CNF**.
