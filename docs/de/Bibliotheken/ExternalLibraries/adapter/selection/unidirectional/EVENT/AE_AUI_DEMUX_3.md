...
- Keine expliziten Ereignis-Eingänge am Baustein selbst. Der Index wird ausschließlich über den Adapter-Socket **K** empfangen: Sobald der dort angeschlossene `AUI`-Plug sein internes Ereignis `E1` mit dem Datenwert `D1` sendet, wertet `AE_AUI_DEMUX_3` intern den Index aus und löst die Verarbeitung aus.
...
- **OUT1** (Plug): Ausgangsadapter 1 von 3 (AE-Adaptertyp).
- **OUT2** (Plug): Ausgangsadapter 2 von 3 (AE-Adaptertyp).
- **OUT3** (Plug): Ausgangsadapter 3 von 3 (AE-Adaptertyp).

## Funktionsweise

Der `AE_AUI_DEMUX_3` empfängt über den Socket **IN** das durchgereichte Ereignis und leitet ihn an genau einen der 3 Ausgangs-Plugs `OUT1` … `OUT3` weiter. Welcher Ausgang beschrieben wird, bestimmt der Index, der über den Adapter-Socket **K** (Typ `AUI`) hereinkommt: Sobald der am Socket K angeschlossene `AUI`-Plug sein internes `E1`-Ereignis mit dem Datenwert `D1` sendet, wertet der `AE_AUI_DEMUX_3` diesen Wert als Index (0-basiert, `0` … `2`) aus, schreibt Ereignis auf den entsprechenden `OUT`-Plug und löst dort dessen Adapter-Event aus. Anschließend meldet `AE_AUI_DEMUX_3` den Abschluss der Operation über das Ereignis **CNF**.
...
- Der `AE`-Adapter überträgt ausschließlich ein Ereignis, keinen Datenwert -- `AE_AUI_DEMUX_3` schaltet also reine Ereignisimpulse, keine Nutzdaten.
...
- Verteilen eines Steuer-Ereignisses auf bis zu 3 nachgeschaltete Verbraucher, ausgewählt über einen Adapter-Index
