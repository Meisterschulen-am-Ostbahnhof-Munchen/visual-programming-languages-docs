...
- Keine expliziten Ereignis-Eingänge am Baustein selbst. Der Index wird ausschließlich über den Adapter-Socket **K** empfangen: Sobald der dort angeschlossene `AUI`-Plug sein internes Ereignis `E1` mit dem Datenwert `D1` sendet, wertet `AE_AUI_MUX_3` intern den Index aus und löst die Verarbeitung aus.
...
- **IN1** (Socket): Eingangsadapter 1 von 3, wird bei Index `K = 0` an `OUT` durchgereicht (AE-Adaptertyp).
- **IN2** (Socket): Eingangsadapter 2 von 3, wird bei Index `K = 1` an `OUT` durchgereicht (AE-Adaptertyp).
- **IN3** (Socket): Eingangsadapter 3 von 3, wird bei Index `K = 2` an `OUT` durchgereicht (AE-Adaptertyp).
- **OUT** (Plug): Ausgangsadapter, gibt den über den Index ausgewählten Eingang weiter (AE-Adaptertyp).

## Funktionsweise

Der `AE_AUI_MUX_3` wählt über den Adapter-Socket **K** (Typ `AUI`) einen von 3 Eingangs-Sockets `IN1` … `IN3` aus und reicht dessen Wert an den einzigen Ausgangs-Plug `OUT` weiter. Sobald der an K angeschlossene `AUI`-Plug sein internes `E1`-Ereignis mit dem Datenwert `D1` sendet, interpretiert `AE_AUI_MUX_3` diesen Wert als 0-basierten Index (`0` … `2`), kopiert das durchgereichte Ereignis auf `OUT` und löst dort das Adapter-Event aus. Danach bestätigt `AE_AUI_MUX_3` den Vorgang über das Ereignis **CNF**.
...
- Der `AE`-Adapter überträgt ausschließlich ein Ereignis, keinen Datenwert -- `AE_AUI_MUX_3` schaltet also reine Ereignisimpulse, keine Nutzdaten.
...
- Auswahl zwischen bis zu 3 Ereignisquellen über einen zentral verwalteten Adapter-Index
