...
- **OUT1** (Plug): Ausgangsadapter 1 von 2 (AIWS-Adaptertyp).
- **OUT2** (Plug): Ausgangsadapter 2 von 2 (AIWS-Adaptertyp).

## Funktionsweise

Der `AIWS_AUI_DEMUX_2` empfängt über den Socket **IN** den am Eingang anliegenden Wert (`WSTRING`, Zeichenkette (16-Bit-Wide-Zeichen)) und leitet ihn an genau einen der 2 Ausgangs-Plugs `OUT1` … `OUT2` weiter. Welcher Ausgang beschrieben wird, bestimmt der Index, der über den Adapter-Socket **K** (Typ `AUI`) hereinkommt: Sobald der am Socket K angeschlossene `AUI`-Plug sein internes `E1`-Ereignis mit dem Datenwert `D1` sendet, wertet der `AIWS_AUI_DEMUX_2` diesen Wert als Index (0-basiert, `0` … `1`) aus, schreibt den Eingangswert auf den entsprechenden `OUT`-Plug und löst dort dessen Adapter-Event aus. Anschließend meldet `AIWS_AUI_DEMUX_2` den Abschluss der Operation über das Ereignis **CNF**.
...
- Verteilen eines Wertes (`WSTRING`, Zeichenkette (16-Bit-Wide-Zeichen)) auf bis zu 2 nachgeschaltete Verbraucher, ausgewählt über einen Adapter-Index
