...
Der `AND_BOOL_10`-Block besitzt keinen internen Zustand (speicherlos). Sein Ausgang hängt ausschließlich von den aktuellen Werten der Eingänge zum Zeitpunkt der Auslösung durch `REQ` ab. Es gibt keine Verzögerungen, Filter oder Hysterese.
...
-   **`AND` (mit 2 Eingängen):** Der Standard-AND-Baustein nach IEC 61131-3 hat typischerweise nur zwei Eingänge. `AND_BOOL_10` erweitert diese Funktionalität auf zehn Eingänge ohne die Notwendigkeit, mehrere Zwei-Eingangs-AND-Blöcke zu verschalten. Siehe: [AND_10](../../../StandardLibraries/iec61131-3/bitwiseOperators/AND_10.md)
-   **`OR_BOOL_10`:** Führt eine logische ODER-Verknüpfung durch. Das Ergebnis ist TRUE, wenn mindestens ein Eingang TRUE ist, während `AND_BOOL_10` verlangt, dass alle Eingänge TRUE sind.
