...
-   **`OR_BOOL_2` / `OR_BOOL_8`**: Diese Blöcke führen die identische logische ODER-Operation durch, jedoch mit einer unterschiedlichen Anzahl von Eingängen (2 bzw. 8). Die Wahl des Bausteins hängt von der benötigten Anzahl an zu verknüpfenden Signalen ab. Siehe: [OR_4](../../../StandardLibraries/iec61131-3/bitwiseOperators/OR_4.md)
-   **`AND_BOOL_4`**: Führt eine logische UND-Verknüpfung durch. Das Ergebnis ist nur `TRUE`, wenn *alle* Eingänge `TRUE` sind.
-   **`XOR_BOOL_4`**: Führt eine exklusiv-ODER (XOR) Verknüpfung durch. Das Ergebnis ist `TRUE`, wenn eine ungerade Anzahl an Eingängen `TRUE` ist.

## Fazit

Der `OR_BOOL_4` ist ein grundlegender, zuverlässiger und standardkonformer Funktionsblock zur Realisierung einer ODER-Logik mit vier Eingängen. Seine ereignisgesteuerte, zustandslose Arbeitsweise macht ihn ideal für die Integration in größere, ereignisgesteuerte Steuerungsanwendungen nach IEC 61131-3. Durch seine generische Natur dient er zudem als Basis für die Bibliothekserweiterung.
