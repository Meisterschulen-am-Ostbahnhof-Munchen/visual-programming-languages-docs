...
-   **Fest verdrahtete Logik:** Die ODER-Verknüpfung ist für genau sieben Eingänge fest implementiert. Für eine andere Anzahl von Eingängen muss ein entsprechender anderer Baustein (z.B. `OR_BOOL_2`, `OR_BOOL_4`) verwendet werden.
...
-   **AND_BOOL_7:** Führt eine logische UND-Verknüpfung durch. Der Ausgang ist nur `TRUE`, wenn *alle* Eingänge `TRUE` sind.
-   **XOR_BOOL_7:** Führt eine exklusiv-ODER (XOR) Verknüpfung durch. Der Ausgang ist `TRUE`, wenn eine ungerade Anzahl an Eingängen `TRUE` ist.
