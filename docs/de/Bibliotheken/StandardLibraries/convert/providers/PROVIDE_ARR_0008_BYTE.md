...
- **D1** (`BYTE`, Array-Größe 8, Index 0…7): Der Array-Container (8-Bit-Bitmuster je Element). Kann im FB-Instanzeditor mit Startwerten vorbelegt werden (z. B. `[16#01, 16#00, ...]` für `BYTE`).
...
- **Feste Größe 8**: Weitere Größen derselben oder anderer Datentypen: `PROVIDE_ARR_0002_BYTE`, `PROVIDE_ARR_0007_BYTE`, `PROVIDE_ARR_0014_BYTE`, `PROVIDE_ARR_0016_BYTE`.
...
- `PROVIDE_ARR_0002_BYTE`, `PROVIDE_ARR_0007_BYTE`, `PROVIDE_ARR_0014_BYTE`, `PROVIDE_ARR_0016_BYTE`: dieselbe generische Implementierung mit anderer Array-Größe bzw. anderem Datentyp.
- **`ARRAY2ARRAY_8_BYTE`**: kopiert ein bereits vorhandenes Array, ohne es bereitzustellen/vorzubelegen.

## Fazit

`PROVIDE_ARR_0008_BYTE` liefert einen einfachen, zur Entwurfszeit vorbelegbaren `BYTE`-Array-Container fester Größe 8 und eignet sich für Konfigurations- und Testdatenarrays, die wiederverwendbar in 4diac-Applikationen eingebunden werden sollen.
