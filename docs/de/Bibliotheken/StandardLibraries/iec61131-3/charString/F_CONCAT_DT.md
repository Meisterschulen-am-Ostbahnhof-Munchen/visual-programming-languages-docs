...
- **OUT** (`DATE_AND_TIME`): Der aus den Einzelbestandteilen zusammengesetzte Wert.

## Funktionsweise

Bei Eintreffen von `REQ` werden die Eingangswerte `YEAR`, `MONTH`, `DAY`, `HOUR`, `MINUTE`, `SECOND`, `MILLISECOND` zu einem `DATE_AND_TIME`-Wert kombiniert und über `OUT` ausgegeben. Anschließend wird `CNF` ausgelöst.
...
- **Konfigurationsauswertung**: Zusammenführen von in einzelnen Variablen abgelegten Datums-/Zeitangaben zu einem verwendbaren `DATE_AND_TIME`-Wert.

## ⚖️ Vergleich mit ähnlichen Bausteinen

- **[F_SPLIT_DT](F_SPLIT_DT.md)**: die Umkehrrichtung — zerlegt einen `DATE_AND_TIME`-Wert in seine Einzelbestandteile.
...
`F_CONCAT_DT` liefert eine einfache, direkte Zusammenführung von Zeit-/Datumseinzelwerten zu einem `DATE_AND_TIME`-Wert und ergänzt damit die entsprechende Zerlegungsfunktion [F_SPLIT_DT](F_SPLIT_DT.md).
