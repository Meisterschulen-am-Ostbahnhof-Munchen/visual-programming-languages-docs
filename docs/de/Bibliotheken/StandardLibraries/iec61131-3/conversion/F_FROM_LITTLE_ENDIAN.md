...
Bei Eintreffen von `REQ` interpretiert die Bytefolge von `IN` als Little-Endian (niedrigstwertiges Byte zuerst)-kodierten Wert und wandelt sie in die native Byte-Reihenfolge der Zielplattform um. Das Ergebnis wird über `OUT` ausgegeben, anschließend wird `CNF` ausgelöst. Die Bitbreite des Datentyps bleibt dabei unverändert — es wird ausschließlich die Byte-Reihenfolge angepasst.
...
- **Netzwerkkommunikation**: Viele Netzwerkprotokolle (z. B. TCP/IP-Header) verwenden Big-Endian ("Network Byte Order"); `F_FROM_LITTLE_ENDIAN` stellt die korrekte Byte-Reihenfolge beim Senden/Empfangen sicher.
...
- **`F_FROM_BIG_ENDIAN`**: dieselbe Konvertierungsrichtung (aus), aber für die jeweils andere Byte-Reihenfolge.
- **`F_TO_LITTLE_ENDIAN`**: die Gegenrichtung für dieselbe Byte-Reihenfolge.

## Fazit

`F_FROM_LITTLE_ENDIAN` liefert eine einfache, generisch typisierte Byte-Reihenfolgen-Konvertierung und ist ein wichtiger Baustein für die korrekte Interoperabilität mit Netzwerkprotokollen und Fremdsystemen unterschiedlicher Endianness.
