...
- Keine Empfangsdaten (`CLIENT_2_0` erhält keine Nutzdaten über `CNF`, nur Verbindungs-/Statusinformationen).

## Funktionsweise

`CLIENT_2_0` initialisiert über `INIT` eine Verbindung zum passenden `SERVER_0_2`-Block (bei `QI = TRUE`) bzw. schließt sie (bei `QI = FALSE`); der Abschluss wird über `INITO` bestätigt. Über `REQ` werden `SD_1` `SD_2` an den Server übertragen. Sobald eine Antwort vorliegen, löst der Baustein `CNF` aus.
...
- **Gegenstück `SERVER_0_2`**: `CLIENT_2_0` ist funktional nur mit einem `SERVER_0_2`-Block auf der Server-Seite kompatibel — die Anzahl der Sendefelder der einen Seite muss der Anzahl der Empfangsfelder der Gegenseite entsprechen.
...
- **Client-seitige Anbindung** eines Kommunikationspartners, der bereits als `SERVER_0_2` mit passender Feldanzahl ausgelegt ist.

## ⚖️ Vergleich mit ähnlichen Bausteinen

- **[SERVER_0_2](SERVER_0_2.md)**: das direkte Gegenstück auf der Server-Seite, mit vertauschter Sende-/Empfangsfeldanzahl.
- **[CLIENT_1](CLIENT_1.md) / [SERVER_1](SERVER_1.md)**: dieselbe generische Implementierung mit je einem Sende- und Empfangsfeld.

## Fazit

`CLIENT_2_0` liefert die generische, auf 2 Sende- und 0 Empfangsfelder zugeschnittene Client-Variante der `GEN_CLIENT`-Familie und eignet sich für Netzwerkverbindungen, deren Nutzdatenanzahl von der Standardvariante mit einem Feld abweicht.
