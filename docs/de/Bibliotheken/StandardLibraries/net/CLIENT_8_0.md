...
Der `CLIENT_8_0`-Funktionsblock ist die generische Client-Variante mit 8 Sende- und 0 Empfangsdatenfeldern für die Kommunikation mit einem passenden [SERVER_0_8](SERVER_0_8.md)-Block. Er überträgt 8 Datenwerte (`SD_1` `SD_2` `SD_3` `SD_4` `SD_5` `SD_6` `SD_7` `SD_8`) an den Server, ohne Rückgabedaten zu empfangen. Wie alle `CLIENT_*`-Bausteine basiert er auf der generischen `GEN_CLIENT`-Implementierung — dieselbe C++-Basis wie [CLIENT_1](CLIENT_1.md)/[SERVER_1](SERVER_1.md), lediglich die Anzahl der Sende-/Empfangsfelder unterscheidet sich pro Instanziierung.
...
- Keine Empfangsdaten (`CLIENT_8_0` erhält keine Nutzdaten über `CNF`, nur Verbindungs-/Statusinformationen).

## Funktionsweise

`CLIENT_8_0` initialisiert über `INIT` eine Verbindung zum passenden `SERVER_0_8`-Block (bei `QI = TRUE`) bzw. schließt sie (bei `QI = FALSE`); der Abschluss wird über `INITO` bestätigt. Über `REQ` werden `SD_1` `SD_2` `SD_3` `SD_4` `SD_5` `SD_6` `SD_7` `SD_8` an den Server übertragen. Sobald eine Antwort vorliegen, löst der Baustein `CNF` aus.
...
- **Gegenstück `SERVER_0_8`**: `CLIENT_8_0` ist funktional nur mit einem `SERVER_0_8`-Block auf der Server-Seite kompatibel — die Anzahl der Sendefelder der einen Seite muss der Anzahl der Empfangsfelder der Gegenseite entsprechen.
...
- **Kommunikation zwischen verteilten Steuerungssystemen**, bei denen genau 8 Werte gesendet und 0 Werte empfangen werden sollen, ohne überzählige, ungenutzte Datenfelder in Kauf zu nehmen.
- **Client-seitige Anbindung** eines Kommunikationspartners, der bereits als `SERVER_0_8` mit passender Feldanzahl ausgelegt ist.

## ⚖️ Vergleich mit ähnlichen Bausteinen

- **[SERVER_0_8](SERVER_0_8.md)**: das direkte Gegenstück auf der Server-Seite, mit vertauschter Sende-/Empfangsfeldanzahl.
- **[CLIENT_1](CLIENT_1.md) / [SERVER_1](SERVER_1.md)**: dieselbe generische Implementierung mit je einem Sende- und Empfangsfeld.

## Fazit

`CLIENT_8_0` liefert die generische, auf 8 Sende- und 0 Empfangsfelder zugeschnittene Client-Variante der `GEN_CLIENT`-Familie und eignet sich für Netzwerkverbindungen, deren Nutzdatenanzahl von der Standardvariante mit einem Feld abweicht.
