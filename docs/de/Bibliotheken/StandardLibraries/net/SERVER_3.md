...
Der `SERVER_3`-Funktionsblock ist die generische Server-Variante mit 3 Sende- und 3 Empfangsdatenfeldern für die Kommunikation mit einem passenden [CLIENT_3](CLIENT_3.md)-Block. Er überträgt 3 Datenwerte (`SD_1` `SD_2` `SD_3`) an den Client und empfängt 3 Datenwerte (`RD_1` `RD_2` `RD_3`) zurück. Wie alle `SERVER_*`-Bausteine basiert er auf der generischen `GEN_SERVER`-Implementierung — dieselbe C++-Basis wie [CLIENT_1](CLIENT_1.md)/[SERVER_1](SERVER_1.md), lediglich die Anzahl der Sende-/Empfangsfelder unterscheidet sich pro Instanziierung.
...
`SERVER_3` initialisiert über `INIT` eine Verbindung zum passenden `CLIENT_3`-Block (bei `QI = TRUE`) bzw. schließt sie (bei `QI = FALSE`); der Abschluss wird über `INITO` bestätigt. Über `RSP` werden `SD_1` `SD_2` `SD_3` an den Client übertragen. Sobald Antwortdaten (`RD_1` `RD_2` `RD_3`) vorliegen, löst der Baustein `IND` aus und stellt `RD_1` `RD_2` `RD_3` bereit.
...
- **Gegenstück `CLIENT_3`**: `SERVER_3` ist funktional nur mit einem `CLIENT_3`-Block auf der Client-Seite kompatibel — die Anzahl der Sendefelder der einen Seite muss der Anzahl der Empfangsfelder der Gegenseite entsprechen.
...
- **Server-seitige Anbindung** eines Kommunikationspartners, der bereits als `CLIENT_3` mit passender Feldanzahl ausgelegt ist.

## ⚖️ Vergleich mit ähnlichen Bausteinen

- **[CLIENT_3](CLIENT_3.md)**: das direkte Gegenstück auf der Client-Seite, mit vertauschter Sende-/Empfangsfeldanzahl.
- **[CLIENT_1](CLIENT_1.md) / [SERVER_1](SERVER_1.md)**: dieselbe generische Implementierung mit je einem Sende- und Empfangsfeld.

## Fazit

`SERVER_3` liefert die generische, auf 3 Sende- und 3 Empfangsfelder zugeschnittene Server-Variante der `GEN_SERVER`-Familie und eignet sich für Netzwerkverbindungen, deren Nutzdatenanzahl von der Standardvariante mit einem Feld abweicht.
