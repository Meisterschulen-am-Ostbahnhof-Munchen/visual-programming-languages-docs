...
Der `SERVER_4`-Funktionsblock ist die generische Server-Variante mit 4 Sende- und 4 Empfangsdatenfeldern für die Kommunikation mit einem passenden [CLIENT_4](CLIENT_4.md)-Block. Er überträgt 4 Datenwerte (`SD_1` `SD_2` `SD_3` `SD_4`) an den Client und empfängt 4 Datenwerte (`RD_1` `RD_2` `RD_3` `RD_4`) zurück. Wie alle `SERVER_*`-Bausteine basiert er auf der generischen `GEN_SERVER`-Implementierung — dieselbe C++-Basis wie [CLIENT_1](CLIENT_1.md)/[SERVER_1](SERVER_1.md), lediglich die Anzahl der Sende-/Empfangsfelder unterscheidet sich pro Instanziierung.
...
- **RSP**: Sendet Daten an den Client, trägt `QI` sowie `SD_1` `SD_2` `SD_3` `SD_4`.
...
`SERVER_4` initialisiert über `INIT` eine Verbindung zum passenden `CLIENT_4`-Block (bei `QI = TRUE`) bzw. schließt sie (bei `QI = FALSE`); der Abschluss wird über `INITO` bestätigt. Über `RSP` werden `SD_1` `SD_2` `SD_3` `SD_4` an den Client übertragen. Sobald Antwortdaten (`RD_1` `RD_2` `RD_3` `RD_4`) vorliegen, löst der Baustein `IND` aus und stellt `RD_1` `RD_2` `RD_3` `RD_4` bereit.
...
- **Gegenstück `CLIENT_4`**: `SERVER_4` ist funktional nur mit einem `CLIENT_4`-Block auf der Client-Seite kompatibel — die Anzahl der Sendefelder der einen Seite muss der Anzahl der Empfangsfelder der Gegenseite entsprechen.
...
- **Kommunikation zwischen verteilten Steuerungssystemen**, bei denen genau 4 Werte gesendet und 4 Werte empfangen werden sollen, ohne überzählige, ungenutzte Datenfelder in Kauf zu nehmen.
- **Server-seitige Anbindung** eines Kommunikationspartners, der bereits als `CLIENT_4` mit passender Feldanzahl ausgelegt ist.

## ⚖️ Vergleich mit ähnlichen Bausteinen

- **[CLIENT_4](CLIENT_4.md)**: das direkte Gegenstück auf der Client-Seite, mit vertauschter Sende-/Empfangsfeldanzahl.
- **[CLIENT_1](CLIENT_1.md) / [SERVER_1](SERVER_1.md)**: dieselbe generische Implementierung mit je einem Sende- und Empfangsfeld.

## Fazit

`SERVER_4` liefert die generische, auf 4 Sende- und 4 Empfangsfelder zugeschnittene Server-Variante der `GEN_SERVER`-Familie und eignet sich für Netzwerkverbindungen, deren Nutzdatenanzahl von der Standardvariante mit einem Feld abweicht.
