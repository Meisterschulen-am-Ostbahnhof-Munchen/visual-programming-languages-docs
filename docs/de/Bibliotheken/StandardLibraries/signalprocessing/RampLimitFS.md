...
- **`INIT` ist Pflicht vor dem ersten Ereignis**:
  Jedes Ereignis liest per `WITH`-Deklaration nur die Dateneingänge ein, die es für seine eigene Berechnung braucht. Da jedes Ereignis am Ende `qAtZero`/`qAtFull` neu berechnet, steht in jeder WITH-Liste sowohl `VAL_ZERO` als auch `VAL_FULL` — z. B. liest `UP_SLOW` `SLOW`, `VAL_FULL` (für die eigentliche Begrenzung) **und** `VAL_ZERO` (nur für `qAtZero`), `DOWN_SLOW` liest `SLOW`, `VAL_ZERO` (Begrenzung) **und** `VAL_FULL` (nur für `qAtFull`). Ohne einen vorherigen `INIT`-Aufruf haben diese Werte noch nie einen gültigen Wert erhalten und stehen auf ihrem Vorgabewert 0 — bei `VAL_ZERO` fällt das meist nicht auf, bei `VAL_FULL` klemmt dann aber jeder `UP_SLOW`/`UP_FAST`-Schritt sofort auf 0.
