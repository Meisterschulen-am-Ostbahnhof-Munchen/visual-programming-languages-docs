...
`GreenWhiteBackground4C` ist ein **Kompakt-Wrapper** um [`GreenWhiteBackground4`](GreenWhiteBackground4.md) — reicht `u16ObjId` und den Selector durch, ohne die `CNF`-Ereignisausgänge und Diagnosewerte (`STATUS_n`, `u8OldColour_n`, `result_n`) nach außen zu führen.
