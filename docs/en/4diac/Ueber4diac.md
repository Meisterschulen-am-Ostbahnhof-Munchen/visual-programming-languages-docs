# About Eclipse 4diac™
## A Poem about 4diac in the Style of Goethe
In 4diac, streams of code flow,
it lives and pulsates in its own ode.
It dances to the rhythm of bits and bytes,
merging electronics and automation into one.
In its depths, like an ocean of data,
flows the life of processes and machine rates.

It weaves a network from sensor to actuator,
creating connections from remote control to the control room.

The 4diac system is the pacemaker,
guiding the steps of robotics and controlling the levers.

It is the heart that beats in time,
bringing all machines into harmonious accord.

O Goethe, if only you were still here,
you would see 4diac in all its splendor and beauty.

Yet its beauty is visible to us all;
we marvel at and admire this technological marvel.

## xx

[Literatur.md#sonstiges-englisch](../Literatur/SonstigesEnglisch.md)

[https://www.eclipse.org/4diac/](https://www.eclipse.org/4diac/)

[https://www.acin.tuwien.ac.at/en/](https://www.acin.tuwien.ac.at/en/)

[https://www.fortiss.org/](https://www.fortiss.org/)

[https://www.eclipse.org/4diac/events/docs/2019/EclipseInsight_SelfConfigurableAutomationSystems.pdf](https://web.archive.org/web/20230129204811/https://www.eclipse.org/4diac/events/docs/2019/EclipseInsight_SelfConfigurableAutomationSystems.pdf)

fortiss GmbH, State Research Institute of the Free State of Bavaria for Software-Intensive Systems

Guerickestr. 25 · 80805 Munich · Germany

[https://www.fortiss.org/aktuelles/details/professur-fuer-fortiss-wissenschaftler](https://www.fortiss.org/aktuelles/details/professur-fuer-fortiss-wissenschaftler)

[https://www.jku.at/lit-cyber-physical-systems-lab/ueber-uns/team/univ-prof-di-dr-alois-zoitl/](https://www.jku.at/lit-cyber-physical-systems-lab/ueber-uns/team/univ-prof-di-dr-alois-zoitl/)

## History:

The first version of 4diac was presented by ACIN TU Vienna on July 7, 2007.

[ This followed approximately two years of research and development.

In the meantime, 4diac was developed at fortiss and is now developed at JKU.

On July 7, 2022, 4diac celebrated its 15th anniversary.

## Open Source

[https://de.wikipedia.org/wiki/Open_Source](https://de.wikipedia.org/wiki/Open_Source)

4diac is completely open source. This means it is available free of charge, and anyone can further develop and adapt the program.

Details are governed by this agreement:

[https://www.eclipse.org/legal/epl-2.0/](https://www.eclipse.org/legal/epl-2.0/)

[https://de.wikipedia.org/wiki/Eclipse_Public_License](https://de.wikipedia.org/wiki/Eclipse_Public_License)

---

### 🌐 Related topic subpages on ms-muc-docs.de
* [🌐 Eclipse 4diac IDE & Color Reference on ms-muc-docs.de](https://www.ms-muc-docs.de/iec-61499/eclipse-4diac/)

