...
`RedWhiteBackground4_AXS` switches the VT background color of 4 objects based on a boolean selector signal: `TRUE` → **Rot**, `FALSE` → **Weiß**. The selector signal arrives via an `AX` adapter socket (`DI1`). The object ID is passed via the structured type `s1ObjectID` (`u16ObjIds`).
...
One of many variants in the background color block family: color pair Rot/Weiß, 4 objects, adapter selector, struct. object ID.
