...
**SAFE_ADD_2** is part of the **SafeArithmetic** library (`SafeArithmetic::arithmetic`, EPL-2.0,
HR Agrartechnik GmbH). It is a generic, 2-input addition function block that behaves like the
standard library's `ADD_2`, but never lets an overflow pass silently: on overflow the
result is **clamped** (saturated) into the valid range of the result type, and `LIMIT_HIT`
reports that clamping happened.

This library exists because plain IEC 61131-3 arithmetic function blocks (like `ADD_2`
...
- [ADD_2](../../../StandardLibraries/iec61131-3/arithmetic/ADD_2.md) — the
...
SAFE_ADD_2 gives you the same 2-input addition as `ADD_2`, but with an explicit,
