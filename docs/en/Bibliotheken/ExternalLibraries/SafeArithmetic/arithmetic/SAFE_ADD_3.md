...
HR Agrartechnik GmbH). It is a generic, 3-input addition function block that behaves like the
standard library's `ADD_3`, but never lets an overflow pass silently: on overflow the
result is **clamped** (saturated) into the valid range of the result type, and `LIMIT_HIT`
reports that clamping happened.

This library exists because plain IEC 61131-3 arithmetic function blocks (like `ADD_3`
...
`SAFE_ADD_3` evaluates as a strict left-to-right pairwise fold: `((IN1 + IN2) + IN3) + ...`,
...
Example (INT, IN1=20000, IN2=20000, IN3=-30000): the true result is `10000`, well within INT's range, but
...
evaluates as a real chain of fixed-width operations, exactly like the plain (non-safe) `ADD_3`
...
- [ADD_3](../../../StandardLibraries/iec61131-3/arithmetic/ADD_3.md) — the
...
SAFE_ADD_3 gives you the same 3-input addition as `ADD_3`, but with an explicit,
