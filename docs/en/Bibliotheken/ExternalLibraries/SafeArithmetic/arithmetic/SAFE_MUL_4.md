...
**SAFE_MUL_4** is part of the **SafeArithmetic** library (`SafeArithmetic::arithmetic`, EPL-2.0,
HR Agrartechnik GmbH). It is a generic, 4-input multiplication function block that extends the
standard library's fixed 2-input `F_MUL` pattern to 4 inputs (the standard library has no
...
`SAFE_MUL_4` evaluates as a strict left-to-right pairwise fold: `((IN1 * IN2) * IN3) * ...`,
...
Example (INT, IN1=40000, IN2=40000, IN3=0, IN4=1): the true result is `0`, well within INT's range, but
...
SAFE_MUL_4 extends `F_MUL`'s multiplication to 4 inputs, with an explicit,
