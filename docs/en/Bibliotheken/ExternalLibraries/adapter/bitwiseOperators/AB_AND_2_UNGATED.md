...
> ℹ️ **UNGATED variant:** This block is the ungated version of [`AB_AND_2`](AB_AND_2.md). It suppresses **no** unchanged repeats – every newly computed result is forwarded unconditionally, even without a value change. This matters for consumers that need a periodic cadence regardless of value change (e.g. derivative/frequency calculations that would otherwise fail to decay toward zero). Any change-detection/gating statements further down this page do **not** apply to this block.
...
**AB_AND_2_UNGATED** is a generic function block for the bitwise AND operation across 2 input values of type `BYTE` (8-bit bit pattern (byte)). Unlike the Boolean operation on single truth values (as in the `AX_AND` blocks), every bit of the data word is combined independently here.
...
Since this is a combinational logic block, AB_AND_2_UNGATED has no internal states. The output is recalculated directly from the current input values on every incoming event.
...
Unlike `AX_AND_2`, which combines individual Boolean truth values, `AB_AND_2_UNGATED` operates on the full bit pattern of a `BYTE` value. Compared to the standard block [AND_2](../../../StandardLibraries/iec61131-3/bitwiseOperators/AND_2.md), `AB_AND_2_UNGATED` uses adapter-based interfaces instead of direct data/event inputs/outputs, enabling more flexible integration into adapter-based system architectures.

- **[`AB_AND_2`](AB_AND_2.md)**: The gated variant – updates the output only on an actual value change.
