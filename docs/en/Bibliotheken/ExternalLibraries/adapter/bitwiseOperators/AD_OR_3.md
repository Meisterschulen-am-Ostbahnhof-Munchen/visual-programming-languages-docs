...
Since this is a combinational logic block, AD_OR_3 has no internal states. The output is recalculated directly from the current input values on every incoming event.
...
Unlike `AX_OR_3`, which combines individual Boolean truth values, `AD_OR_3` operates on the full bit pattern of a `DWORD` value. Compared to the standard block [OR_2](../../../StandardLibraries/iec61131-3/bitwiseOperators/OR_2.md), `AD_OR_3` uses adapter-based interfaces instead of direct data/event inputs/outputs, enabling more flexible integration into adapter-based system architectures.

- **[`AD_OR_3_UNGATED`](AD_OR_3_UNGATED.md)**: Ungated variant – updates the output on every run, even without a value change.
