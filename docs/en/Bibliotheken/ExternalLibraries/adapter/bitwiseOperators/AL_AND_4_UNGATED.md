...
> ℹ️ **UNGATED variant:** This block is the ungated version of [`AL_AND_4`](AL_AND_4.md). It suppresses **no** unchanged repeats – every newly computed result is forwarded unconditionally, even without a value change. This matters for consumers that need a periodic cadence regardless of value change (e.g. derivative/frequency calculations that would otherwise fail to decay toward zero). Any change-detection/gating statements further down this page do **not** apply to this block.
...
**AL_AND_4_UNGATED** is a generic function block for the bitwise AND operation across 4 input values of type `LWORD` (64-bit bit pattern (long word)). Unlike the Boolean operation on single truth values (as in the `AX_AND` blocks), every bit of the data word is combined independently here.
...
- **IN4**: AND input 4 (Type: adapter::types::unidirectional::AL)
...
Since this is a combinational logic block, AL_AND_4_UNGATED has no internal states. The output is recalculated directly from the current input values on every incoming event.
...
Unlike `AX_AND_4`, which combines individual Boolean truth values, `AL_AND_4_UNGATED` operates on the full bit pattern of a `LWORD` value. Compared to the standard block [AND_2](../../../StandardLibraries/iec61131-3/bitwiseOperators/AND_2.md), `AL_AND_4_UNGATED` uses adapter-based interfaces instead of direct data/event inputs/outputs, enabling more flexible integration into adapter-based system architectures.

- **[`AL_AND_4`](AL_AND_4.md)**: The gated variant – updates the output only on an actual value change.
