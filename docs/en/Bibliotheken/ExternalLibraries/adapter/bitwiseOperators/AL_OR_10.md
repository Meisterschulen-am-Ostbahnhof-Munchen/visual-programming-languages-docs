...
Since this is a combinational logic block, AL_OR_10 has no internal states. The output is recalculated directly from the current input values on every incoming event.
...
Unlike `AX_OR_10`, which combines individual Boolean truth values, `AL_OR_10` operates on the full bit pattern of a `LWORD` value. Compared to the standard block [OR_2](../../../StandardLibraries/iec61131-3/bitwiseOperators/OR_2.md), `AL_OR_10` uses adapter-based interfaces instead of direct data/event inputs/outputs, enabling more flexible integration into adapter-based system architectures.

- **[`AL_OR_10_UNGATED`](AL_OR_10_UNGATED.md)**: Ungated variant – updates the output on every run, even without a value change.
