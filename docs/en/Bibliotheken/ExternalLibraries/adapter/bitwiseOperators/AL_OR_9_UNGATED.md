...
> ℹ️ **UNGATED variant:** This block is the ungated version of [`AL_OR_9`](AL_OR_9.md). It suppresses **no** unchanged repeats – every newly computed result is forwarded unconditionally, even without a value change. This matters for consumers that need a periodic cadence regardless of value change (e.g. derivative/frequency calculations that would otherwise fail to decay toward zero). Any change-detection/gating statements further down this page do **not** apply to this block.
...
**AL_OR_9_UNGATED** is a generic function block for the bitwise OR operation across 9 input values of type `LWORD` (64-bit bit pattern (long word)). Unlike the Boolean operation on single truth values (as in the `AX_OR` blocks), every bit of the data word is combined independently here.
...
- **IN7**: OR input 7 (Type: adapter::types::unidirectional::AL)
- **IN8**: OR input 8 (Type: adapter::types::unidirectional::AL)
- **IN9**: OR input 9 (Type: adapter::types::unidirectional::AL)
...
Since this is a combinational logic block, AL_OR_9_UNGATED has no internal states. The output is recalculated directly from the current input values on every incoming event.
...
Unlike `AX_OR_9`, which combines individual Boolean truth values, `AL_OR_9_UNGATED` operates on the full bit pattern of a `LWORD` value. Compared to the standard block [OR_2](../../../StandardLibraries/iec61131-3/bitwiseOperators/OR_2.md), `AL_OR_9_UNGATED` uses adapter-based interfaces instead of direct data/event inputs/outputs, enabling more flexible integration into adapter-based system architectures.

- **[`AL_OR_9`](AL_OR_9.md)**: The gated variant – updates the output only on an actual value change.
