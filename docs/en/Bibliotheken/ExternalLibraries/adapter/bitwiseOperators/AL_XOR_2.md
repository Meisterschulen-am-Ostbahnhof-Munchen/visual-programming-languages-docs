...
![AL_XOR_2](./AL_XOR_2.svg)

* * * * * * * * * *

## Introduction

**AL_XOR_2** is a generic function block for the bitwise XOR (exclusive OR) operation across 2 input values of type `LWORD` (64-bit bit pattern (long word)). Unlike the Boolean operation on single truth values (as in the `AX_XOR` blocks), every bit of the data word is combined independently here.
...
Since this is a combinational logic block, AL_XOR_2 has no internal states. The output is recalculated directly from the current input values on every incoming event.
...
Unlike `AX_XOR_2`, which combines individual Boolean truth values, `AL_XOR_2` operates on the full bit pattern of a `LWORD` value. Compared to the standard block [XOR_2](../../../StandardLibraries/iec61131-3/bitwiseOperators/XOR_2.md), `AL_XOR_2` uses adapter-based interfaces instead of direct data/event inputs/outputs, enabling more flexible integration into adapter-based system architectures.

- **[`AL_XOR_2_UNGATED`](AL_XOR_2_UNGATED.md)**: Ungated variant – updates the output on every run, even without a value change.
