...
Since this is a combinational logic block, AL_XOR_5 has no internal states. The output is recalculated directly from the current input values on every incoming event.
...
Unlike `AX_XOR_5`, which combines individual Boolean truth values, `AL_XOR_5` operates on the full bit pattern of a `LWORD` value. Compared to the standard block [XOR_2](../../../StandardLibraries/iec61131-3/bitwiseOperators/XOR_2.md), `AL_XOR_5` uses adapter-based interfaces instead of direct data/event inputs/outputs, enabling more flexible integration into adapter-based system architectures.

- **[`AL_XOR_5_UNGATED`](AL_XOR_5_UNGATED.md)**: Ungated variant – updates the output on every run, even without a value change.
