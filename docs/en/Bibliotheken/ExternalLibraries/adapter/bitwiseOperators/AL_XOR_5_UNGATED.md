...
> ℹ️ **UNGATED variant:** This block is the ungated version of [`AL_XOR_5`](AL_XOR_5.md). It suppresses **no** unchanged repeats – every newly computed result is forwarded unconditionally, even without a value change. This matters for consumers that need a periodic cadence regardless of value change (e.g. derivative/frequency calculations that would otherwise fail to decay toward zero). Any change-detection/gating statements further down this page do **not** apply to this block.
...
**AL_XOR_5_UNGATED** is a generic function block for the bitwise XOR (exclusive OR) operation across 5 input values of type `LWORD` (64-bit bit pattern (long word)). Unlike the Boolean operation on single truth values (as in the `AX_XOR` blocks), every bit of the data word is combined independently here.
...
Since this is a combinational logic block, AL_XOR_5_UNGATED has no internal states. The output is recalculated directly from the current input values on every incoming event.
...
Unlike `AX_XOR_5`, which combines individual Boolean truth values, `AL_XOR_5_UNGATED` operates on the full bit pattern of a `LWORD` value. Compared to the standard block [XOR_2](../../../StandardLibraries/iec61131-3/bitwiseOperators/XOR_2.md), `AL_XOR_5_UNGATED` uses adapter-based interfaces instead of direct data/event inputs/outputs, enabling more flexible integration into adapter-based system architectures.

- **[`AL_XOR_5`](AL_XOR_5.md)**: The gated variant – updates the output only on an actual value change.
