...
> ℹ️ **UNGATED variant:** This block is the ungated version of [`AQ_AND_4`](AQ_AND_4.md). It suppresses **no** unchanged repeats – every newly computed result is forwarded unconditionally, even without a value change. This matters for consumers that need a periodic cadence regardless of value change (e.g. derivative/frequency calculations that would otherwise fail to decay toward zero). Any change-detection/gating statements further down this page do **not** apply to this block.
...
**AQ_AND_4_UNGATED** is a generic function block for the bitwise AND operation across 4 input values of type `BYTE` (2-bit value (quarter byte, transmitted as a byte)). Unlike the Boolean operation on single truth values (as in the `AX_AND` blocks), every bit of the data word is combined independently here.
...
- **IN1**: AND input 1 (Type: adapter::types::unidirectional::AQ)
- **IN2**: AND input 2 (Type: adapter::types::unidirectional::AQ)
- **IN3**: AND input 3 (Type: adapter::types::unidirectional::AQ)
- **IN4**: AND input 4 (Type: adapter::types::unidirectional::AQ)

**Output Adapter:**

- **OUT**: AND result (Type: adapter::types::unidirectional::AQ)
...
- **Generic function block**: The FB is defined as a generic type (`GEN_AQ_AND`) and covers all arities (2 to 4 inputs) of the same underlying logic via the GenericClassName mechanism.
...
Since this is a combinational logic block, AQ_AND_4_UNGATED has no internal states. The output is recalculated directly from the current input values on every incoming event.
...
Unlike `AX_AND_4`, which combines individual Boolean truth values, `AQ_AND_4_UNGATED` operates on the full bit pattern of a `BYTE` value. Compared to the standard block [AND_2](../../../StandardLibraries/iec61131-3/bitwiseOperators/AND_2.md), `AQ_AND_4_UNGATED` uses adapter-based interfaces instead of direct data/event inputs/outputs, enabling more flexible integration into adapter-based system architectures.

- **[`AQ_AND_4`](AQ_AND_4.md)**: The gated variant – updates the output only on an actual value change.
