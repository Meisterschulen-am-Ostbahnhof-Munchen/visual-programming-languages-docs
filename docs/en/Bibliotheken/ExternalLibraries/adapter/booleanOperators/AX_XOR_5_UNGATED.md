...
The AX_XOR_5_UNGATED is a generic function block for calculating the Boolean XOR operation with five inputs. The block implements the exclusive OR operation for up to five input signals and outputs the result via an adapter output.
...
Since it is a combinational logic block without memory, the AX_XOR_5_UNGATED has no internal states. The output is determined solely by the current input values.
...
Compared to standard XOR blocks with fewer inputs, the AX_XOR_5_UNGATED offers the ability to process up to five signals simultaneously. While simple XOR blocks typically have only two inputs, this function block enables more complex logical operations without additional nesting.

Comparison with [XOR_5](../../../StandardLibraries/iec61131-3/bitwiseOperators/XOR_5.md)

- **[`AX_XOR_5`](AX_XOR_5.md)**: The gated variant – updates the output only on an actual value change.
...
The AX_XOR_5_UNGATED function block provides a flexible solution for multi-input XOR operations. Its adapter-based interface allows for easy integration into existing control systems, while its generic implementation ensures reusability and extensibility.
