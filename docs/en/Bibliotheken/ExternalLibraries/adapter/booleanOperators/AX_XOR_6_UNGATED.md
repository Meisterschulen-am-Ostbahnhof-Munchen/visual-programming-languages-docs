...
The AX_XOR_6_UNGATED is a generic function block for calculating the Boolean XOR operation with six inputs. The block implements the exclusive OR operation for up to six binary input signals and outputs the result via an adapter output.
...
Since this is a purely combinational function block without event control, the AX_XOR_6_UNGATED has no internal states. The output is determined solely by the current input values.
...
Compared to standard XOR function blocks with fewer inputs, the AX_XOR_6_UNGATED offers the ability to process up to six signals simultaneously. While simple XOR blocks typically only have two inputs, this function block enables more complex logical operations without the need for additional nesting of multiple blocks.
...
The AX_XOR_6_UNGATED is a specialized function block for demanding multi-input XOR operations. Its adapter-based interface allows for easy integration into existing control systems and provides an efficient solution for complex Boolean operations without additional wiring complexity.
