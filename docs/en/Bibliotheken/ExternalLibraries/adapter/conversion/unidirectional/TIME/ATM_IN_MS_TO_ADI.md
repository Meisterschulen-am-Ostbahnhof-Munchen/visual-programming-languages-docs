...
The function block internally contains a single function block, `F_TIME_IN_MS_TO_DINT`, from the IEC 61131-3 standard library. Sequence control is purely event-driven:

1. An event at the input adapter **IN.E1** triggers the input **REQ** of the internal conversion block.
2. The data value **IN.D1** (`TIME`) is passed to the input **IN** of `F_TIME_IN_MS_TO_DINT`.
3. `F_TIME_IN_MS_TO_DINT` converts the time value into a milliseconds figure of type `DINT` and signals this via its output **CNF**.
4. This event is forwarded to the output adapter **OUT.E1**.
5. Simultaneously, the converted value (output **OUT** of `F_TIME_IN_MS_TO_DINT`) is passed to the data point **OUT.D1**.
...
- **Unit conversion per IEC 61131-3**: The internal block `F_TIME_IN_MS_TO_DINT` interprets the `TIME` value (internally held in nanoseconds) as a figure in milliseconds and returns the result as `DINT` (32-bit signed integer).
...
- **Displaying/processing durations**: A duration carried via an `ATM` adapter (e.g., a cycle time or delay) needs to be forwarded as a figure in milliseconds to an adapter socket of type `ADI`, for example for display on a panel or further use in a calculation.
...
- **Same unit, other target types**: [ATM_IN_MS_TO_ALI](ATM_IN_MS_TO_ALI.md), [ATM_IN_MS_TO_ALR](ATM_IN_MS_TO_ALR.md), [ATM_IN_MS_TO_AUDI](ATM_IN_MS_TO_AUDI.md), [ATM_IN_MS_TO_AULI](ATM_IN_MS_TO_AULI.md)
- **Same target type, other units**: [ATM_IN_NS_TO_ADI](ATM_IN_NS_TO_ADI.md), [ATM_IN_S_TO_ADI](ATM_IN_S_TO_ADI.md), [ATM_IN_US_TO_ADI](ATM_IN_US_TO_ADI.md)
- **[ATM_TIME_TO_TM](ATM_TIME_TO_TM.md) / [ATM_TM_TO_TIME](ATM_TM_TO_TIME.md)**: pure adapter-to-variable passthrough without unit conversion, unlike the numeric conversion performed by `ATM_IN_MS_TO_ADI`.

## Conclusion

`ATM_IN_MS_TO_ADI` provides a standards-compliant, fully adapter-based conversion of a `TIME` value in milliseconds into a `DINT` value, fitting seamlessly into networks that consistently rely on adapter connections instead of classic data wiring.
