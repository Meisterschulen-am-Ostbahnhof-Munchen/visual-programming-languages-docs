...
> ℹ️ **UNGATED variant:** This block is the ungated version of [`AB_SPLIT_5`](AB_SPLIT_5.md). It suppresses **no** unchanged repeats – every newly computed result is forwarded unconditionally, even without a value change. This matters for consumers that need a periodic cadence regardless of value change (e.g. derivative/frequency calculations that would otherwise fail to decay toward zero). Any change-detection/gating statements further down this page do **not** apply to this block.
...
The function block `AB_SPLIT_5_UNGATED` is used to distribute one unidirectional AB adapter input (socket) to five identical AB adapter outputs (plugs). It is a generic function block designed for signal distribution in automation systems.
...
- **Generic Component**: `AB_SPLIT_5_UNGATED` uses the generic class name `GEN_AB_SPLIT` and enables type-safe instantiation in the 4diac IDE.
...
| `AB_SPLIT_5_UNGATED` | 5 | 1:5 distribution (this block) |
| `AB_SPLIT_N` (generic) | configurable | Individual number via GenericClassName |

Compared to a configurable split block, `AB_SPLIT_5_UNGATED` offers a fixed, yet simple and type-safe solution without additional configuration effort.
...
AB_SPLIT_5_UNGATED` is a lean, specialized function block for the 1:5 distribution of unidirectional AB adapter signals. Thanks to its generic implementation and clear interface structure, it is ideally suited for the modular design of automation systems where a signal is required multiple times.
