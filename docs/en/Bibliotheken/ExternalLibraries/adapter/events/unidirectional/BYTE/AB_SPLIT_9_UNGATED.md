...
> ℹ️ **UNGATED variant:** This block is the ungated version of [`AB_SPLIT_9`](AB_SPLIT_9.md). It suppresses **no** unchanged repeats – every newly computed result is forwarded unconditionally, even without a value change. This matters for consumers that need a periodic cadence regardless of value change (e.g. derivative/frequency calculations that would otherwise fail to decay toward zero). Any change-detection/gating statements further down this page do **not** apply to this block.
...
| ---------- | ---------- | -------------- |
| `AB_SPLIT_9_UNGATED` | 9 | Standard split for 1→9, generic |
| `AB_SPLIT_4` | 4 | Same functionality, fewer outputs |
| `AB_MERGE` | – | Combines multiple inputs into one output (opposite direction) |

While `AB_SPLIT_9_UNGATED` distributes a single source to multiple sinks, `AB_MERGE` combines several sources into a single output. Therefore, `AB_SPLIT_9_UNGATED` is the ideal choice when a signal needs to be routed to many consumers.
...
The **AB_SPLIT_9_UNGATED** offers a simple yet powerful way to split a single adapter-based data stream into nine parallel paths. Its generic nature allows it to be used in any 4diac project, requires no state programming, and enables a clean, modular architecture. It is a fundamental building block for the serial or parallel distribution of adapter signals in automation technology.
