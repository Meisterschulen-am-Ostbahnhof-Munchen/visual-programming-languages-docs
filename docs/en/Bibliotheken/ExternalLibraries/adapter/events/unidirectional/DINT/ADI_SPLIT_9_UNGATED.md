...
The function block `ADI_SPLIT_9_UNGATED` is a generic component that splits an incoming ADI signal (adapter) into nine identical output signals. It functions purely as a splitter at the adapter level.
...
- Unlike data- or event-based splitters, `ADI_SPLIT_9_UNGATED` operates at the higher adapter level, enabling clean architectural separation.
...
ADI_SPLIT_9_UNGATED` is a simple, generic function block for 1-to-9 distribution of a unidirectional ADI signal. It is suitable for applications that require multiple, identical forwarding of adapter signals without additional logic.
