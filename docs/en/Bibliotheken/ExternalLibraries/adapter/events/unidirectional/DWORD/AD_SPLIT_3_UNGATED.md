...
> ℹ️ **UNGATED variant:** This block is the ungated version of [`AD_SPLIT_3`](AD_SPLIT_3.md). It suppresses **no** unchanged repeats – every newly computed result is forwarded unconditionally, even without a value change. This matters for consumers that need a periodic cadence regardless of value change (e.g. derivative/frequency calculations that would otherwise fail to decay toward zero). Any change-detection/gating statements further down this page do **not** apply to this block.
...
The function block `AD_SPLIT_3_UNGATED` serves as a generic splitter for a unidirectional adapter (AD). It receives an incoming AD and forwards it to three separate outputs. This allows multiple uses of the same data or signal path.
...
The `AD_SPLIT_3_UNGATED` does not have its own state diagram (ECC). The outputs follow the input without any internal logic or memory behavior. Therefore, there is only a single, passive state in which the current input is constantly passed on to the outputs.
...
- **AD_SPLIT_2**: Distributes the input to only two outputs. `AD_SPLIT_3_UNGATED` extends this to three outputs.
- **AD_MERGE**: Combines multiple adapter inputs into one output – functionally the opposite.
- **Specific Data Splitters**: Function blocks like `SPLIT_INT` or `SPLIT_BOOL` operate at the data level, not the adapter level. `AD_SPLIT_3_UNGATED` is designed for complete adapter structures (data plus events).
