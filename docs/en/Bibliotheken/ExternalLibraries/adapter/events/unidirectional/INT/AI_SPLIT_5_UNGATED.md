...
> ℹ️ **UNGATED variant:** This block is the ungated version of [`AI_SPLIT_5`](AI_SPLIT_5.md). It suppresses **no** unchanged repeats – every newly computed result is forwarded unconditionally, even without a value change. This matters for consumers that need a periodic cadence regardless of value change (e.g. derivative/frequency calculations that would otherwise fail to decay toward zero). Any change-detection/gating statements further down this page do **not** apply to this block.
...
The function block **AI_SPLIT_5_UNGATED** is used to distribute an incoming analog signal (AI) to five identical outputs. It is a generic function block specifically designed for splitting an analog value into multiple parallel paths. It is implemented using unidirectional adapters of type `AI`.
...
- **AI_SPLIT_2, AI_SPLIT_3, AI_SPLIT_N**: These function blocks split an analog signal to a different number of outputs. AI_SPLIT_5_UNGATED is specifically optimized for five outputs.
- **Analog Splitters in Other Libraries**: Often, such functions are implemented using adapter coupling or data copiers. AI_SPLIT_5_UNGATED offers a clean, reusable structure within the 4diac framework.
...
**AI_SPLIT_5_UNGATED is not a data-event-based splitter.**
...
The AI_SPLIT_5_UNGATED is a simple yet useful function block for duplicating an analog signal. Its generic nature and clear adapter interface make it an ideal building block for modular automation solutions where an analog value is needed multiple times. The lack of event-driven control reduces complexity and enables direct signal transmission without time delays.
