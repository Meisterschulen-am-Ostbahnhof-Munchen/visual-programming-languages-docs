...
> ℹ️ **UNGATED variant:** This block is the ungated version of [`ALI_SPLIT_2`](ALI_SPLIT_2.md). It suppresses **no** unchanged repeats – every newly computed result is forwarded unconditionally, even without a value change. This matters for consumers that need a periodic cadence regardless of value change (e.g. derivative/frequency calculations that would otherwise fail to decay toward zero). Any change-detection/gating statements further down this page do **not** apply to this block.
...
The **ALI_SPLIT_2_UNGATED** function block serves as a generic distributor for a unidirectional ALI (Application Layer Interface) adapter. It copies all incoming data and events from a single input socket to two output plugs. This allows a signal or data source to be passed on to two independent receivers.
...
**ALI_SPLIT_2_UNGATED** is a simple but essential function block for distributing unidirectional ALI interfaces. Its generic nature and lean implementation make it a universal tool in IEC 61499 component development when a signal or data stream needs to be passed to multiple subsequent blocks.
