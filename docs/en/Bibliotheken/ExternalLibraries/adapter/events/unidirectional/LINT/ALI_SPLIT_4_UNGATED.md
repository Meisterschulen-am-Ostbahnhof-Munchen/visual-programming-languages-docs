...
> ℹ️ **UNGATED variant:** This block is the ungated version of [`ALI_SPLIT_4`](ALI_SPLIT_4.md). It suppresses **no** unchanged repeats – every newly computed result is forwarded unconditionally, even without a value change. This matters for consumers that need a periodic cadence regardless of value change (e.g. derivative/frequency calculations that would otherwise fail to decay toward zero). Any change-detection/gating statements further down this page do **not** apply to this block.
...
The function block **ALI_SPLIT_4_UNGATED** is used to distribute a single ALI adapter input (type: `adapter::types::unidirectional::ALI`) to four separate ALI adapter outputs. It is designed as a generic function block (FB) and enables unidirectional signal transmission from one source to up to four destinations.
...
- **ALI_SPLIT_2**: Distributes the input signal to two outputs instead of four. The ALI_SPLIT_4_UNGATED offers twice the number of output channels.
...
The **ALI_SPLIT_4_UNGATED** is a simple yet useful function block for passively distributing a unidirectional ALI signal to four identical outputs. Its generic design and the absence of event or data logic make it particularly suitable for rapid signal multiplication in automation technology without incurring additional processing load.
