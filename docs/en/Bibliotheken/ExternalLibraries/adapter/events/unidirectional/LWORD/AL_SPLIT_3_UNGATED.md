...
> ℹ️ **UNGATED variant:** This block is the ungated version of [`AL_SPLIT_3`](AL_SPLIT_3.md). It suppresses **no** unchanged repeats – every newly computed result is forwarded unconditionally, even without a value change. This matters for consumers that need a periodic cadence regardless of value change (e.g. derivative/frequency calculations that would otherwise fail to decay toward zero). Any change-detection/gating statements further down this page do **not** apply to this block.
...
The function block **AL_SPLIT_3_UNGATED** is used to split an incoming unidirectional adapter signal (AL – Adapter Label) into three identical output adapters. It is designed as a generic building block and enables the multiple forwarding of an adapter-based data or event flow within an IEC 61499 application.
...
- **Event-Based Splitters (e.g., E_SPLIT):** These split events, while AL_SPLIT_3_UNGATED distributes adapter objects (complex data structures). Event splitters often require handling of event sequences.
