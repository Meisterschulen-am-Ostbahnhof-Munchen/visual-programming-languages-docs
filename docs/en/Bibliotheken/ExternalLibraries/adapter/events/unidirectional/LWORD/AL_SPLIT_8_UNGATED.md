...
> ℹ️ **UNGATED variant:** This block is the ungated version of [`AL_SPLIT_8`](AL_SPLIT_8.md). It suppresses **no** unchanged repeats – every newly computed result is forwarded unconditionally, even without a value change. This matters for consumers that need a periodic cadence regardless of value change (e.g. derivative/frequency calculations that would otherwise fail to decay toward zero). Any change-detection/gating statements further down this page do **not** apply to this block.
...
The function block `AL_SPLIT_8_UNGATED` distributes an incoming unidirectional AL signal (socket `IN`) to eight identical outputs (plugs `OUT1` to `OUT8`). It is declared as a generic block and serves for simple signal multiplication in 4diac applications.
...
- **Standard SPLIT function blocks** (e.g., `SPLIT` for simple data types) distribute individual values, while `AL_SPLIT_8_UNGATED` is specifically designed for the unidirectional adapter `AL`.
- **Adapter mergers** (such as a hypothetical `AL_MERGE`) combine multiple signals into one; `AL_SPLIT_8_UNGATED` implements the reverse 1:n functionality.
...
The `AL_SPLIT_8_UNGATED` is a simple yet important function block for multiplying AL adapter signals. Its passive, stateless operation allows it to integrate seamlessly into data-flow-oriented 4diac applications and facilitates the structured distribution of alarm or control signals.
