...
> ℹ️ **UNGATED variant:** This block is the ungated version of [`AR_SPLIT_4`](AR_SPLIT_4.md). It suppresses **no** unchanged repeats – every newly computed result is forwarded unconditionally, even without a value change. This matters for consumers that need a periodic cadence regardless of value change (e.g. derivative/frequency calculations that would otherwise fail to decay toward zero). Any change-detection/gating statements further down this page do **not** apply to this block.
...
The function block **AR_SPLIT_4_UNGATED** is used to split one incoming AR adapter (unidirectional) into four outgoing AR adapters. It is implemented as a generic function block (`GenericClassName = 'GEN_AR_SPLIT'`) and enables simple signal multiplication for adapter-based communication in control systems.
...
- **AR_MERGE**: Merges multiple AR signals into one – the inverse function of AR_SPLIT_4_UNGATED.

Compared to these alternatives, AR_SPLIT_4_UNGATED offers a fixed number of four outputs, enabling a compact and clear implementation without configuration overhead.
