...
> ℹ️ **UNGATED variant:** This block is the ungated version of [`AS_SPLIT_3`](AS_SPLIT_3.md). It suppresses **no** unchanged repeats – every newly computed result is forwarded unconditionally, even without a value change. This matters for consumers that need a periodic cadence regardless of value change (e.g. derivative/frequency calculations that would otherwise fail to decay toward zero). Any change-detection/gating statements further down this page do **not** apply to this block.
...
The AS_SPLIT_3_UNGATED function block distributes an incoming adapter data stream to three identical outputs. It is implemented as a generic function block and designed for use with the `adapter::types::unidirectional::AS` adapter type.
...
- **AS_SPLIT_2**: An analog function block with only two outputs. AS_SPLIT_3_UNGATED extends the number to three outputs.
- **AS_MERGE (Theoretical)**: Unlike a merge function block, which combines multiple inputs into one output, AS_SPLIT_3_UNGATED performs the reverse function (1 → N).
- **Generic Split Function Blocks**: Similar concepts exist for data inputs (e.g., SPLIT_INT), but these work with elementary data, while AS_SPLIT_3_UNGATED processes adapters (complex data types).
...
AS_SPLIT_3_UNGATED is a simple yet useful function block for duplicating adapter data in the 4diac IDE. Its generic design and the absence of event traffic make it particularly lightweight and usable in many scenarios. The documented license (EPL 2.0) ensures that the block can be legally integrated into your own projects.
