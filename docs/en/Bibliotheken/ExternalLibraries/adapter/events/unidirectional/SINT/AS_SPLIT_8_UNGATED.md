...
> ℹ️ **UNGATED variant:** This block is the ungated version of [`AS_SPLIT_8`](AS_SPLIT_8.md). It suppresses **no** unchanged repeats – every newly computed result is forwarded unconditionally, even without a value change. This matters for consumers that need a periodic cadence regardless of value change (e.g. derivative/frequency calculations that would otherwise fail to decay toward zero). Any change-detection/gating statements further down this page do **not** apply to this block.
...
The function block **AS_SPLIT_8_UNGATED** is used to split an incoming *Application Specific* (AS) adapter data stream into eight identical outputs. It is provided as a generic function block (generic FB) and is specifically designed for distributing adapter data within an IEC 61499-based control application.
...
- **AS_SPLIT_2 / AS_SPLIT_4**: Analog function blocks with fewer outputs. AS_SPLIT_8_UNGATED offers maximum distribution in one step with eight outputs.
...
- **Data-based splitters (e.g., ANY_DISTRIBUTE)**: Split data values but require additional events. AS_SPLIT_8_UNGATED is optimized for simple adapter forwarding.
