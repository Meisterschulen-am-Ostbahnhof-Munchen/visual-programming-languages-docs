...
> ℹ️ **UNGATED variant:** This block is the ungated version of [`AIS_SPLIT_2`](AIS_SPLIT_2.md). It suppresses **no** unchanged repeats – every newly computed result is forwarded unconditionally, even without a value change. This matters for consumers that need a periodic cadence regardless of value change (e.g. derivative/frequency calculations that would otherwise fail to decay toward zero). Any change-detection/gating statements further down this page do **not** apply to this block.
...
The function block **AIS_SPLIT_2_UNGATED** is a generic adapter module that splits an incoming unidirectional AIS adapter (Application Interface Socket) into two identical outgoing AIS adapters. It serves as a passive splitter without its own logic or event processing.
...
The AIS_SPLIT_2_UNGATED does not have its own state machine. The operating state is determined exclusively by the connected AIS adapters.
...
- **Event/Data Splitter**: Other splitters work with simple events or data (e.g., F_SPLIT), while AIS_SPLIT_2_UNGATED splits complete adapter interfaces.
...
The AIS_SPLIT_2_UNGATED is a minimalist, generically applicable adapter splitter that enables the flexible branching of AIS interfaces in 4diac projects. By configuring it via the GenericClassName attribute, it can be used for different AIS types without modifying the source code.
