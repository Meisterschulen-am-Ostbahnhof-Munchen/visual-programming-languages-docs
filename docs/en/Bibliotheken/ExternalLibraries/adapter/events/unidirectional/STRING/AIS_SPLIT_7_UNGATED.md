...
> ℹ️ **UNGATED variant:** This block is the ungated version of [`AIS_SPLIT_7`](AIS_SPLIT_7.md). It suppresses **no** unchanged repeats – every newly computed result is forwarded unconditionally, even without a value change. This matters for consumers that need a periodic cadence regardless of value change (e.g. derivative/frequency calculations that would otherwise fail to decay toward zero). Any change-detection/gating statements further down this page do **not** apply to this block.
...
- **Data Splitters (e.g., SPLIT_1_to_N)** – These typically split simple data types (BOOL, INT, REAL). The AIS_SPLIT_7_UNGATED is specifically designed for adapter interfaces and utilizes the adapter concept defined in IEC 61499 for complex, structured data streams.
- **Adapter Multiplexers (e.g., AIS_MERGE)** – While a splitter distributes one input to multiple outputs, a merger combines multiple inputs into one output. The AIS_SPLIT_7_UNGATED is the counterpart to this.
- **Event Splitters** – These differ in their event processing capabilities; the AIS_SPLIT_7_UNGATED, however, operates purely on an adapter basis without separate event channels.
