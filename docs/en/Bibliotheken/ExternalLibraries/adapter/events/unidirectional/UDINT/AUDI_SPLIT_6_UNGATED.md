...
> ℹ️ **UNGATED variant:** This block is the ungated version of [`AUDI_SPLIT_6`](AUDI_SPLIT_6.md). It suppresses **no** unchanged repeats – every newly computed result is forwarded unconditionally, even without a value change. This matters for consumers that need a periodic cadence regardless of value change (e.g. derivative/frequency calculations that would otherwise fail to decay toward zero). Any change-detection/gating statements further down this page do **not** apply to this block.
...
Other split function blocks exist (e.g., `AUDI_SPLIT_2`, `AUDI_SPLIT_4`) that differ only in the number of outputs. With six outputs, `AUDI_SPLIT_6_UNGATED` offers a higher distribution density. Unlike pure data splitters for basic data types, this FB works exclusively with AUDI adapters and supports unidirectional coupling.
...
AUDI_SPLIT_6_UNGATED` is a simple yet flexible function block for multiplying adapter connections in a 4diac environment. Its generic nature and lean implementation without event overhead make it ideal for scenarios where an incoming data stream needs to be distributed to multiple independent receivers.
