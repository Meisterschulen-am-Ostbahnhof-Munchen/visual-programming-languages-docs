...
## State Overview
The AUI_SPLIT_2 function block has **no state machine**. There are no internal states or transitions – the signal is continuously routed without clocking.
...
- **AUI_SPLIT_N**: An extended variant that splits one input into N outputs (e.g., 1:3, 1:4). The AUI_SPLIT_2 is the simplest member of this family.
