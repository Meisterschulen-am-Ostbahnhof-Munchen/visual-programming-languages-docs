...
2. **Hysteresis:** The new output value is only adopted if the difference between the current input `D1` and the last valid output exceeds the hysteresis value.
3. **Minimum Time:** After each adoption (output of an event on `Q.E1`), no further adoption is permitted for the duration `Tmin`.
4. **Output:** On a valid edge, the new value is output via `Q.D1`, and an event is generated on `Q.E1`.
