...
> ℹ️ **UNGATED variant:** This block is the ungated version of [`AUS_SPLIT_6`](AUS_SPLIT_6.md). It suppresses **no** unchanged repeats – every newly computed result is forwarded unconditionally, even without a value change. This matters for consumers that need a periodic cadence regardless of value change (e.g. derivative/frequency calculations that would otherwise fail to decay toward zero). Any change-detection/gating statements further down this page do **not** apply to this block.
...
- **Event Splitter (e.g., `E_SPLIT`)**: Processes discrete events at its own input/output events. In contrast, `AUS_SPLIT_6_UNGATED` operates at the adapter level and distributes both the event and its associated data in one step.
