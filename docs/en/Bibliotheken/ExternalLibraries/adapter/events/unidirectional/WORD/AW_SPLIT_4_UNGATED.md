...
The function block `AW_SPLIT_4_UNGATED` distributes an incoming AW adapter (unidirectional) to four identical AW output adapters. The block is designed generically and can be used with different AW types.
...
Unlike data- or event-based split function blocks (e.g., `SPLIT`, `F_SPLIT`), `AW_SPLIT_4_UNGATED` operates exclusively at the adapter level. The advantage lies in its loose coupling and reusability across different AW types. The disadvantage: Only AW data can be distributed, not events or general values.
...
The `AW_SPLIT_4_UNGATED` function block is a simple, generic distributor for unidirectional AW adapters. It is ideally suited to split a single AW signal into up to four outputs and, thanks to its generic nature, integrates flexibly into various IEC 61499 applications.
