...
The function block `AW_SPLIT_7_UNGATED` is used to distribute a single incoming AW adapter (unidirectional) to seven identical AW adapter outputs. It is designed as a generic function block and enables the simple distribution of adapter connections without data processing.
...
The `AW_SPLIT_7_UNGATED` function block does not have an internal state machine. Its behavior is deterministic and consists solely of passing through the adapter. A state machine is therefore not required.
...
- **Adapter Multiplexers**: Unlike multiplexers that select between multiple inputs, the `AW_SPLIT_7_UNGATED` distributes a single input without selection.
