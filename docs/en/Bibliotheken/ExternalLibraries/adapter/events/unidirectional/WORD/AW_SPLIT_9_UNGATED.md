...
> ℹ️ **UNGATED variant:** This block is the ungated version of [`AW_SPLIT_9`](AW_SPLIT_9.md). It suppresses **no** unchanged repeats – every newly computed result is forwarded unconditionally, even without a value change. This matters for consumers that need a periodic cadence regardless of value change (e.g. derivative/frequency calculations that would otherwise fail to decay toward zero). Any change-detection/gating statements further down this page do **not** apply to this block.
...
- **AW_SPLIT_N**: A generic split function block with a variable number of outputs (e.g., `AW_SPLIT_4`, `AW_SPLIT_16`). `AW_SPLIT_9_UNGATED` is a special implementation for exactly nine outputs.
- **Data Split Blocks**: Blocks that split data inputs (e.g., `INT`, `REAL`). `AW_SPLIT_9_UNGATED` operates exclusively at the adapter level and not at individual data points.
