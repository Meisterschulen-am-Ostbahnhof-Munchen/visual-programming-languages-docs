...
> ℹ️ **UNGATED variant:** This block is the ungated version of [`AIWS_SPLIT_3`](AIWS_SPLIT_3.md). It suppresses **no** unchanged repeats – every newly computed result is forwarded unconditionally, even without a value change. This matters for consumers that need a periodic cadence regardless of value change (e.g. derivative/frequency calculations that would otherwise fail to decay toward zero). Any change-detection/gating statements further down this page do **not** apply to this block.
...
- **ALL_SPLIT (for data):** A generic function block for splitting data inputs/outputs, not specific to adapters. AIWS_SPLIT_3_UNGATED, on the other hand, is specifically optimized for the AIWS adapter type and operates at the adapter level.
...
The **AIWS_SPLIT_3_UNGATED** is a simple yet effective generic function block for duplicating an AIWS adapter signal to three parallel outputs. Thanks to its generic nature, it can be flexibly used in various automation and control applications where a data stream is required multiple times.
