...
> ℹ️ **UNGATED variant:** This block is the ungated version of [`AIWS_SPLIT_7`](AIWS_SPLIT_7.md). It suppresses **no** unchanged repeats – every newly computed result is forwarded unconditionally, even without a value change. This matters for consumers that need a periodic cadence regardless of value change (e.g. derivative/frequency calculations that would otherwise fail to decay toward zero). Any change-detection/gating statements further down this page do **not** apply to this block.
...
The function block **AIWS_SPLIT_7_UNGATED** is a generic component that splits a single incoming AIWS adapter (unidirectional) into seven identical outgoing AIWS adapters. It serves as a signal distributor for automation applications where a data stream or connection needs to be copied to multiple parallel paths.
...
- **AIWS_SPLIT_2, AIWS_SPLIT_4** – splitters with the same functionality but a different number of outputs (2 or 4). AIWS_SPLIT_7_UNGATED offers the maximum number of outputs within this splitter family.
- **Generic 1:N Splitters** – Other IEC 61499 libraries often contain generic splitters for data types such as `ANY`. AIWS_SPLIT_7_UNGATED is specifically designed for the adapter type `AIWS` and therefore requires no type conversion.
...
The **AIWS_SPLIT_7_UNGATED** is a simple yet powerful function block for multiplying a unidirectional AIWS data stream to seven parallel outputs. Its generic definition and purely adapter-based interface enable flexible integration into IEC 61499 applications, particularly for signal distribution and parallel processing tasks.
