...
> ℹ️ **UNGATED variant:** This block is the ungated version of [`ADI_ADD_2`](ADI_ADD_2.md). It suppresses **no** unchanged repeats – every newly computed result is forwarded unconditionally, even without a value change. This matters for consumers that need a periodic cadence regardless of value change (e.g. derivative/frequency calculations that would otherwise fail to decay toward zero). Any change-detection/gating statements further down this page do **not** apply to this block.
...
The function block `ADI_ADD_2_UNGATED` performs the arithmetic operation:
...
Compared to a classic `ADD` component according to IEC 61499 (which typically has dedicated `REQ` and `CNF` event ports, as well as direct data inputs such as `IN1` and `IN2` as `ANY_NUM`), the `ADI_ADD_2_UNGATED` encapsulates these interfaces in adapters.

- **Standard ADD:** Requires manual wiring of a minimum of 2 events and 3 data lines (5 connections in total).
- **ADI_ADD_2_UNGATED:** Only requires connecting the three adapter lines (`IN1`, `IN2`, `OUT`), which significantly improves the readability of complex control diagrams.
...
The `ADI_ADD_2_UNGATED` is a highly efficient, clear, and modern component for arithmetic addition in the 4diac IDE. It is ideally suited for demanding architectures where clarity through the use of standardized adapters is paramount.
