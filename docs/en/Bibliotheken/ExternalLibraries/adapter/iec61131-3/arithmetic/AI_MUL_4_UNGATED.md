...
> ℹ️ **UNGATED variant:** This block is the ungated version of [`AI_MUL_4`](AI_MUL_4.md). It suppresses **no** unchanged repeats – every newly computed result is forwarded unconditionally, even without a value change. This matters for consumers that need a periodic cadence regardless of value change (e.g. derivative/frequency calculations that would otherwise fail to decay toward zero). Any change-detection/gating statements further down this page do **not** apply to this block.
...
Since `AI_MUL_4_UNGATED` is a purely mathematical, data- and event-driven function block, it does not have an internal state machine (ECC). Its execution is purely stateless:
...
- **Standard MUL (IEC 61131-3):** Classic multiplication function blocks typically only work with elementary data types (e.g., `REAL`, `INT`) and require separate event lines (`REQ` / `CNF`). Furthermore, they often only support two inputs by default. `AI_MUL_4_UNGATED`, on the other hand, directly processes four inputs and uses adapters for encapsulation, resulting in a clearer network layout.
- **AI_MUL_2:** A similar adapter-based function block, but only for two inputs. `AI_MUL_4_UNGATED` saves additional wiring effort and function block instances for more complex calculations with up to four factors.
...
The `AI_MUL_4_UNGATED` function block offers an efficient and clean way to implement complex multiplication tasks with up to four analog signals in 4diac-IDE. The consistent use of unidirectional adapters minimizes cabling effort and significantly improves the readability of the application design.
