...
> ℹ️ **UNGATED variant:** This block is the ungated version of [`AI_SUB_2`](AI_SUB_2.md). It suppresses **no** unchanged repeats – every newly computed result is forwarded unconditionally, even without a value change. This matters for consumers that need a periodic cadence regardless of value change (e.g. derivative/frequency calculations that would otherwise fail to decay toward zero). Any change-detection/gating statements further down this page do **not** apply to this block.
...
Compared to standard subtraction components of IEC 61131-3 (such as a classic `SUB` component with discrete pins), the `AI_SUB_2_UNGATED` eliminates the need for manually wiring separate `REQ` and `CNF` event lines as well as individual analog data lines. All relevant information is cleanly transmitted in the `AI` adapters.
...
The `AI_SUB_2_UNGATED` is a specialized and highly efficient component for modern, adapter-based programming in 4diac. It is ideally suited for clean software architectures in the field of analog signal preprocessing, where clarity and encapsulation are paramount.
