...
## Functionality
The `ALR_ADD_2` reads the values via the two input adapters `IN1` and `IN2`. As soon as a corresponding event signals via the adapters that new data is available, the function block performs the addition:
...
Compared to a standard `ADD` function block (which has separate event inputs like `REQ`, event outputs like `CNF`, and classic data pins), the `ALR_ADD_2` completely omits these classic pins. It replaces them with standardized `ALR` adapters. This significantly simplifies wiring in the 4diac IDE, as event and data are bundled in a single connection.
