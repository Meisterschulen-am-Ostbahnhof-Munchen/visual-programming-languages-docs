...
The function block **AR_ADD_4_UNGATED** performs the summation of the four values applied to the input adapters and outputs the result to the output adapter **OUT**. The calculation is performed according to the general formula:
...
| ---------- | ---------------- | -------------- |
| **AR_ADD_4_UNGATED** | 4 | Adapter-based, generic, no events |
...
The **AR_ADD_4_UNGATED** function block stands out from classic IEC 61499 arithmetic function blocks due to its pure adapter communication and generic design. It is particularly suitable for systems that already rely on adapter technology and do not require explicit event control.
