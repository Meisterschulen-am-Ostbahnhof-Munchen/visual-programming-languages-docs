...
> ℹ️ **UNGATED variant:** This block is the ungated version of [`AR_MUL_2`](AR_MUL_2.md). It suppresses **no** unchanged repeats – every newly computed result is forwarded unconditionally, even without a value change. This matters for consumers that need a periodic cadence regardless of value change (e.g. derivative/frequency calculations that would otherwise fail to decay toward zero). Any change-detection/gating statements further down this page do **not** apply to this block.
...
Compared to standard multiplication components (such as the classic `MUL` component according to IEC 61131-3), which work with elementary data types (e.g., `INT`, `REAL`), `AR_MUL_2_UNGATED` uses structured **adapters**. This offers the following advantages:
...
The `AR_MUL_2_UNGATED` function block is an efficient tool for arithmetic operations in modern, adapter-based control programs. Its encapsulation in unidirectional interfaces offers a high degree of reusability and ensures a clean and structured design within the 4diac environment.
