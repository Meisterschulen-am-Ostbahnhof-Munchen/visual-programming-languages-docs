...
> ℹ️ **UNGATED variant:** This block is the ungated version of [`AS_ADD_2`](AS_ADD_2.md). It suppresses **no** unchanged repeats – every newly computed result is forwarded unconditionally, even without a value change. This matters for consumers that need a periodic cadence regardless of value change (e.g. derivative/frequency calculations that would otherwise fail to decay toward zero). Any change-detection/gating statements further down this page do **not** apply to this block.
...
The function block `AS_ADD_2_UNGATED` performs the mathematical operation:
...
As a purely mathematical combination block, `AS_ADD_2_UNGATED` does not have a complex internal state diagram (ECC). Its behavior can be divided into three cyclical steps:
...
- **Cascaded Calculations:** Easy expansion for more than two summands by cascading multiple `AS_ADD_2_UNGATED` blocks.
- **Standard ADD (e.g., F_ADD):** The classic IEC 61131-3 or IEC 61499 ADD block works with discrete variables (e.g., `ANY_NUM`) and separate event ports (`REQ` / `CNF`). `AS_ADD_2_UNGATED`, on the other hand, bundles these signals in adapters, which simplifies wiring but requires the use of the specific adapter type `AS`.
...
The `AS_ADD_2_UNGATED` is a specialized auxiliary function block for modern, modular IEC 61499 control programs. Through the consistent use of adapters, it integrates seamlessly into service-oriented architectures and minimizes design and wiring effort in the 4diac IDE.
...
The `AS_ADD_2_UNGATED` is a clean, adapter-based addition block that keeps a two-input arithmetic operation consistent with the rest of the adapter-based `AS` family, making it easy to wire and reuse across IEC 61499 applications.
