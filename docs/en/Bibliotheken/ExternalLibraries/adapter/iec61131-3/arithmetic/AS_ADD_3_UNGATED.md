...
> ℹ️ **UNGATED variant:** This block is the ungated version of [`AS_ADD_3`](AS_ADD_3.md). It suppresses **no** unchanged repeats – every newly computed result is forwarded unconditionally, even without a value change. This matters for consumers that need a periodic cadence regardless of value change (e.g. derivative/frequency calculations that would otherwise fail to decay toward zero). Any change-detection/gating statements further down this page do **not** apply to this block.
...
The function block `AS_ADD_3_UNGATED` performs a classic addition:
...
- **Standard ADD Components (IEC 61131-3):** Classic ADD components use dedicated data pins (e.g., `IN1`, `IN2`) and separate event pins (`REQ`, `CNF`). `AS_ADD_3_UNGATED` minimizes the wiring effort in the FBD (Function Block Diagram) by using adapters.
- **AS_ADD_2 (Two-Way Adder):** Compared to a variant with only two inputs, the `AS_ADD_3_UNGATED` saves an entire component and the associated intermediate wiring when adding three values.
...
The `AS_ADD_3_UNGATED` is a highly efficient, modular component for adding three values. Thanks to the consistent use of unidirectional adapters, it integrates perfectly into modern, service-oriented, and well-structured IEC 61499 control applications.
