...
> ℹ️ **UNGATED variant:** This block is the ungated version of [`ATM_ADD_4`](ATM_ADD_4.md). It suppresses **no** unchanged repeats – every newly computed result is forwarded unconditionally, even without a value change. This matters for consumers that need a periodic cadence regardless of value change (e.g. derivative/frequency calculations that would otherwise fail to decay toward zero). Any change-detection/gating statements further down this page do **not** apply to this block.
...
The function block **ATM_ADD_4_UNGATED** is used to calculate the arithmetic addition of two time values (type `TIME`). It is designed as a generic function block and implements the basic arithmetic operation via adapter interfaces. The block is platform-independent and complies with the IEC 61499 standard.
...
- **Generic Structure** – The function block uses a generic class name (`GEN_ATM_ADD`) at runtime, defined by the attribute `eclipse4diac::core::GenericClassName`. The same class covers the arities `ATM_ADD_4_UNGATED`, `ATM_ADD_3`, and `ATM_ADD_4_UNGATED` via the GenericClassName mechanism.
...
Compared to `AR_ADD_2` (addition of two `REAL` values via adapters), `ATM_ADD_4_UNGATED` is specialized for the `TIME` data type. Unlike `AR_MULTIME`/`ATM_AR_MULTIME`, which multiply a time value by a numeric factor, `ATM_ADD_4_UNGATED` adds two values of the same kind.
...
`ATM_ADD_4_UNGATED` is a compact, generic function block for adding two time values using adapter interfaces. It is particularly suitable for applications where multiple durations need to be combined into a total duration.
