...
> ℹ️ **UNGATED variant:** This block is the ungated version of [`AUI_ADD_2`](AUI_ADD_2.md). It suppresses **no** unchanged repeats – every newly computed result is forwarded unconditionally, even without a value change. This matters for consumers that need a periodic cadence regardless of value change (e.g. derivative/frequency calculations that would otherwise fail to decay toward zero). Any change-detection/gating statements further down this page do **not** apply to this block.
...
The function block `AUI_ADD_2_UNGATED` is a generic function block (Generic FB) designed to perform arithmetic addition. It uses unidirectional adapters of type `AUI` (Adapter Unidirectional Interface) to add two input values and output the result. Because it is a generic block based on the class `GEN_AUI_ADD`, it can be flexibly used for various data types supported by the corresponding adapters.
...
The `AUI_ADD_2_UNGATED` is a highly efficient, modular component for mathematical addition in modern IEC 61499 systems. Through the consistent use of adapters, he promotes a clear, error-resistant and easily maintainable application design in the 4diac IDE.
