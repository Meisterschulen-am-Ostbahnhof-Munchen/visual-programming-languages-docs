...
> ℹ️ **UNGATED variant:** This block is the ungated version of [`AUI_ADD_3`](AUI_ADD_3.md). It suppresses **no** unchanged repeats – every newly computed result is forwarded unconditionally, even without a value change. This matters for consumers that need a periodic cadence regardless of value change (e.g. derivative/frequency calculations that would otherwise fail to decay toward zero). Any change-detection/gating statements further down this page do **not** apply to this block.
...
- **Standard ADD Block (IEC 61131-3):** A classic `ADD` block has discrete inputs for data and events. `AUI_ADD_3_UNGATED` logically groups these in adapters, which increases reusability and clarity.
- **AUI_ADD_2 (Dual Adder):** While a dual adder would require cascading two function blocks to sum three values, `AUI_ADD_3_UNGATED` accomplishes this in a single step. This saves resources and reduces system latency.

---

- **[`AUI_ADD_3`](AUI_ADD_3.md)**: The gated variant – updates the output only on an actual value change.
...
The `AUI_ADD_3_UNGATED` is an efficient auxiliary function block for arithmetic operations in modern IEC 61499 control applications. Its consistent use of the adapter concept allows it to integrate seamlessly into service-oriented and modular software architectures, contributing to reduced complexity in graphical programming.
