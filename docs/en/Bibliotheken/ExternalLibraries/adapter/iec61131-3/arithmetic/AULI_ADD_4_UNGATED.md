...
> ℹ️ **UNGATED variant:** This block is the ungated version of [`AULI_ADD_4`](AULI_ADD_4.md). It suppresses **no** unchanged repeats – every newly computed result is forwarded unconditionally, even without a value change. This matters for consumers that need a periodic cadence regardless of value change (e.g. derivative/frequency calculations that would otherwise fail to decay toward zero). Any change-detection/gating statements further down this page do **not** apply to this block.
...
- **Standard ADD Function Blocks (e.g., `ADD` from IEC 61131-3):** These use direct data and event ports (such as `REQ`, `CNF`, `IN1`, `IN2`). `AULI_ADD_4_UNGATED`, on the other hand, completely encapsulates these interfaces in adapters, resulting in a cleaner control flow diagram.
- **AULI_ADD_2:** A similar function block, but with only two inputs. `AULI_ADD_4_UNGATED` eliminates the need to cascade multiple individual addition function blocks when adding four values.
