...
> ℹ️ **UNGATED variant:** This block is the ungated version of [`AULI_MUL_2`](AULI_MUL_2.md). It suppresses **no** unchanged repeats – every newly computed result is forwarded unconditionally, even without a value change. This matters for consumers that need a periodic cadence regardless of value change (e.g. derivative/frequency calculations that would otherwise fail to decay toward zero). Any change-detection/gating statements further down this page do **not** apply to this block.
...
The function block `AULI_MUL_2_UNGATED` is used to perform an arithmetic multiplication of two values. It is a generic function block (`GEN_AULI_MUL`) whose interfaces are implemented entirely via adapters of type `AULI` (unidirectional). This allows for clean encapsulation of data and events and reduces the wiring effort within the 4diac IDE.
...
The `AULI_MUL_2_UNGATED` is a specialized and modern variant of a multiplication function block for IEC 61499. Due to the consistent use of adapters, it is ideally suited for demanding, modular applications where clarity and standardization are paramount.
