...
The function block `AUS_MUL_4_UNGATED` is a generic function block (Generic FB) for performing an arithmetic multiplication (`MUL`) with up to four input values. It is based on the IEC 61499 architecture and uses unidirectional adapters to transmit data and the associated control events in a clean, encapsulated manner.
...
Compared to standard multiplication components (such as the classic `MUL` of IEC 61131-3), which use separate pins for events and data, the `AUS_MUL_4_UNGATED` offers the following advantages:
...
The `AUS_MUL_4_UNGATED` is a highly efficient and modern function block for the 4diac IDE. It combines the fundamental mathematical function of multiplication with the modern structuring advantages of IEC 61499 adapters, resulting in cleaner, more maintainable, and more transparent control applications.
