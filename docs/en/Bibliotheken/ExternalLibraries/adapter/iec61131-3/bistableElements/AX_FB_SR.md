...
2. If **SET1** is `FALSE` and **RESET** is `TRUE`, the output **Q1** is set to `FALSE`.

3. If both inputs are `FALSE`, **Q1** retains its previous state (memory function).

After the new state is calculated, an event is triggered at the output adapter (`Q1.E1`) to inform subsequent function blocks about the update.
