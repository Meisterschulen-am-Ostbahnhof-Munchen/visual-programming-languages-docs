...
- **Standard compliance**: The function block implements the comparison function from IEC 61131-3 ("equal to") for the `DWORD` data type.
...
- **Control Logic**: Triggering an action once two values of type `DWORD` satisfy the "equal to" condition.
- **Data Filtering**: Selecting data records or signals that satisfy a `DWORD`-based comparison condition.

## Comparison with Similar Function Blocks

- **AD_NE**: Checks for "not equal to" – the inverse function of AD_EQ.
- **AD_NE**: Checks for inequality of values.

AD_EQ is part of the complete AD comparison family (EQ/NE/GT/GE/LT/LE) for the `DWORD` data type.

## Conclusion

AD_EQ is a simple yet important comparison block for IEC 61131-3 control applications using the `DWORD` data type. Its adapter-oriented interface makes it versatile and allows for a clear separation of event and data paths. Due to its event-driven operation, it is particularly suitable for real-time systems where conditions must react immediately to changes in values.
