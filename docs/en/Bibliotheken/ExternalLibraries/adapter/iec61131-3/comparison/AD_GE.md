...
The adapters `AD` and `AX` are unidirectional interfaces, each providing one event channel and one data channel.
...
- **Adapter-based interface**: The function block uses only adapters (`AD`/`AX`), allowing for flexible integration into various communication contexts.
...
- **Standard compliance**: The function block implements the comparison function from IEC 61131-3 ("greater than or equal to") for the `DWORD` data type.
...
- **Limit Monitoring**: Checking bit masks against a reference or threshold value of type `DWORD`.
- **Control Logic**: Triggering an action once two values of type `DWORD` satisfy the "greater than or equal to" condition.
- **Data Filtering**: Selecting data records or signals that satisfy a `DWORD`-based comparison condition.

## Comparison with Similar Function Blocks

- **AD_LE**: Checks for "less than or equal to" – the inverse function of AD_GE.
- **AD_EQ**: Checks for equality of values.
- **AD_GT**: Performs the "greater than" comparison – the base variant without the equality case, of which AD_GE is the extended version.

AD_GE completes the AD comparison family with the boundary case of equality.

## Conclusion

AD_GE is a simple yet important comparison block for IEC 61131-3 control applications using the `DWORD` data type. Its adapter-oriented interface makes it versatile and allows for a clear separation of event and data paths. Due to its event-driven operation, it is particularly suitable for real-time systems where conditions must react immediately to changes in values.
