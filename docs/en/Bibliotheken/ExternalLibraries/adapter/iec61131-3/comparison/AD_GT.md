...
- **Standard compliance**: The function block implements the comparison function from IEC 61131-3 ("greater than") for the `DWORD` data type.
...
- **Control Logic**: Triggering an action once two values of type `DWORD` satisfy the "greater than" condition.
- **Data Filtering**: Selecting data records or signals that satisfy a `DWORD`-based comparison condition.

## Comparison with Similar Function Blocks

- **AD_LT**: Checks for "less than" – the inverse function of AD_GT.
- **AD_EQ**: Checks for equality of values.
- **AD_GE**: Performs the "greater than or equal to" comparison – the extended version of AD_GT that includes the equality case.

AD_GT is the basic "greater than" component without the equality case, complemented by AD_GE.

## Conclusion

AD_GT is a simple yet important comparison block for IEC 61131-3 control applications using the `DWORD` data type. Its adapter-oriented interface makes it versatile and allows for a clear separation of event and data paths. Due to its event-driven operation, it is particularly suitable for real-time systems where conditions must react immediately to changes in values.
