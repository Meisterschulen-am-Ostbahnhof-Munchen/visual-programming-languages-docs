...
- **Standard compliance**: The function block implements the comparison function from IEC 61131-3 ("less than") for the `DWORD` data type.
...
- **Control Logic**: Triggering an action once two values of type `DWORD` satisfy the "less than" condition.
- **Data Filtering**: Selecting data records or signals that satisfy a `DWORD`-based comparison condition.

## Comparison with Similar Function Blocks

- **AD_GT**: Checks for "greater than" – the inverse function of AD_LT.
- **AD_EQ**: Checks for equality of values.
- **AD_LE**: Performs the "less than or equal to" comparison – the extended version of AD_LT that includes the equality case.

AD_LT is the basic "less than" component without the equality case, complemented by AD_LE.

## Conclusion

AD_LT is a simple yet important comparison block for IEC 61131-3 control applications using the `DWORD` data type. Its adapter-oriented interface makes it versatile and allows for a clear separation of event and data paths. Due to its event-driven operation, it is particularly suitable for real-time systems where conditions must react immediately to changes in values.
