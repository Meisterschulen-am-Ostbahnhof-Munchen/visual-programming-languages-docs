...
- **Standard compliance**: The function block implements the comparison function from IEC 61131-3 ("less than") for the `STRING` data type.
...
- **Control Logic**: Triggering an action once two values of type `STRING` satisfy the "less than" condition.
- **Data Filtering**: Selecting data records or signals that satisfy a `STRING`-based comparison condition.

## Comparison with Similar Function Blocks

- **AIS_GT**: Checks for "greater than" – the inverse function of AIS_LT.
- **AIS_EQ**: Checks for equality of values.
- **AIS_LE**: Performs the "less than or equal to" comparison – the extended version of AIS_LT that includes the equality case.

AIS_LT is the basic "less than" component without the equality case, complemented by AIS_LE.

## Conclusion

AIS_LT is a simple yet important comparison block for IEC 61131-3 control applications using the `STRING` data type. Its adapter-oriented interface makes it versatile and allows for a clear separation of event and data paths. Due to its event-driven operation, it is particularly suitable for real-time systems where conditions must react immediately to changes in values.
