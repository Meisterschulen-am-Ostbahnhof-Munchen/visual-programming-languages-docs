...
- **Standard compliance**: The function block implements the comparison function from IEC 61131-3 ("greater than") for the `WSTRING` data type.
...
- **Control Logic**: Triggering an action once two values of type `WSTRING` satisfy the "greater than" condition.
- **Data Filtering**: Selecting data records or signals that satisfy a `WSTRING`-based comparison condition.

## Comparison with Similar Function Blocks

- **AIWS_LT**: Checks for "less than" – the inverse function of AIWS_GT.
- **AIWS_EQ**: Checks for equality of values.
- **AIWS_GE**: Performs the "greater than or equal to" comparison – the extended version of AIWS_GT that includes the equality case.

AIWS_GT is the basic "greater than" component without the equality case, complemented by AIWS_GE.

## Conclusion

AIWS_GT is a simple yet important comparison block for IEC 61131-3 control applications using the `WSTRING` data type. Its adapter-oriented interface makes it versatile and allows for a clear separation of event and data paths. Due to its event-driven operation, it is particularly suitable for real-time systems where conditions must react immediately to changes in values.
