...
- **IN1.D1**: First value to be compared (data type `INT`, 16-bit signed integer).
...
The adapters `AI` and `AX` are unidirectional interfaces, each providing one event channel and one data channel.
...
- **Adapter-based interface**: The function block uses only adapters (`AI`/`AX`), allowing for flexible integration into various communication contexts.
...
- **Standard compliance**: The function block implements the comparison function from IEC 61131-3 ("greater than or equal to") for the `INT` data type.
...
- **Limit Monitoring**: Checking counter readings against a reference or threshold value of type `INT`.
- **Control Logic**: Triggering an action once two values of type `INT` satisfy the "greater than or equal to" condition.
- **Data Filtering**: Selecting data records or signals that satisfy a `INT`-based comparison condition.

## Comparison with Similar Function Blocks

- **AI_LE**: Checks for "less than or equal to" – the inverse function of AI_GE.
- **AI_EQ**: Checks for equality of values.
- **AI_GT**: Performs the "greater than" comparison – the base variant without the equality case, of which AI_GE is the extended version.

AI_GE completes the AI comparison family with the boundary case of equality.

## Conclusion

AI_GE is a simple yet important comparison block for IEC 61131-3 control applications using the `INT` data type. Its adapter-oriented interface makes it versatile and allows for a clear separation of event and data paths. Due to its event-driven operation, it is particularly suitable for real-time systems where conditions must react immediately to changes in values.
