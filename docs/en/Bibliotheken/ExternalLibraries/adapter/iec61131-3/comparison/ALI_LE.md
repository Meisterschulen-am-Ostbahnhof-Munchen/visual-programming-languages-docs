...
- **Standard compliance**: The function block implements the comparison function from IEC 61131-3 ("less than or equal to") for the `LINT` data type.
...
- **Control Logic**: Triggering an action once two values of type `LINT` satisfy the "less than or equal to" condition.
- **Data Filtering**: Selecting data records or signals that satisfy a `LINT`-based comparison condition.

## Comparison with Similar Function Blocks

- **ALI_GE**: Checks for "greater than or equal to" – the inverse function of ALI_LE.
- **ALI_EQ**: Checks for equality of values.
- **ALI_LT**: Performs the "less than" comparison – the base variant without the equality case, of which ALI_LE is the extended version.

ALI_LE completes the ALI comparison family with the boundary case of equality.

## Conclusion

ALI_LE is a simple yet important comparison block for IEC 61131-3 control applications using the `LINT` data type. Its adapter-oriented interface makes it versatile and allows for a clear separation of event and data paths. Due to its event-driven operation, it is particularly suitable for real-time systems where conditions must react immediately to changes in values.
