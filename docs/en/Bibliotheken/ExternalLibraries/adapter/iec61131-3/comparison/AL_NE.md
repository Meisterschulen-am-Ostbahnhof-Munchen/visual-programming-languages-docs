...
- **Standard compliance**: The function block implements the comparison function from IEC 61131-3 ("not equal to") for the `LWORD` data type.
...
- **Control Logic**: Triggering an action once two values of type `LWORD` satisfy the "not equal to" condition.
- **Data Filtering**: Selecting data records or signals that satisfy a `LWORD`-based comparison condition.

## Comparison with Similar Function Blocks

- **AL_EQ**: Checks for "equal to" – the inverse function of AL_NE.
- **AL_EQ**: Checks for equality of values.

AL_NE is part of the complete AL comparison family (EQ/NE/GT/GE/LT/LE) for the `LWORD` data type.

## Conclusion

AL_NE is a simple yet important comparison block for IEC 61131-3 control applications using the `LWORD` data type. Its adapter-oriented interface makes it versatile and allows for a clear separation of event and data paths. Due to its event-driven operation, it is particularly suitable for real-time systems where conditions must react immediately to changes in values.
