...
- **Standard compliance**: The function block implements the comparison function from IEC 61131-3 ("less than or equal to") for the `QUARTER` data type.
...
- **Control Logic**: Triggering an action once two values of type `QUARTER` satisfy the "less than or equal to" condition.
- **Data Filtering**: Selecting data records or signals that satisfy a `QUARTER`-based comparison condition.

## Comparison with Similar Function Blocks

- **AQ_GE**: Checks for "greater than or equal to" – the inverse function of AQ_LE.
- **AQ_EQ**: Checks for equality of values.
- **AQ_LT**: Performs the "less than" comparison – the base variant without the equality case, of which AQ_LE is the extended version.

AQ_LE completes the AQ comparison family with the boundary case of equality.

## Conclusion

AQ_LE is a simple yet important comparison block for IEC 61131-3 control applications using the `QUARTER` data type. Its adapter-oriented interface makes it versatile and allows for a clear separation of event and data paths. Due to its event-driven operation, it is particularly suitable for real-time systems where conditions must react immediately to changes in values.
