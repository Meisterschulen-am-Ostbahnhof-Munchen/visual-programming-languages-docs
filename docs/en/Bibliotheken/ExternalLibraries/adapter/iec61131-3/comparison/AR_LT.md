...
- **Standard compliance**: The function block implements the comparison function from IEC 61131-3 ("less than") for the `REAL` data type.
...
- **Control Logic**: Triggering an action once two values of type `REAL` satisfy the "less than" condition.
- **Data Filtering**: Selecting data records or signals that satisfy a `REAL`-based comparison condition.

## Comparison with Similar Function Blocks

- **AR_GT**: Checks for "greater than" – the inverse function of AR_LT.
- **AR_EQ**: Checks for equality of values.
- **AR_LE**: Performs the "less than or equal to" comparison – the extended version of AR_LT that includes the equality case.

AR_LT is the basic "less than" component without the equality case, complemented by AR_LE.

## Conclusion

AR_LT is a simple yet important comparison block for IEC 61131-3 control applications using the `REAL` data type. Its adapter-oriented interface makes it versatile and allows for a clear separation of event and data paths. Due to its event-driven operation, it is particularly suitable for real-time systems where conditions must react immediately to changes in values.
