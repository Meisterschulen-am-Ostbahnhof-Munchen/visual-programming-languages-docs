...
- **Standard compliance**: The function block implements the comparison function from IEC 61131-3 ("not equal to") for the `REAL` data type.
...
- **Control Logic**: Triggering an action once two values of type `REAL` satisfy the "not equal to" condition.
- **Data Filtering**: Selecting data records or signals that satisfy a `REAL`-based comparison condition.

## Comparison with Similar Function Blocks

- **AR_EQ**: Checks for "equal to" – the inverse function of AR_NE.
- **AR_EQ**: Checks for equality of values.

AR_NE is part of the complete AR comparison family (EQ/NE/GT/GE/LT/LE) for the `REAL` data type.

## Conclusion

AR_NE is a simple yet important comparison block for IEC 61131-3 control applications using the `REAL` data type. Its adapter-oriented interface makes it versatile and allows for a clear separation of event and data paths. Due to its event-driven operation, it is particularly suitable for real-time systems where conditions must react immediately to changes in values.
