...
- **IN1.D1**: First value to be compared (data type `SINT`, 8-bit signed integer).
...
The adapters `AS` and `AX` are unidirectional interfaces, each providing one event channel and one data channel.
...
- **Adapter-based interface**: The function block uses only adapters (`AS`/`AX`), allowing for flexible integration into various communication contexts.
...
- **Standard compliance**: The function block implements the comparison function from IEC 61131-3 ("greater than or equal to") for the `SINT` data type.
...
- **Limit Monitoring**: Checking small counter and index values against a reference or threshold value of type `SINT`.
- **Control Logic**: Triggering an action once two values of type `SINT` satisfy the "greater than or equal to" condition.
- **Data Filtering**: Selecting data records or signals that satisfy a `SINT`-based comparison condition.

## Comparison with Similar Function Blocks

- **AS_LE**: Checks for "less than or equal to" – the inverse function of AS_GE.
- **AS_EQ**: Checks for equality of values.
- **AS_GT**: Performs the "greater than" comparison – the base variant without the equality case, of which AS_GE is the extended version.

AS_GE completes the AS comparison family with the boundary case of equality.

## Conclusion

AS_GE is a simple yet important comparison block for IEC 61131-3 control applications using the `SINT` data type. Its adapter-oriented interface makes it versatile and allows for a clear separation of event and data paths. Due to its event-driven operation, it is particularly suitable for real-time systems where conditions must react immediately to changes in values.
