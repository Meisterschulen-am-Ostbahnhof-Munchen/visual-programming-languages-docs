...
- **Standard compliance**: The function block implements the comparison function from IEC 61131-3 ("equal to") for the `TIME` data type.
...
- **Control Logic**: Triggering an action once two values of type `TIME` satisfy the "equal to" condition.
- **Data Filtering**: Selecting data records or signals that satisfy a `TIME`-based comparison condition.

## Comparison with Similar Function Blocks

- **ATM_NE**: Checks for "not equal to" – the inverse function of ATM_EQ.
- **ATM_NE**: Checks for inequality of values.

ATM_EQ is part of the complete ATM comparison family (EQ/NE/GT/GE/LT/LE) for the `TIME` data type.

## Conclusion

ATM_EQ is a simple yet important comparison block for IEC 61131-3 control applications using the `TIME` data type. Its adapter-oriented interface makes it versatile and allows for a clear separation of event and data paths. Due to its event-driven operation, it is particularly suitable for real-time systems where conditions must react immediately to changes in values.
