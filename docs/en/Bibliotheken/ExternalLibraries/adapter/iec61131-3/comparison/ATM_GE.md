...
- **Standard compliance**: The function block implements the comparison function from IEC 61131-3 ("greater than or equal to") for the `TIME` data type.
...
- **Control Logic**: Triggering an action once two values of type `TIME` satisfy the "greater than or equal to" condition.
- **Data Filtering**: Selecting data records or signals that satisfy a `TIME`-based comparison condition.

## Comparison with Similar Function Blocks

- **ATM_LE**: Checks for "less than or equal to" – the inverse function of ATM_GE.
- **ATM_EQ**: Checks for equality of values.
- **ATM_GT**: Performs the "greater than" comparison – the base variant without the equality case, of which ATM_GE is the extended version.

ATM_GE completes the ATM comparison family with the boundary case of equality.

## Conclusion

ATM_GE is a simple yet important comparison block for IEC 61131-3 control applications using the `TIME` data type. Its adapter-oriented interface makes it versatile and allows for a clear separation of event and data paths. Due to its event-driven operation, it is particularly suitable for real-time systems where conditions must react immediately to changes in values.
