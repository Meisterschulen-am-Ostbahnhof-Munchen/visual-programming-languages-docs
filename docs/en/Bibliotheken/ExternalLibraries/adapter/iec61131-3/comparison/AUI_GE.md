...
The function block **AUI_GE** compares two values and checks whether the first value (IN1) is greater than or equal to the second value (IN2). The result is provided as a Boolean value via the output adapter. The function block is event-driven and operates asynchronously.
## Interface Structure
...
- **Standard compliance**: The function block implements the comparison function from IEC 61131-3 ("greater than or equal to").
...
- **Limit Monitoring**: Checks whether a measured value (e.g., fill level, rotational speed) has reached or exceeded a predefined threshold.
- **Control Logic**: Activates an action (e.g., stop a pump) when an actual value reaches or exceeds a setpoint.
- **Data Filtering**: Selects data records or signals that meet a minimum condition.

## Comparison with Similar Function Blocks

- **AUI_LE**: Checks for "less than or equal to" – the inverse function of AUI_GE.
- **AUI_EQ**: Checks for equality of values.
- **AUI_GT**: Checks for "greater than" – the base variant without the equality case, of which AUI_GE is the extended version.

AUI_GE completes the AUI comparison family with the boundary case of equality, making it the right choice when a threshold should be inclusive.

## Conclusion

AUI_GE is a simple yet important comparison block for IEC 61131-3 control applications. Its adapter-oriented interface makes it versatile and allows for a clear separation of event and data paths. Due to its event-driven operation, it is particularly suitable for real-time systems where conditions must react immediately to changes in values.
