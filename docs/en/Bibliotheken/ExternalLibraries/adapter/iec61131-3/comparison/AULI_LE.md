...
- **Standard compliance**: The function block implements the comparison function from IEC 61131-3 ("less than or equal to") for the `ULINT` data type.
...
- **Control Logic**: Triggering an action once two values of type `ULINT` satisfy the "less than or equal to" condition.
- **Data Filtering**: Selecting data records or signals that satisfy a `ULINT`-based comparison condition.

## Comparison with Similar Function Blocks

- **AULI_GE**: Checks for "greater than or equal to" – the inverse function of AULI_LE.
- **AULI_EQ**: Checks for equality of values.
- **AULI_LT**: Performs the "less than" comparison – the base variant without the equality case, of which AULI_LE is the extended version.

AULI_LE completes the AULI comparison family with the boundary case of equality.

## Conclusion

AULI_LE is a simple yet important comparison block for IEC 61131-3 control applications using the `ULINT` data type. Its adapter-oriented interface makes it versatile and allows for a clear separation of event and data paths. Due to its event-driven operation, it is particularly suitable for real-time systems where conditions must react immediately to changes in values.
