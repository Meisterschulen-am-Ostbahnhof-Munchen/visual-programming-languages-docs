...
- **IN1.D1**: First value to be compared (data type `WORD`, 16-bit bit pattern (word)).
...
The adapters `AW` and `AX` are unidirectional interfaces, each providing one event channel and one data channel.
...
- **Adapter-based interface**: The function block uses only adapters (`AW`/`AX`), allowing for flexible integration into various communication contexts.
...
- **Standard compliance**: The function block implements the comparison function from IEC 61131-3 ("greater than or equal to") for the `WORD` data type.
...
- **Limit Monitoring**: Checking status words against a reference or threshold value of type `WORD`.
- **Control Logic**: Triggering an action once two values of type `WORD` satisfy the "greater than or equal to" condition.
- **Data Filtering**: Selecting data records or signals that satisfy a `WORD`-based comparison condition.

## Comparison with Similar Function Blocks

- **AW_LE**: Checks for "less than or equal to" – the inverse function of AW_GE.
- **AW_EQ**: Checks for equality of values.
- **AW_GT**: Performs the "greater than" comparison – the base variant without the equality case, of which AW_GE is the extended version.

AW_GE completes the AW comparison family with the boundary case of equality.

## Conclusion

AW_GE is a simple yet important comparison block for IEC 61131-3 control applications using the `WORD` data type. Its adapter-oriented interface makes it versatile and allows for a clear separation of event and data paths. Due to its event-driven operation, it is particularly suitable for real-time systems where conditions must react immediately to changes in values.
