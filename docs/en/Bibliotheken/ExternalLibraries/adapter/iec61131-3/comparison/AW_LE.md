...
- **Standard compliance**: The function block implements the comparison function from IEC 61131-3 ("less than or equal to") for the `WORD` data type.
...
- **Control Logic**: Triggering an action once two values of type `WORD` satisfy the "less than or equal to" condition.
- **Data Filtering**: Selecting data records or signals that satisfy a `WORD`-based comparison condition.

## Comparison with Similar Function Blocks

- **AW_GE**: Checks for "greater than or equal to" – the inverse function of AW_LE.
- **AW_EQ**: Checks for equality of values.
- **AW_LT**: Performs the "less than" comparison – the base variant without the equality case, of which AW_LE is the extended version.

AW_LE completes the AW comparison family with the boundary case of equality.

## Conclusion

AW_LE is a simple yet important comparison block for IEC 61131-3 control applications using the `WORD` data type. Its adapter-oriented interface makes it versatile and allows for a clear separation of event and data paths. Due to its event-driven operation, it is particularly suitable for real-time systems where conditions must react immediately to changes in values.
