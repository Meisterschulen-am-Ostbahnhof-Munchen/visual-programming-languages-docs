...
![ADI_SUBSCRIBE_1](./ADI_SUBSCRIBE_1.svg)

* * * * * * * * * *

## Introduction
The function block **ADI_SUBSCRIBE_1** is a composite function block that encapsulates the network-based `SUBSCRIBE_1` function block from the IEC 61499 standard library and maps its interface to a unidirectional **ADI adapter**. It subscribes to a publish channel configured via `ID` and provides the received **DINT** value, buffered via a D flip-flop (E_D_FF_ANY), at the adapter plug `OUT`. This allows for the clean integration of network communication and DINT signal processing in the ADI adapter environment.
...
- **Buffering with D flip-flop**: The received DINT value is buffered via an internal `iec61499::events::E_D_FF_ANY` and remains stable at the output until the next value is received. `EO` only fires when `Q` changes.
...
- **RSP is a no-op for subscribers**: According to source code analysis of `4diac-forte` (`CCommFB::sendData()`), the `RSP` event in `SUBSCRIBE_1` (service type subscriber) does not execute any action – the branch `!= e_Subscriber` is skipped. The event input pair `INIT`/`RSP` is only executed for acknowledgment and is non-functional for acknowledgment in the subscriber case. It is nevertheless routed externally to maintain a configurable connection.
 ...
 - **Encapsulation**: The original event/data interface of `SUBSCRIBE_1` (including `IND`/`RD_1`) is moved internally; only the ADI adapter interface is visible externally.
...
3. **Data Reception**: At `IND`, the received DINT value is transferred to the internal D flip-flop and output via `OUT`.


## Application Scenarios

- **Network Coupling in ADI Applications**: Receiving DINT signals (e.g., switching commands or status bits) via publish/subscribe and forwarding them via ADI adapters to the adapter-based logic.
...
- **Modular Control Architectures**: Integration of distributed communication into libraries that rely entirely on ADI adapters.


## Comparison with Similar Function Blocks

- **SUBSCRIBE_1**: The standard function block exposes its interface as events/variables (`IND`, `RD_1`, ...). **ADI_SUBSCRIBE_1** encapsulates this function block, buffers the value with a D flip-flop (`E_D_FF_ANY`), and only provides the ADI adapter – making the interface significantly more compact and integrable into adapter networks.

- **AX_D_FF**: Bufferes a DINT value at the adapter level but lacks network functionality. **ADI_SUBSCRIBE_1** combines subscribing and buffering in a single function block.


## Conclusion

**ADI_SUBSCRIBE_1** combines the publish/subscribe communication of the standard function block `SUBSCRIBE_1` with adapter-based DINT processing. Internal D flip-flop buffering keeps the received value stable until new data arrives, and the simplified ADI interface makes the function block ideal for fully adapter-based IEC 61499 applications.
