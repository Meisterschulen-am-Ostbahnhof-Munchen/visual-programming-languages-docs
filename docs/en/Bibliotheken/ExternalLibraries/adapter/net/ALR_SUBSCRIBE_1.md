...
![ALR_SUBSCRIBE_1](./ALR_SUBSCRIBE_1.svg)

* * * * * * * * * *

## Introduction

The function block **ALR_SUBSCRIBE_1** is a composite function block that encapsulates the network-based `SUBSCRIBE_1` function block from the IEC 61499 standard library and maps its interface to a unidirectional **ALR adapter**. It subscribes to a publish channel configured via `ID` and provides the received **LREAL** value, buffered via a D flip-flop (E_D_FF_ANY), at the adapter plug `OUT`. This allows for clean integration of network communication and LREAL signal processing in the ALR adapter environment.
...
- **Buffering with D flip-flop**: The received LREAL value is buffered via an internal `iec61499::events::E_D_FF_ANY` and remains stable at the output until the next value is received. `EO` only fires when `Q` changes.
...
- **Encapsulation**: The original event/data interface of `SUBSCRIBE_1` (including `IND`/`RD_1`) is moved internally; only the ALR adapter interface is visible externally.
...
3. **Data Receipt**: At `IND`, the received LREAL value is transferred to the internal D flip-flop and output via `OUT`.

## Application Scenarios

- **Network Coupling in ALR Applications**: Receiving LREAL signals (e.g., switching commands or status bits) via publish/subscribe and forwarding them via ALR adapters to the adapter-based logic.
...
- **Modular Control Architectures**: Integration of distributed communication into libraries that rely entirely on ALR adapters.

## Comparison with Similar Function Blocks

- **SUBSCRIBE_1**: The standard function block exposes its interface as events/variables (`IND`, `RD_1`, ...). **ALR_SUBSCRIBE_1** encapsulates this function block, buffers the value with a D flip-flop (`E_D_FF_ANY`), and only provides the ALR adapter – making the interface significantly more compact and integrable into adapter networks.
- **AX_D_FF**: Bufferes an LREAL value at the adapter level but lacks network functionality. **ALR_SUBSCRIBE_1** combines subscribing and buffering in a single function block.

## Conclusion

**ALR_SUBSCRIBE_1** combines the publish/subscribe communication of the standard function block `SUBSCRIBE_1` with adapter-based LREAL processing. Internal D flip-flop buffering keeps the received value stable until new data arrives, and the simplified ALR interface makes the function block ideal for end-to-end adapter-based IEC 61499 applications.
