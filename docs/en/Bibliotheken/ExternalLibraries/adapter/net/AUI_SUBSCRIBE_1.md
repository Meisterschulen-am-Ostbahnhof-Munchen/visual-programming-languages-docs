...
The function block **AUI_SUBSCRIBE_1** is a composite function block that encapsulates the network-based `SUBSCRIBE_1` function block from the IEC 61499 standard library and maps its interface to a unidirectional **AUI adapter**. It subscribes to a publish channel configured via `ID` and makes the received **UINT** value buffered via a D flip-flop (E_D_FF_ANY) available on the adapter plug `OUT`. This allows for clean integration of network communication and UINT signal processing in the AUI adapter environment.
...
- **Buffering with D flip-flop**: The received UINT value is buffered via an internal `iec61499::events::E_D_FF_ANY` and remains stable at the output until the next value is received. `EO` only fires when `Q` changes.
...
- **Encapsulation**: The original event/data interface of `SUBSCRIBE_1` (including `IND`/`RD_1`) is moved internally; only the AUI adapter interface is visible externally.
...
3. **Data Receipt**: At `IND`, the received UINT value is transferred to the internal D flip-flop and output via `OUT`.

## Application Scenarios

- **Network Coupling in AUI Applications**: Receiving UINT signals (e.g., switching commands or status bits) via publish/subscribe and forwarding them via AUI adapters to the adapter-based logic.
...
- **SUBSCRIBE_1**: The standard building block exposes its interface as events/variables (`IND`, `RD_1`, ...). **AUI_SUBSCRIBE_1** encapsulates this building block, buffers the value with a D flip-flop (`E_D_FF_ANY`), and only provides the AUI adapter – making the interface significantly more compact and integrable into adapter networks.
- **AX_D_FF**: Bufferes a UINT value at the adapter level but lacks network functionality. **AUI_SUBSCRIBE_1** combines subscribing and buffering in a single building block.

## Conclusion

**AUI_SUBSCRIBE_1** combines the publish/subscribe communication of the standard function block `SUBSCRIBE_1` with adapter-based UINT processing. Internal D flip-flop buffering keeps the received value stable until new data arrives, and the simplified AUI interface makes the function block ideal for end-to-end adapter-based IEC 61499 applications.
