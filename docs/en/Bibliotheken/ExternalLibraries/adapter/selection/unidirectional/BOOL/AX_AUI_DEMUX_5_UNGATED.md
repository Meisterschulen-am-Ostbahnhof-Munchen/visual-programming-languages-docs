...
> ℹ️ **UNGATED variant:** This block is the ungated version of [`AX_AUI_DEMUX_5`](AX_AUI_DEMUX_5.md). It suppresses **no** unchanged repeats – every newly computed result is forwarded unconditionally, even without a value change. This matters for consumers that need a periodic cadence regardless of value change (e.g. derivative/frequency calculations that would otherwise fail to decay toward zero). Any change-detection/gating statements further down this page do **not** apply to this block.
...
`AX_AUI_DEMUX_5_UNGATED` is the adapter-based variant of the generic demultiplexer for data type `BOOL`. Unlike `AX_DEMUX_5`, it does not receive the selection index through a REQ event with an associated K data input, but through its own adapter socket **K** of type `AUI` ("Adapter Unidirectional Interface"). This lets the index be fed directly from another block with a matching `AUI` plug, without wiring a separate event and data line for it.

## Interface Structure

### **Event Inputs**

- No explicit event inputs on the block itself. The index is received exclusively through the adapter socket **K**: as soon as the `AUI` plug connected there fires its internal event `E1` carrying the data value `D1`, `AX_AUI_DEMUX_5_UNGATED` evaluates the index internally and triggers processing.
...
- **IN** (Socket): input adapter carrying the Boolean value to forward, routed to exactly one of the outputs (AX adapter type).
- **OUT1** (Plug): output adapter 1 of 5 (AX adapter type).
- **OUT2** (Plug): output adapter 2 of 5 (AX adapter type).
- **OUT3** (Plug): output adapter 3 of 5 (AX adapter type).
- **OUT4** (Plug): output adapter 4 of 5 (AX adapter type).
- **OUT5** (Plug): output adapter 5 of 5 (AX adapter type).
...
- Generic implementation (`GEN_AX_AUI_DEMUX`) -- shared across all port counts of this block family (AX_AUI_DEMUX_2 … AX_AUI_DEMUX_5_UNGATED).
...
- Distributing a Boolean value to up to 5 downstream consumers, selected via an adapter-carried index
...
Compare with `AX_DEMUX_5` (same distribution logic, but the index arrives through a classic **REQ** event plus **K** data input instead of an adapter).
