...
`AD_AUI_DEMUX_3` receives a value on socket **IN** and forwards it to exactly one of the 3 output plugs `OUT1` … `OUT3`. Which output is written is decided by the index arriving on the adapter socket **K** (type `AUI`): as soon as the `AUI` plug connected to K fires its internal `E1` event carrying the data value `D1`, `AD_AUI_DEMUX_3` interprets that value as a 0-based index (`0` … `2`), writes the input value to the corresponding `OUT` plug and triggers that plug's adapter event. `AD_AUI_DEMUX_3` then confirms completion via the **CNF** event.
...
Compare with [AD_DEMUX_3](AD_DEMUX_3.md) (same distribution logic, but the index arrives through a classic **REQ** event plus **K** data input instead of an adapter).
