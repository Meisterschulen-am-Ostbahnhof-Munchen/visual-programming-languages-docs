...
- No explicit event inputs on the block itself. The index is received exclusively through the adapter socket **K**: as soon as the `AUI` plug connected there fires its internal event `E1` carrying the data value `D1`, `AE_AUI_MUX_2` evaluates the index internally and triggers processing.
...
- **IN1** (Socket): input adapter 1 of 2, passed through to `OUT` when the index is `K = 0` (AE adapter type).
- **IN2** (Socket): input adapter 2 of 2, passed through to `OUT` when the index is `K = 1` (AE adapter type).
- **OUT** (Plug): output adapter, forwards whichever input the index selected (AE adapter type).

## Functionality

`AE_AUI_MUX_2` selects one of the 2 input sockets `IN1` … `IN2` via the adapter socket **K** (type `AUI`) and forwards its value to the single output plug `OUT`. As soon as the `AUI` plug connected to K fires its internal `E1` event carrying the data value `D1`, `AE_AUI_MUX_2` interprets that value as a 0-based index (`0` … `1`), copies the selected event to `OUT` and triggers its adapter event. `AE_AUI_MUX_2` then confirms the operation via the **CNF** event.
...
- The `AE` adapter carries only an event, no data value -- `AE_AUI_MUX_2` therefore switches pure event pulses, not payload data.
...
- Choosing between up to 2 event sources through a centrally managed adapter index
...
`AE_AUI_MUX_2` carries the multiplexer logic of the `AE_MUX_2` family over to a purely adapter-based index supply. This makes it the right choice whenever the selection index is already available as an `AUI` adapter from another block and no additional event/data wiring for the index is wanted.
