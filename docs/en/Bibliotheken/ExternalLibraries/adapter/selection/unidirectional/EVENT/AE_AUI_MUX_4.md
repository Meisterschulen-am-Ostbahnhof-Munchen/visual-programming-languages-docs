...
- No explicit event inputs on the block itself. The index is received exclusively through the adapter socket **K**: as soon as the `AUI` plug connected there fires its internal event `E1` carrying the data value `D1`, `AE_AUI_MUX_4` evaluates the index internally and triggers processing.
...
- **IN1** (Socket): input adapter 1 of 4, passed through to `OUT` when the index is `K = 0` (AE adapter type).
- **IN2** (Socket): input adapter 2 of 4, passed through to `OUT` when the index is `K = 1` (AE adapter type).
- **IN3** (Socket): input adapter 3 of 4, passed through to `OUT` when the index is `K = 2` (AE adapter type).
- **IN4** (Socket): input adapter 4 of 4, passed through to `OUT` when the index is `K = 3` (AE adapter type).
- **OUT** (Plug): output adapter, forwards whichever input the index selected (AE adapter type).

## Functionality

`AE_AUI_MUX_4` selects one of the 4 input sockets `IN1` … `IN4` via the adapter socket **K** (type `AUI`) and forwards its value to the single output plug `OUT`. As soon as the `AUI` plug connected to K fires its internal `E1` event carrying the data value `D1`, `AE_AUI_MUX_4` interprets that value as a 0-based index (`0` … `3`), copies the selected event to `OUT` and triggers its adapter event. `AE_AUI_MUX_4` then confirms the operation via the **CNF** event.
...
- The `AE` adapter carries only an event, no data value -- `AE_AUI_MUX_4` therefore switches pure event pulses, not payload data.
...
- Choosing between up to 4 event sources through a centrally managed adapter index
...
`AE_AUI_MUX_4` carries the multiplexer logic of the `AE_MUX_4` family over to a purely adapter-based index supply. This makes it the right choice whenever the selection index is already available as an `AUI` adapter from another block and no additional event/data wiring for the index is wanted.
