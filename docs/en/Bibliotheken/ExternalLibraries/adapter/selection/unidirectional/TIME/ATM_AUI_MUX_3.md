...
`ATM_AUI_MUX_3` selects one of the 3 input sockets `IN1` … `IN3` via the adapter socket **K** (type `AUI`) and forwards its value to the single output plug `OUT`. As soon as the `AUI` plug connected to K fires its internal `E1` event carrying the data value `D1`, `ATM_AUI_MUX_3` interprets that value as a 0-based index (`0` … `2`), copies the value of the selected input to `OUT` and triggers its adapter event. `ATM_AUI_MUX_3` then confirms the operation via the **CNF** event.
...
Compare with [ATM_MUX_3](ATM_MUX_3.md) (same selection logic, but the index arrives through a classic **REQ** event plus **K** data input instead of an adapter).
