...
> ℹ️ **UNGATED variant:** This block is the ungated version of [`AUI_AUI_DEMUX_3`](AUI_AUI_DEMUX_3.md). It suppresses **no** unchanged repeats – every newly computed result is forwarded unconditionally, even without a value change. This matters for consumers that need a periodic cadence regardless of value change (e.g. derivative/frequency calculations that would otherwise fail to decay toward zero). Any change-detection/gating statements further down this page do **not** apply to this block.
...
- Because this block's own payload type is `UINT`, `AUI_AUI_DEMUX_3_UNGATED` uses the same adapter type (`AUI`) for both the payload (`IN`/`OUT`) and the index socket **K** -- the two roles are wired independently even though they share an adapter type.
...
Compare with `AUI_DEMUX_3` (same distribution logic, but the index arrives through a classic **REQ** event plus **K** data input instead of an adapter).
...
- **[`AUI_AUI_DEMUX_3`](AUI_AUI_DEMUX_3.md)**: The gated variant – updates the output only on an actual value change.

## Conclusion

`AUI_AUI_DEMUX_3_UNGATED` carries the demultiplexer logic of the `AUI_DEMUX_3` family over to a purely adapter-based index supply. This makes it the right choice whenever the selection index is already available as an `AUI` adapter from another block and no additional event/data wiring for the index is wanted.
