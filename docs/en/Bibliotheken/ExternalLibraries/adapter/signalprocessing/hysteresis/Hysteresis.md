...
1. If the function block is in the **sON** state (output is The output (`TRUE`) only switches back to `FALSE` (**sOFF**) when the input signal falls well below the lower limit:
