...
The function block `OR_BOOL_8` is stateless (combinatorial logic). It has no internal memory. The output `OUT` is, at any given time, solely a direct function of the current input values read in by the last `REQ` event.
...
- **`OR_BOOL_2`, `OR_BOOL_4`:** These are structurally identical function blocks, but they only connect two or four inputs, respectively. `OR_BOOL_8` offers a higher number of inputs without having to cascade multiple instances of blocks with fewer inputs. See: [OR_8](../../../StandardLibraries/iec61131-3/bitwiseOperators/OR_8.md)
...
The `OR_BOOL_8` function block is a fundamental, reliable, and easy-to-use building block for Boolean logic processing. Its strength lies in its clear, standards-compliant interface and efficient processing of up to eight signals into a single statement. It is the first choice for applications requiring an inclusive OR operation across multiple signals.
