...
The `OR_BOOL_9` block operates according to the following algorithm:
...
- **`OR_BOOL_2`, `OR_BOOL_4` etc.**: These blocks offer the same OR functionality, but for a smaller number of inputs (2, 4, ...). `OR_BOOL_9` is optimized for applications with nine signals to be combined. See: [OR_9](../../../StandardLibraries/iec61131-3/bitwiseOperators/OR_9.md)
- **`AND_BOOL_9`**: Performs the logical AND operation. The result is only `TRUE` if *all* inputs are `TRUE`, unlike OR, where *at least one* input must be `TRUE`.
- **`XOR_BOOL_9`**: Calculates the exclusive OR (antivalence). The result is `TRUE` if there is an odd number of inputs, resulting in `TRUE`.

## Conclusion

The `OR_BOOL_9` is a robust and easy-to-use standard block for the logical OR operation of nine Boolean signals. Its event-driven, stateless architecture makes it a reliable and efficient component for control and monitoring logic in 4diac-based automation systems. Using this specialized block instead of a cascade of basic gates improves the readability and maintainability of the application code.
