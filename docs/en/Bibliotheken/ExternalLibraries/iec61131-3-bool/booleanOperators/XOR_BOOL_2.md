...
Compared to other basic logic function blocks like `AND_BOOL_2` or `OR_BOOL_2`, `XOR_BOOL_2` is the only one that returns a `TRUE` result when the inputs are *different*. Blocks like `NOT` only invert a single input. `XOR_BOOL_2` can be considered a special case of an inequality comparison (`NE_BOOL`) for two Boolean values. See: [XOR_2](../../../StandardLibraries/iec61131-3/bitwiseOperators/XOR_2.md)

## Conclusion

The `XOR_BOOL_2` is a fundamental, reliable, and easy-to-use building block for logical operation in IEC 61131-3-based control applications. Its event-driven, stateless architecture makes it particularly suitable for integration into complex, deterministic control networks. Its clear functionality and minimal interface make it an indispensable tool for every control system developer.
