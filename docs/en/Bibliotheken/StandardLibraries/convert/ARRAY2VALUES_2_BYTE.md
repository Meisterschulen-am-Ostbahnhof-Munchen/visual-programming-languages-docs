...
- **CNF**: Confirms completion, carries `OUT_1`, `OUT_2`.

### **Data Inputs**

- **IN** (`BYTE`, array size 2): The array to split (8-bit bit pattern).

### **Data Outputs**

- `OUT_1`, `OUT_2` (`BYTE`): `OUT_i` corresponds to `IN[i-1]`, i.e. the i-th element of the array.

## Functionality

On `REQ`, every element of `IN` is copied to the corresponding scalar output `OUT_i` (`IN[0]` → `OUT_1`, …, `IN[1]` → `OUT_2`), then `CNF` is triggered.
...
- **Fixed size 2**: For other array sizes of the same type see `ARRAY2VALUES_4_BYTE`, `ARRAY2VALUES_8_BYTE`, `ARRAY2VALUES_16_BYTE`, `ARRAY2VALUES_32_BYTE`.
...
- **`VALUES2ARRAY_2_BYTE`**: the reverse direction — combines 2 individual values into an array.
- **`ARRAY2ARRAY_2_BYTE`**: copies the array unchanged instead of splitting it.

## Conclusion

`ARRAY2VALUES_2_BYTE` provides a simple, generically implemented split of a `BYTE` array of size 2 into 2 discrete individual variables and is suitable for adapting array-based to variable-based interfaces.
