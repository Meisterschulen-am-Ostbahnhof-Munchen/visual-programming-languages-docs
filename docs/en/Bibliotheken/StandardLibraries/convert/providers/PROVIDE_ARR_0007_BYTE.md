...
- **D1** (`BYTE`, array size 7, index 0…6): The array container (8-bit bit pattern per element). Can be pre-filled with start values in the FB instance editor (e.g. `[16#01, 16#00, ...]` for `BYTE`).
...
- **Fixed size 7**: Other sizes of the same or different data types: `PROVIDE_ARR_0002_BYTE`, `PROVIDE_ARR_0008_BYTE`, `PROVIDE_ARR_0014_BYTE`, `PROVIDE_ARR_0016_BYTE`.
...
- `PROVIDE_ARR_0002_BYTE`, `PROVIDE_ARR_0008_BYTE`, `PROVIDE_ARR_0014_BYTE`, `PROVIDE_ARR_0016_BYTE`: the same generic implementation with a different array size or data type.
- **`ARRAY2ARRAY_2_BYTE`**: copies an already existing array instead of providing/pre-filling one.

## Conclusion

`PROVIDE_ARR_0007_BYTE` provides a simple, design-time pre-fillable `BYTE` array container of fixed size 7 and is suitable for configuration and test-data arrays intended for reuse across 4diac applications.
