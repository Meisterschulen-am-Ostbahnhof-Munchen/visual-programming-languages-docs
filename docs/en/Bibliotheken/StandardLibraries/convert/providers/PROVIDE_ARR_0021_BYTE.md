...
- **D1** (`BYTE`, array size 21, index 0…20): The array container (8-bit bit pattern per element). Can be pre-filled with start values in the FB instance editor (e.g. `[16#01, 16#00, ...]` for `BYTE`).
...
- **Fixed size 21**: Other sizes of the same or different data types: `PROVIDE_ARR_0002_BYTE`, `PROVIDE_ARR_0007_BYTE`, `PROVIDE_ARR_0008_BYTE`, `PROVIDE_ARR_0014_BYTE`.
...
`PROVIDE_ARR_0021_BYTE` provides a simple, design-time pre-fillable `BYTE` array container of fixed size 21 and is suitable for configuration and test-data arrays intended for reuse across 4diac applications.
