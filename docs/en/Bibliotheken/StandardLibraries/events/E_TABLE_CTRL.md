...
4. `E_DELAY` 3. After `E_DELAY` has expired, it signals this via its `EO` output back to the `CLK` input of `E_TABLE_CTRL`.
5. `E_TABLE_CTRL` then calculates the next delay time, and the cycle repeats.
