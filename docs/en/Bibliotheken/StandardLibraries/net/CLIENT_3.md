...
`CLIENT_3` initializes a connection to the matching `SERVER_3` block via `INIT` (when `QI = TRUE`) or closes it (when `QI = FALSE`); completion is confirmed via `INITO`. Via `REQ`, the values `SD_1` `SD_2` `SD_3` are transmitted to the server. Once response data (`RD_1` `RD_2` `RD_3`) is available, the block triggers `CNF` and provides `RD_1` `RD_2` `RD_3`.
...
- **Counterpart `SERVER_3`**: `CLIENT_3` is only functionally compatible with a `SERVER_3` block on the server side — the number of send fields on one side must match the number of receive fields on the other.
...
- **Client-side connection** to a communication partner already laid out as a `SERVER_3` with the matching field count.

## Comparison with similar function blocks

- **[SERVER_3](SERVER_3.md)**: the direct counterpart on the server side, with send/receive field counts swapped.
- **[CLIENT_1](CLIENT_1.md) / [SERVER_1](SERVER_1.md)**: the same generic implementation with one send and one receive field each.

## Conclusion

`CLIENT_3` provides the generic client variant of the `GEN_CLIENT` family tailored to 3 send and 3 receive fields, suiting network connections whose payload count differs from the standard single-field variant.
