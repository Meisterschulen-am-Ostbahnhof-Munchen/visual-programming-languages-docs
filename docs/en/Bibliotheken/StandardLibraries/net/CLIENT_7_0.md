...
- No receive fields (`CLIENT_7_0` receives no payload data via `CNF`, only connection/status information).

## Functionality

`CLIENT_7_0` initializes a connection to the matching `SERVER_0_7` block via `INIT` (when `QI = TRUE`) or closes it (when `QI = FALSE`); completion is confirmed via `INITO`. Via `REQ`, the values `SD_1` `SD_2` `SD_3` `SD_4` `SD_5` `SD_6` `SD_7` are transmitted to the server. Once a response is available, the block triggers `CNF`.
...
- **Counterpart `SERVER_0_7`**: `CLIENT_7_0` is only functionally compatible with a `SERVER_0_7` block on the server side — the number of send fields on one side must match the number of receive fields on the other.
...
- **Client-side connection** to a communication partner already laid out as a `SERVER_0_7` with the matching field count.

## Comparison with similar function blocks

- **[SERVER_0_7](SERVER_0_7.md)**: the direct counterpart on the server side, with send/receive field counts swapped.
- **[CLIENT_1](CLIENT_1.md) / [SERVER_1](SERVER_1.md)**: the same generic implementation with one send and one receive field each.

## Conclusion

`CLIENT_7_0` provides the generic client variant of the `GEN_CLIENT` family tailored to 7 send and 0 receive fields, suiting network connections whose payload count differs from the standard single-field variant.
