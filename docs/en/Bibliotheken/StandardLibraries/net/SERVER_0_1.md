...
- **IND**: Signals that new data is available from the client, carries `QO`, `STATUS` and `RD_1`.
...
- No send fields (`SERVER_0_1` transmits no payload data with `RSP`, only connection control).
...
`SERVER_0_1` initializes a connection to the matching `CLIENT_1_0` block via `INIT` (when `QI = TRUE`) or closes it (when `QI = FALSE`); completion is confirmed via `INITO`. Via `RSP`, a request without payload is sent. Once response data (`RD_1`) is available, the block triggers `IND` and provides `RD_1`.
...
- **Counterpart `CLIENT_1_0`**: `SERVER_0_1` is only functionally compatible with a `CLIENT_1_0` block on the client side — the number of send fields on one side must match the number of receive fields on the other.
...
- **Communication between distributed control systems** that need to send exactly 0 values and receive exactly 1 values, without carrying unused surplus data fields.
- **Server-side connection** to a communication partner already laid out as a `CLIENT_1_0` with the matching field count.

## Comparison with similar function blocks

- **[CLIENT_1_0](CLIENT_1_0.md)**: the direct counterpart on the client side, with send/receive field counts swapped.
- **[CLIENT_1](CLIENT_1.md) / [SERVER_1](SERVER_1.md)**: the same generic implementation with one send and one receive field each.

## Conclusion

`SERVER_0_1` provides the generic server variant of the `GEN_SERVER` family tailored to 0 send and 1 receive fields, suiting network connections whose payload count differs from the standard single-field variant.
