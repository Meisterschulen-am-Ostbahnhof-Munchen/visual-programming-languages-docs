...
The `SERVER_10_0` function block is the generic server variant with 10 send and 0 receive data fields for communicating with a matching [CLIENT_0_10](CLIENT_0_10.md) block. It transmits 10 data values (`SD_1` `SD_2` `SD_3` `SD_4` `SD_5` `SD_6` `SD_7` `SD_8` `SD_9` `SD_10`) to the client without receiving any data back. Like all `SERVER_*` blocks, it is based on the generic `GEN_SERVER` implementation — the same C++ base as [CLIENT_1](CLIENT_1.md)/[SERVER_1](SERVER_1.md); only the number of send/receive fields differs per instantiation.
...
- No receive fields (`SERVER_10_0` receives no payload data via `IND`, only connection/status information).

## Functionality

`SERVER_10_0` initializes a connection to the matching `CLIENT_0_10` block via `INIT` (when `QI = TRUE`) or closes it (when `QI = FALSE`); completion is confirmed via `INITO`. Via `RSP`, the values `SD_1` `SD_2` `SD_3` `SD_4` `SD_5` `SD_6` `SD_7` `SD_8` `SD_9` `SD_10` are transmitted to the client. Once a response is available, the block triggers `IND`.
...
- **Counterpart `CLIENT_0_10`**: `SERVER_10_0` is only functionally compatible with a `CLIENT_0_10` block on the client side — the number of send fields on one side must match the number of receive fields on the other.
...
- **Communication between distributed control systems** that need to send exactly 10 values and receive exactly 0 values, without carrying unused surplus data fields.
- **Server-side connection** to a communication partner already laid out as a `CLIENT_0_10` with the matching field count.

## Comparison with similar function blocks

- **[CLIENT_0_10](CLIENT_0_10.md)**: the direct counterpart on the client side, with send/receive field counts swapped.
- **[CLIENT_1](CLIENT_1.md) / [SERVER_1](SERVER_1.md)**: the same generic implementation with one send and one receive field each.

## Conclusion

`SERVER_10_0` provides the generic server variant of the `GEN_SERVER` family tailored to 10 send and 0 receive fields, suiting network connections whose payload count differs from the standard single-field variant.
