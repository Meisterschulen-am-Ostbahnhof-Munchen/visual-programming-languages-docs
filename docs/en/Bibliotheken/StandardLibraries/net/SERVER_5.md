...
`SERVER_5` initializes a connection to the matching `CLIENT_5` block via `INIT` (when `QI = TRUE`) or closes it (when `QI = FALSE`); completion is confirmed via `INITO`. Via `RSP`, the values `SD_1` `SD_2` `SD_3` `SD_4` `SD_5` are transmitted to the client. Once response data (`RD_1` `RD_2` `RD_3` `RD_4` `RD_5`) is available, the block triggers `IND` and provides `RD_1` `RD_2` `RD_3` `RD_4` `RD_5`.
...
- **Counterpart `CLIENT_5`**: `SERVER_5` is only functionally compatible with a `CLIENT_5` block on the client side — the number of send fields on one side must match the number of receive fields on the other.
...
- **Server-side connection** to a communication partner already laid out as a `CLIENT_5` with the matching field count.

## Comparison with similar function blocks

- **[CLIENT_5](CLIENT_5.md)**: the direct counterpart on the client side, with send/receive field counts swapped.
- **[CLIENT_1](CLIENT_1.md) / [SERVER_1](SERVER_1.md)**: the same generic implementation with one send and one receive field each.

## Conclusion

`SERVER_5` provides the generic server variant of the `GEN_SERVER` family tailored to 5 send and 5 receive fields, suiting network connections whose payload count differs from the standard single-field variant.
