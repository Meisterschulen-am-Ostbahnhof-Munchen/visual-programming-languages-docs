...
- No receive fields (`SERVER_6_0` receives no payload data via `IND`, only connection/status information).

## Functionality

`SERVER_6_0` initializes a connection to the matching `CLIENT_0_6` block via `INIT` (when `QI = TRUE`) or closes it (when `QI = FALSE`); completion is confirmed via `INITO`. Via `RSP`, the values `SD_1` `SD_2` `SD_3` `SD_4` `SD_5` `SD_6` are transmitted to the client. Once a response is available, the block triggers `IND`.
...
- **Counterpart `CLIENT_0_6`**: `SERVER_6_0` is only functionally compatible with a `CLIENT_0_6` block on the client side — the number of send fields on one side must match the number of receive fields on the other.
...
- **Server-side connection** to a communication partner already laid out as a `CLIENT_0_6` with the matching field count.

## Comparison with similar function blocks

- **[CLIENT_0_6](CLIENT_0_6.md)**: the direct counterpart on the client side, with send/receive field counts swapped.
- **[CLIENT_1](CLIENT_1.md) / [SERVER_1](SERVER_1.md)**: the same generic implementation with one send and one receive field each.

## Conclusion

`SERVER_6_0` provides the generic server variant of the `GEN_SERVER` family tailored to 6 send and 0 receive fields, suiting network connections whose payload count differs from the standard single-field variant.
