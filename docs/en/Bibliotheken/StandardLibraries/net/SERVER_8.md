...
`SERVER_8` initializes a connection to the matching `CLIENT_8` block via `INIT` (when `QI = TRUE`) or closes it (when `QI = FALSE`); completion is confirmed via `INITO`. Via `RSP`, the values `SD_1` `SD_2` `SD_3` `SD_4` `SD_5` `SD_6` `SD_7` `SD_8` are transmitted to the client. Once response data (`RD_1` `RD_2` `RD_3` `RD_4` `RD_5` `RD_6` `RD_7` `RD_8`) is available, the block triggers `IND` and provides `RD_1` `RD_2` `RD_3` `RD_4` `RD_5` `RD_6` `RD_7` `RD_8`.
...
- **Counterpart `CLIENT_8`**: `SERVER_8` is only functionally compatible with a `CLIENT_8` block on the client side — the number of send fields on one side must match the number of receive fields on the other.
...
- **Server-side connection** to a communication partner already laid out as a `CLIENT_8` with the matching field count.

## Comparison with similar function blocks

- **[CLIENT_8](CLIENT_8.md)**: the direct counterpart on the client side, with send/receive field counts swapped.
- **[CLIENT_1](CLIENT_1.md) / [SERVER_1](SERVER_1.md)**: the same generic implementation with one send and one receive field each.

## Conclusion

`SERVER_8` provides the generic server variant of the `GEN_SERVER` family tailored to 8 send and 8 receive fields, suiting network connections whose payload count differs from the standard single-field variant.
