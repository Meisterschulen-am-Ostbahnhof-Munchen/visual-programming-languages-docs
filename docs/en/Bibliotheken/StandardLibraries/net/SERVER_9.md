...
The `SERVER_9` function block is the generic server variant with 9 send and 9 receive data fields for communicating with a matching [CLIENT_9](CLIENT_9.md) block. It transmits 9 data values (`SD_1` `SD_2` `SD_3` `SD_4` `SD_5` `SD_6` `SD_7` `SD_8` `SD_9`) to the client and receives 9 data values (`RD_1` `RD_2` `RD_3` `RD_4` `RD_5` `RD_6` `RD_7` `RD_8` `RD_9`) back. Like all `SERVER_*` blocks, it is based on the generic `GEN_SERVER` implementation — the same C++ base as [CLIENT_1](CLIENT_1.md)/[SERVER_1](SERVER_1.md); only the number of send/receive fields differs per instantiation.
...
`SERVER_9` initializes a connection to the matching `CLIENT_9` block via `INIT` (when `QI = TRUE`) or closes it (when `QI = FALSE`); completion is confirmed via `INITO`. Via `RSP`, the values `SD_1` `SD_2` `SD_3` `SD_4` `SD_5` `SD_6` `SD_7` `SD_8` `SD_9` are transmitted to the client. Once response data (`RD_1` `RD_2` `RD_3` `RD_4` `RD_5` `RD_6` `RD_7` `RD_8` `RD_9`) is available, the block triggers `IND` and provides `RD_1` `RD_2` `RD_3` `RD_4` `RD_5` `RD_6` `RD_7` `RD_8` `RD_9`.
...
- **Counterpart `CLIENT_9`**: `SERVER_9` is only functionally compatible with a `CLIENT_9` block on the client side — the number of send fields on one side must match the number of receive fields on the other.
...
- **Server-side connection** to a communication partner already laid out as a `CLIENT_9` with the matching field count.

## Comparison with similar function blocks

- **[CLIENT_9](CLIENT_9.md)**: the direct counterpart on the client side, with send/receive field counts swapped.
- **[CLIENT_1](CLIENT_1.md) / [SERVER_1](SERVER_1.md)**: the same generic implementation with one send and one receive field each.

## Conclusion

`SERVER_9` provides the generic server variant of the `GEN_SERVER` family tailored to 9 send and 9 receive fields, suiting network connections whose payload count differs from the standard single-field variant.
