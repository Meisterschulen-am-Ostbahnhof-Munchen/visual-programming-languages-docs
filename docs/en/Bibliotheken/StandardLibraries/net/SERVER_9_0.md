...
![SERVER_9_0](SERVER_9_0.svg)

* * * * * * * * * *

## Introduction

The `SERVER_9_0` function block is the generic server variant with 9 send and 0 receive data fields for communicating with a matching [CLIENT_0_9](CLIENT_0_9.md) block. It transmits 9 data values (`SD_1` `SD_2` `SD_3` `SD_4` `SD_5` `SD_6` `SD_7` `SD_8` `SD_9`) to the client without receiving any data back. Like all `SERVER_*` blocks, it is based on the generic `GEN_SERVER` implementation — the same C++ base as [CLIENT_1](CLIENT_1.md)/[SERVER_1](SERVER_1.md); only the number of send/receive fields differs per instantiation.
...
- **RSP**: Sends data to the client, carries `QI` and `SD_1` `SD_2` `SD_3` `SD_4` `SD_5` `SD_6` `SD_7` `SD_8` `SD_9`.
...
- No receive fields (`SERVER_9_0` receives no payload data via `IND`, only connection/status information).

## Functionality

`SERVER_9_0` initializes a connection to the matching `CLIENT_0_9` block via `INIT` (when `QI = TRUE`) or closes it (when `QI = FALSE`); completion is confirmed via `INITO`. Via `RSP`, the values `SD_1` `SD_2` `SD_3` `SD_4` `SD_5` `SD_6` `SD_7` `SD_8` `SD_9` are transmitted to the client. Once a response is available, the block triggers `IND`.
...
- **Counterpart `CLIENT_0_9`**: `SERVER_9_0` is only functionally compatible with a `CLIENT_0_9` block on the client side — the number of send fields on one side must match the number of receive fields on the other.
...
- **Server-side connection** to a communication partner already laid out as a `CLIENT_0_9` with the matching field count.

## Comparison with similar function blocks

- **[CLIENT_0_9](CLIENT_0_9.md)**: the direct counterpart on the client side, with send/receive field counts swapped.
- **[CLIENT_1](CLIENT_1.md) / [SERVER_1](SERVER_1.md)**: the same generic implementation with one send and one receive field each.

## Conclusion

`SERVER_9_0` provides the generic server variant of the `GEN_SERVER` family tailored to 9 send and 0 receive fields, suiting network connections whose payload count differs from the standard single-field variant.
