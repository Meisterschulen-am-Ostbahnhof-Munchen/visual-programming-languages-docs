...
All 12 `Output_Qxx` instances are the same reusable composite SubApp `RampLimitFS_TO_logiBUS_QDA_PWM_OPC` (see [sub-block](https://docs.ms-muc-docs.de/projects/4diac-library-reference-docs/en/latest/ExternalLibraries/MyLib_AX/sys/RampLimitFS_TO_logiBUS_QDA_PWM_OPC/) below) and differ only in their parameters:
