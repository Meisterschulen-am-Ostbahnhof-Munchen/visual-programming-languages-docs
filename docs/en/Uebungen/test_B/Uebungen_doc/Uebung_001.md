...
1. The function block `DigitalInput_I1` detects a change at the hardware input `I1`.

2. It updates its data output `IN` with the new value (TRUE or FALSE).

3. Simultaneously, it fires an event on port `IND` (Indication).

4. This event travels via the **Event Connection** to port `REQ` (Request) of the function block `DigitalOutput_Q1`.

5. Only upon receiving the event does `DigitalOutput_Q1` read the value present at its port `OUT` via the **Data Connection** and switch the hardware output accordingly.
