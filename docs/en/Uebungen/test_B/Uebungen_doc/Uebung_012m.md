...
5. Simultaneously, with each read operation (`NVS.GETO`), the retrieved value is sent to the ISOBUS bus via `Q_StringValue.REQ`, enabling other devices to query the current value.
6. Simultaneously, with each read operation (`NVS.GETO`), the retrieved value is sent to the ISOBUS bus, allowing other devices to query the current value.
