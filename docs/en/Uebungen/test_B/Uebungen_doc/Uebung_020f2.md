...
3. **Calculation**: `E_CYCLE` triggers the `REQ` input of `FB_TP`. 3. **Termination**: As soon as the pulse is complete (`Q` goes to `FALSE`), `E_CYCLE` stops automatically to avoid unnecessary CPU load.
