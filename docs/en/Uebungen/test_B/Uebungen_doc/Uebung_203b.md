...
1. The output `DO_UP` is fed to the data output **Right Rotation** (`OUT`), and its `REQ` event is activated via `EO_UP`. The same applies to `DO_DOWN` and **Left Rotation**.
...
1. The signals `DO_UP` and `DO_DOWN` are simultaneously fed to the OR gate **OR_2_BOOL**:
