...
# NOTE: the Function-Block/Adapter reference (Bibliotheken/) and the exercise
# solutions (Uebungen/) were split out of this repo into their own RTD
# projects (see README.md). Their internal folder structure was kept
...
def modify_data(data):
    ...
    for dt in dts:
        link = f'{lib_base_url}Bibliotheken/ExternalLibraries/isobus/pgn/CAN_MSG/' if dt == "CAN_MSG" else f'{dt_base_url}{dt}/'
    ...
    for term, dtype in adapters:
        link = f'{lib_base_url}Bibliotheken/ExternalLibraries/adapter/types/unidirectional/{dtype}/{term}/'
    ...
    for term, ex in io_defs:
        link = f'{lib_base_url}Bibliotheken/StandardLibraries/io/{term}/'
    ...
    for term, t, ex in timers:
        link = f'{lib_base_url}Bibliotheken/{"ExternalLibraries/adapter/iec61131" if term.startswith("AX_FB") else ("ExternalLibraries/adapter/events/unidirectional" if term.startswith("AX_") else ("StandardLibraries/iec61131-3" if term.startswith("FB_") else "StandardLibraries/events"))}/timers/{term}/'
    ...
    for term, t, ex in ffs:
        ...
        link = f'{lib_base_url}Bibliotheken/{path}/{term}/'
        if term == "AX_SR":
            link = f'{lib_base_url}Bibliotheken/ExternalLibraries/adapter/events/unidirectional/ASR_AX_SR/'
    ...
    for steps in ["04", "05", "08"]:
        for t in ["E", "T", "ET"]:
            for suffix in ["", "_loop", "_AX", "_loop_AX"]:
                ...
                link = f'{lib_base_url}Bibliotheken/{lib}/utils/sequence-control/{group}/{term}/'
                if "AX" in suffix:
                    link = f'{lib_base_url}Bibliotheken/{lib}/utils/sequence/{group}/{term}/'
                update_or_add(cat_sequence, {"term": term, "link": link, "ex": ex})

    # Special Sequences
    update_or_add(cat_sequence, {"term": "sequence_ET_04_04", "link": f"{lib_base_url}Bibliotheken/ExternalLibraries/logiBUS/utils/sequence/combi/sequence_ET_04_04/", "type": "sequence"})
    update_or_add(cat_sequence, {"term": "sequence_ET_04_04_AX", "link": f"{lib_base_url}Bibliotheken/ExternalLibraries/logiBUS/utils/sequence/combi/sequence_ET_04_04_AX/", "type": "adapter"})
    ...
    for cat, term, path, t, ex in muxs:
        update_or_add(cat, {"term": term, "link": f'{lib_base_url}Bibliotheken/{path}/{term}/', "type": t, "ex": ex})

    # Update E_STOPWATCH link in cat_timer
    update_or_add(cat_timer, {"term": "E_STOPWATCH", "link": f'{lib_base_url}Bibliotheken/StandardLibraries/utils/timing/E_STOPWATCH/', "type": "event", "ex": "Uebung_020i"})
    ...
    for term, mean in isobus_blocks:
        ...
        link = f'{lib_base_url}Bibliotheken/ExternalLibraries/isobus/UT/Q/{term}/'
    ...
    for term, mean in new_event_types:
        link = f'{lib_base_url}Bibliotheken/StandardLibraries/events/{term}/'
    ...
    for term, mean, t in counter_adapter_types:
        link = f'{lib_base_url}Bibliotheken/ExternalLibraries/adapter/iec61131/counters/{term}/'
    ...
    for term, mean in conversion_adapters:
        link = f'{lib_base_url}Bibliotheken/ExternalLibraries/adapter/conversion/{term}/'
    ...
    for term, mean in signal_adapters:
        link = f'{lib_base_url}Bibliotheken/ExternalLibraries/adapter/utils/{term}/'
        if "TON" in term:
            link = f'{lib_base_url}Bibliotheken/ExternalLibraries/adapter/iec61131/timers/{term}/'
        if term in ("AR_ADD_2", "AR_MULTIME", "AUDI_ADD_2", "AD_SHL"):
            link = f'{lib_base_url}Bibliotheken/ExternalLibraries/adapter/arithmetic/{term}/'
        update_or_add(cat_adapter, {"term": term, "link": link, "mean": mean, "type": "adapter"})
    ...
    for term, mean in new_hw_types:
        link = f'{lib_base_url}Bibliotheken/ExternalLibraries/logiBUS/io/{term}/'
    ...
    for term, mean in new_isobus_types:
        link = f'{lib_base_url}Bibliotheken/ExternalLibraries/isobus/UT/{term}/'
        if term.startswith("IA_"):
            link = f'{lib_base_url}Bibliotheken/ExternalLibraries/isobus/TECU/{term}/'
    ...
    for entry in interlock_types:
        ...
        link = f'{lib_base_url}Bibliotheken/ExternalLibraries/logiBUS/signalprocessing/interlock/{term}/'
    ...
    for term, mean in storage_types:
        ...
        link = f'{lib_base_url}Bibliotheken/{lib}/{term}/'
    ...
    for term, mean, t in signal_processing_types:
        link = f'{lib_base_url}Bibliotheken/ExternalLibraries/logiBUS/signalprocessing/{term}/'
        update_or_add(cat_logic, {"term": term, "link": link, "mean": mean, "type": t})
